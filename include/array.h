...
class array {
 public:
...
#ifdef _GPU
  void check_cuda_error(const char *message, const char *filename,
                        const int lineno);
#endif

  // setup

  void setup(int in_dim_0, int in_dim_1 = 1, int in_dim_2 = 1,
             int in_dim_3 = 1);
  ...
  T *get_ptr_cpu(int in_pos_0, int in_pos_1 = 0, int in_pos_2 = 0,
                 int in_pos_3 = 0);
  T *get_ptr_gpu(int in_pos_0, int in_pos_1 = 0, int in_pos_2 = 0,
                 int in_pos_3 = 0);
  ...
  /*! Initialize array to zero - Valid for numeric data types (int, float,
   * double) */
...
T &array<T>::operator()(int in_pos_0, int in_pos_1) {
  return cpu_data[in_pos_0 +
                  (dim_0 * in_pos_1)];  // column major with matrix indexing
}

template <typename T>
T &array<T>::operator()(int in_pos_0, int in_pos_1, int in_pos_2) {
  return cpu_data[in_pos_0 + (dim_0 * in_pos_1) +
                  (dim_0 * dim_1 *
...
T &array<T>::operator()(int in_pos_0, int in_pos_1, int in_pos_2,
                        int in_pos_3) {
  return cpu_data[in_pos_0 + (dim_0 * in_pos_1) + (dim_0 * dim_1 * in_pos_2) +
                  (dim_0 * dim_1 * dim_2 *
                   in_pos_3)];  // column major with matrix indexing
}
...
T *array<T>::get_ptr_gpu(void) {
  if (gpu_flag == 1)
    ...
    std::cout << "dim_0=" << dim_0 << " dim_1=" << dim_1 << " dim_2=" << dim_2
              << std::endl;
...
template <typename T>
T *array<T>::get_ptr_cpu(int in_pos_0, int in_pos_1, int in_pos_2,
                         int in_pos_3) {
  if (cpu_flag == 1)
    return cpu_data + in_pos_0 + (dim_0 * in_pos_1) +
           (dim_0 * dim_1 * in_pos_2) +
           (dim_0 * dim_1 * dim_2 *
            in_pos_3);  // column major with matrix indexing
  else
...
template <typename T>
T *array<T>::get_ptr_gpu(int in_pos_0, int in_pos_1, int in_pos_2,
                         int in_pos_3) {
  if (gpu_flag == 1)
    return gpu_data + in_pos_0 + (dim_0 * in_pos_1) +
           (dim_0 * dim_1 * in_pos_2) +
           (dim_0 * dim_1 * dim_2 *
            in_pos_3);  // column major with matrix indexing
  else
...
T array<T>::get_max(void) {
  ...
  for (i = 0; i < dim_0 * dim_1 * dim_2 * dim_3; i++) {
    if (((*this).get_ptr_cpu())[i] > max) max = ((*this).get_ptr_cpu())[i];
  }
  return max;
}
...
T array<T>::get_min(void) {
  ...
  for (i = 0; i < dim_0 * dim_1 * dim_2 * dim_3; i++) {
    if (((*this).get_ptr_cpu())[i] < min) min = ((*this).get_ptr_cpu())[i];
  }
  return min;
}
// print
...
void array<T>::print(void) {
  if (dim_3 == 1) {
    ...
    for (k = 0; k < dim_2; k++) {
      ...
      if (threeD) std::cout << std::endl;
    }
  } else {
    std::cout
        << "ERROR: Can only print an array of dimension three or less ...."
        << std::endl;
  }
}
...
#ifdef _GPU
template <typename T>
void array<T>::check_cuda_error(const char *message, const char *filename,
                                const int lineno) {
...
void array<T>::mv_cpu_gpu(void) {
  ...
  check_cuda_error("After Memcpy, asking for too much memory?", __FILE__,
                   __LINE__);
...
void array<T>::mv_gpu_cpu(void) {
  ...
  cudaMemcpy(cpu_data, gpu_data, dim_0 * dim_1 * dim_2 * dim_3 * sizeof(T),
...
void array<T>::cp_gpu_cpu(void) {
  ...
  if (gpu_flag == 0) FatalError("GPU data does not exist");
