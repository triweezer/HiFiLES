...
class eles {
 public:
  ...
  /*! write extra restart file containing x,y,z of solution points instead of
   * solution data */
  ...
  /*! calculate uncorrected transformed gradient of the discontinuous solution
   * at the solution points */
  void calculate_gradient(int in_disu_upts_from);

  /*! calculate corrected gradient of the discontinuous solution at solution
   * points */
  ...
  /*! calculate divergence of transformed discontinuous viscous flux at solution
   * points */
  ...
  /*! calculate divergence of transformed continuous viscous flux at solution
   * points */
  ...
  /*! get pointer to the equivalent of 'dA' (face area) at a flux point in
   * static physical space */
  double *get_tdA_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                           int in_ele);

  /*! get pointer to the equivalent of 'dA' (face area) at a flux point in
   * dynamic physical space */
  double *get_ndA_dyn_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                               int in_ele);

  /*! get a pointer to the normal at a flux point */
  double *get_norm_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                            int in_dim, int in_ele);

  /*! get a pointer to the normal at a flux point in dynamic space */
  double *get_norm_dyn_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                                int in_dim, int in_ele);

  /*! get a CPU pointer to the coordinates at a flux point */
  double *get_loc_fpts_ptr_cpu(int in_inter_local_fpt, int in_ele_local_inter,
                               int in_dim, int in_ele);

  /*! get a GPU pointer to the coordinates at a flux point */
  double *get_loc_fpts_ptr_gpu(int in_inter_local_fpt, int in_ele_local_inter,
                               int in_dim, int in_ele);

  /*! get a CPU pointer to the dynamic physical coordinates at a flux point */
  double *get_pos_dyn_fpts_ptr_cpu(int in_inter_local_fpt,
                                   int in_ele_local_inter, int in_dim,
                                   int in_ele);

  /*! get a pointer to delta of the transformed discontinuous solution at a flux
   * point */
  double *get_delta_disu_fpts_ptr(int in_inter_local_fpt,
                                  int in_ele_local_inter, int in_field,
                                  int in_ele);

  /*! get a pointer to gradient of discontinuous solution at a flux point */
  double *get_grad_disu_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                                 int in_dim, int in_field, int in_ele);

  /*! get a pointer to gradient of discontinuous solution at a flux point */
  double *get_normal_disu_fpts_ptr(int in_inter_local_fpt,
                                   int in_ele_local_inter, int in_field,
   ...
   * point */
  // double* get_norm_tconvisf_fpts_ptr(int in_inter_local_fpt, int
  // in_ele_local_inter, int in_field, int in_ele);

  /*! get a pointer to the subgrid-scale flux at a flux point */
  double *get_sgsf_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
  ...
  void calc_diagnostic_fields_ppts(int in_ele, array<double> &in_disu_ppts,
  ...
  void calc_wall_distance(int n_seg_noslip_inters, int n_tri_noslip_inters,
  ...
  void calc_wall_distance_parallel(array<int> n_seg_noslip_inters,
                                   array<int> n_tri_noslip_inters,
                                   array<int> n_quad_noslip_inters,
  ...
  /*! calculate derivative of position at a solution point (using pre-computed
   * gradients) */
  void calc_d_pos_upt(int in_upt, int in_ele, array<double> &out_d_pos);

  /*! calculate derivative of position at a flux point (using pre-computed
   * gradients) */
  ...
  virtual double compute_inter_detjac_inters_cubpts(int in_inter,
                                                    array<double> d_pos) = 0;
  ...
  virtual double eval_nodal_basis_restart(int in_index,
                                          array<double> in_loc) = 0;

  /*! evaluate derivative of nodal basis */
  virtual double eval_d_nodal_basis(int in_index, int in_cpnt,
                                    array<double> in_loc) = 0;
  ...
  virtual double eval_nodal_s_basis(int in_index, array<double> in_loc,
                                    int in_n_spts) = 0;

  /*! evaluate derivative of nodal shape basis */
  virtual void eval_d_nodal_s_basis(array<double> &d_nodal_s_basis,
                                    array<double> in_loc, int in_n_spts) = 0;

  /*! Calculate SGS flux */
  void calc_sgsf_upts(array<double> &temp_u, array<double> &temp_grad_u,
  ...
  void calc_wall_stress(double rho, array<double> &urot, double ene, double mu,
                        double Pr, double gamma, double y,
  ...
  void CalcIntegralQuantities(int n_integral_quantities,
                              array<double> &integral_quantities);

  /*! Compute time-average diagnostic quantities */
  void CalcTimeAverageQuantities(double &time);

  void compute_wall_forces(array<double> &inv_force, array<double> &vis_force,
                           double &temp_cl, double &temp_cd,
                           std::ofstream &coeff_file, bool write_forces);

  array<double> compute_error(int in_norm_type, double &time);

  array<double> get_pointwise_error(array<double> &sol, array<double> &grad_sol,
                                    array<double> &loc, double &time,
                                    int in_norm_type);

  /*! calculate position of a point in physical (dynamic) space from (r,s,t)
   * coordinates*/
   ...
   * \param[out] out_d_pos - array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_pos_dyn_inters_cubpt(int in_cubpt, int in_face, int in_ele,
   ...
   * \param[out] out_d_pos - array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_d_pos_dyn(array<double> in_loc, int in_ele,
                      array<double> &out_d_pos);

  /*!
   * Calculate derivative of dynamic position wrt reference (initial,static)
   * position at fpt \param[in] in_fpt - ID of flux point within element to
  ...
  /*!
   * Calculate derivative of dynamic position wrt reference (initial,static)
   * position at upt \param[in] in_upt - ID of solution point within element to
   * evaluate at \param[in] in_ele - local element ID \param[out] out_d_pos -
   * array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  ...
  /*!
   * Calculate derivative of dynamic position wrt reference (initial,static)
   position at volume cubature point
   * \param[in] in_cubpt - ID of cubature point within element to evaluate at
   * \param[in] in_ele - local element ID
   * \param[out] out_d_pos - array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_d_pos_dyn_vol_cubpt(int in_cubpt, int in_ele,
                                array<double> &out_d_pos);
   ...
   * \param[out] out_d_pos - array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
   */
  void calc_d_pos_dyn_inters_cubpt(int in_cubpt, int in_face, int in_ele,
                                   array<double> &out_d_pos);

  /*! pre-computing shape basis contributions at flux points for more efficient
   * access */
  void store_nodal_s_basis_fpts(void);

  /*! pre-computing shape basis contributions at solution points for more
   * efficient access */
  void store_nodal_s_basis_upts(void);

  /*! pre-computing shape basis contributions at plot points for more efficient
   * access */
  ...
  /*! pre-computing shape basis deriavative contributions at flux points for
   * more efficient access */
  void store_d_nodal_s_basis_fpts(void);

  /*! pre-computing shape basis derivative contributions at solution points for
   * more efficient access */
  ...
  double *get_grid_vel_fpts_ptr(int in_ele, int in_ele_local_inter,
                                int in_inter_local_fpt, int in_dim);

  /*! Set the transformation variables for dynamic-physical -> static-physical
   * frames */
  void set_transforms_dynamic(void);

  /* --- Geometric Conservation Law (GCL) Funcitons --- */
  /*! Update the dynamic transformation variables with the GCL-corrected
  ...
  double *get_disu_GCL_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                                int in_ele);
  /* --------------------------------------------------- */
  ...
  */
  ...
  /*!
  Description: Mesh velocity at flux points (interpolated using shape basis
  ...
  */
  ...
  /*! temporary grid velocity storage at a single solution point (transformed to
   * static frame) */
  array<double> temp_v_ref;

  /*! temporary flux storage for GCL at a single solution point (transformed to
   * static frame) */
  ...
  /*! extra arrays for similarity model: Leonard tensors, velocity/energy
   * products */
  ...
  /*! determinant of jacobian at volume cubature points. TODO: what is this
   * really? */
  array<array<double>> vol_detjac_inters_cubpts;

  /*! determinant of volume jacobian at cubature points. TODO: what is this
   * really? */
  array<array<double>> vol_detjac_vol_cubpts;

  /*! Full vector-transform matrix from static physical->computational frame, at
   * solution points [Determinant of Jacobian times inverse of Jacobian]
   * [J*G^-1] */
  array<double> JGinv_upts;

  /*! Full vector-transform matrix from static physical->computational frame, at
   * flux points [Determinant of Jacobian times inverse of Jacobian] [J*G^-1] */
  array<double> JGinv_fpts;

  /*! Magnitude of transformed face-area normal vector from computational ->
   * static-physical frame [magntiude of (normal dot inverse static
   * transformation matrix)] [ |J*(G^-1)*(n*dA)| ] */
  ...
  /*! magnitude of transformed face-area normal vector from static-physical ->
   * dynamic-physical frame [magntiude of (normal dot inverse dynamic
   * transformation matrix)] [ |J*(G^-1)*(n*dA)| ] */
  ...
  */
  ...
  /*! operator to go from transformed discontinuous solution at the solution
   * points to transformed discontinuous solution at the flux points */
  array<double> opp_0;
  ...
  /*! operator to go from transformed discontinuous inviscid flux at the
   * solution points to divergence of transformed discontinuous inviscid flux at
   * the solution points */
  ...
  /*! operator to go from transformed discontinuous inviscid flux at the
   * solution points to normal transformed discontinuous inviscid flux at the
   * flux points */
  ...
  /*! operator to go from normal correction inviscid flux at the flux points to
   * divergence of correction inviscid flux at the solution points*/
  array<double> opp_3;
  ...
  /*! operator to go from transformed solution at solution points to transformed
   * gradient of transformed solution at solution points */
  ...
  /*! operator to go from transformed solution at flux points to transformed
   * gradient of transformed solution at solution points */
  ...
  /*! operator to go from transformed solution at solution points to transformed
   * gradient of transformed solution at flux points */
  array<double> opp_6;
  ...
  /*! operator to go from discontinuous solution at the solution points to
   * discontinuous solution at the plot points */
  array<double> opp_p;

  array<array<double>> opp_inters_cubpts;
  array<double> opp_volume_cubpts;

  /*! operator to go from discontinuous solution at the restart points to
   * discontinuous solution at the solutoin points */
  array<double> opp_r;
