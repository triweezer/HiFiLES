 ...
 * \brief Headers of the main subroutines for creating the sparse
 * matrices-by-blocks. The subroutines and functions are in the
 * <i>matrix_structure.cpp</i> file. \author Aerospace Design Laboratory
 * (Stanford University) <http://su2.stanford.edu>. \version 2.0.8
 *
...
class CSysMatrix {
 private:
  ...
  unsigned long
      *row_ptr; /*!< \brief Pointers to the first element in each row. */
  unsigned long
      *col_ind; /*!< \brief Column index for each of the elements in val(). */
  unsigned long
      nnz; /*!< \brief Number of possible nonzero entries in the matrix. */
  double
      *block; /*!< \brief Internal array to store a subblock of the matrix. */
  double *prod_block_vector; /*!< \brief Internal array to store the product of
                                a subblock with a vector. */
  double *prod_row_vector; /*!< \brief Internal array to store the product of a
                              matrix-by-blocks "row" with a vector. */
  double
      *aux_vector;   /*!< \brief Auxilar array to store intermediate results. */
 ...
 public:
   ...
   */
  void Initialize(int n_verts, int n_verts_global, int n_var, int n_eqns,
                  array<array<int>> &v2e, array<int> &v2n_e, array<int> &e2v);

  /*!
   * \brief Assings values to the sparse-matrix structure.
   * \param[in] val_nPoint - Number of points in the nPoint x nPoint block
   * structure \param[in] val_nVar - Number of nVar x nVar variables in each
   * subblock of the matrix-by-block structure. \param[in] val_nEq - Number of
   * nEqn x nVar variables in each subblock of the matrix-by-block structure.
   ...
   * \param[in] preconditioner - If <code>TRUE</code> then it use a
   * preconditioner.
   */
  void SetIndexes(int n_verts, int n_verts_global, int n_var, int n_eqns,
                  unsigned long *val_row_ptr, unsigned long *val_col_ind,
  ...
  /*!
   * \brief Copies the block (i,j) of the matrix-by-blocks structure in the
   * internal variable *block. \param[in] block_i - Indexes of the block in the
   * matrix-by-blocks structure. \param[in] block_j - Indexes of the block in
   * the matrix-by-blocks structure.
   */
  void GetBlock(unsigned long block_i, unsigned long block_j);

  /*!
   * \brief Displays the content of the internal variable <i>*block</i> (for
   * debug purposes).
   */
  void DisplayBlock(void);

  /*!
   * \brief Returns the content of the internal variable <i>*block</i> (for
   * debug purposes).
   */
   ...
   * \param[in] block_i - Indexes of the block in the matrix-by-blocks
   * structure. \param[in] block_j - Indexes of the block in the
   * matrix-by-blocks structure. \param[in] **val_block - Block to set to
   * A(i,j).
   */
  void SetBlock(unsigned long block_i, unsigned long block_j,
                double **val_block);
   ...
   * \param[in] block_i - Indexes of the block in the matrix-by-blocks
   * structure. \param[in] block_j - Indexes of the block in the
   * matrix-by-blocks structure. \param[in] **val_block - Block to add to
   * A(i,j).
   */
  void AddBlock(unsigned long block_i, unsigned long block_j,
                double **val_block);
  void AddBlock(unsigned long block_i, unsigned long block_j,
                array<double> val_block);

  /*!
   * \brief Subtracts the specified block to the sparse matrix.
   * \param[in] block_i - Indexes of the block in the matrix-by-blocks
   * structure. \param[in] block_j - Indexes of the block in the
   * matrix-by-blocks structure. \param[in] **val_block - Block to subtract to
   * A(i,j).
   */
  void SubtractBlock(unsigned long block_i, unsigned long block_j,
                     double **val_block);
   ...
   */
  void AddVal2Diag(unsigned long block_i, double *val_matrix,
                   unsigned short val_nDim, unsigned short val_nDiatomics);
  ...
  /*!
   * \brief Performs the Gauss Elimination algorithm to solve the linear
   * subsystem of the (i,i) subblock and rhs. \param[in] block_i - Index of the
   * (i,i) subblock in the matrix-by-blocks structure. \param[in] rhs -
   * Right-hand-side of the linear system. \return Solution of the linear system
   * (overwritten on rhs).
   */
  ...
  /*!
   * \brief Performs the Gauss Elimination algorithm to solve the linear
   * subsystem of the (i,i) subblock and rhs. \param[in] A - matrix-by-blocks
   * structure. \param[in] rhs - Right-hand-side of the linear system. \return
   * Solution of the linear system (overwritten on rhs).
   */
  void Gauss_Elimination(double *Block, double *rhs);

  /*!
   * \fn void CSysMatrix::ProdBlockVector(unsigned long block_i, unsigned long
   * block_j, double* vec); \brief Performs the product of the block (i,j) by
   * vector vec. \param[in] block_i - Indexes of the block in the
   * matrix-by-blocks structure. \param[in] block_j - Indexes of the block in
   * the matrix-by-blocks structure. \param[in] vec - Vector to be multiplied by
   * the block (i,j) of the sparse matrix A. \return Product of A(i,j) by vector
   * *vec (stored at *prod_block_vector).
   */
  void ProdBlockVector(unsigned long block_i, unsigned long block_j,
                       const CSysVector &vec);

  /*!
   * \brief Performs the product of i-th row of the upper part of a sparse
   * matrix by a vector. \param[in] vec - Vector to be multiplied by the upper
   * part of the sparse matrix A. \param[in] row_i - Row of the matrix to be
   * multiplied by vector vec. \return prod Result of the product U(A)*vec
   * (stored at *prod_row_vector).
   */
  void UpperProduct(CSysVector &vec, unsigned long row_i);

  /*!
   * \brief Performs the product of i-th row of the lower part of a sparse
   * matrix by a vector. \param[in] vec - Vector to be multiplied by the lower
   * part of the sparse matrix A. \param[in] row_i - Row of the matrix to be
   * multiplied by vector vec. \return prod Result of the product L(A)*vec
   * (stored at *prod_row_vector).
   */
  void LowerProduct(CSysVector &vec, unsigned long row_i);

  /*!
   * \brief Performs the product of i-th row of the diagonal part of a sparse
   * matrix by a vector. \param[in] vec - Vector to be multiplied by the
   * diagonal part of the sparse matrix A. \param[in] row_i - Row of the matrix
   * to be multiplied by vector vec. \return prod Result of the product D(A)*vec
   * (stored at *prod_row_vector).
   */
   ...
   * \param[in] block_i - Indexes of the block in the matrix-by-blocks
   * structure. \param[out] invBlock - Inverse block.
   */
   ...
   */
  void ComputeResidual(const CSysVector &sol, const CSysVector &f,
                       CSysVector &res);
};
...
class CSysMatrixVectorProduct : public CMatrixVectorProduct {
 private:
  CSysMatrix
      *sparse_matrix; /*!< \brief pointer to matrix that defines the product. */
  solution *FlowSol; /*!< \brief pointer to structure containing solution data &
                        configuration. */

 public:
  /*!
   * \brief constructor of the class
   * \param[in] matrix_ref - matrix reference that will be used to define the
   * products
   */
...
class CLU_SGSPreconditioner : public CPreconditioner {
 private:
  CSysMatrix *sparse_matrix; /*!< \brief pointer to matrix that defines the
                                preconditioner. */
  solution *FlowSol; /*!< \brief pointer to structure containing solution data &
                        configuration. */
