...
const double eps =
    std::numeric_limits<double>::epsilon(); /*!< \brief machine epsilon */
...
class CSysVector {
 private:
  unsigned long nElm; /*!< \brief total number of elements (or number elements
                         on this processor) */
  unsigned long
      nElmDomain; /*!< \brief total number of elements (or number elements on
                     this processor without Ghost cells) */
  unsigned long
      nElmGlobal; /*!< \brief total number of elements over all processors */
  unsigned short nVar; /*!< \brief number of elements in a block */
  unsigned long nBlk;  /*!< \brief number of blocks (or number of blocks on this
                          processor) */
  unsigned long nBlkDomain; /*!< \brief number of blocks (or number of blocks on
                               this processor without Ghost cells) */
  unsigned long
      myrank;      /*!< \brief processor rank (only used for parallel runs) */
 ...
 public:
   ...
   * \param[in] val - default value for elements
   */
  CSysVector(const unsigned long &numBlk, const unsigned long &numBlkDomain,
             const unsigned short &numVar, const double &val = 0.0);
   ...
   * \param[in] u_array - vector stored as array being copied
   */
  explicit CSysVector(const unsigned long &numBlk,
                      const unsigned long &numBlkDomain,
                      const unsigned short &numVar, const double *u_array);
   ...
   * \param[in] val - default value for elements
   */
  void Initialize(const unsigned long &numBlk, const unsigned short &numVar,
                  const double &val);
  ...
  /*!
   * \brief return the number of variables at each block (typically number per
   * node)
   */
   ...
   */
  void Equals_AX_Plus_BY(const double &a, CSysVector &x, const double &b,
                         CSysVector &y);
   ...
   */
  void SetBlock(unsigned long val_ipoint, unsigned short val_var,
                double val_residual);
