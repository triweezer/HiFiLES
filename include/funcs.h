...
void get_opp_3_tri(array<double> &opp_3, array<double> &loc_upts_tri,
                   array<double> &loc_fpts_tri, array<double> &vandermonde_tri,
...
void get_opp_3_dg(array<double> &opp_3_dg, array<double> &loc_upts_tri,
                  array<double> &loc_fpts_tri, int n_upts_per_tri, int order);
...
void compute_filt_matrix_tri(array<double> &Filt,
                             ...
                             int n_upts_per_ele, int order, double c_tri,
...
derivatives BEGIN:*/
// Convolution function: returns array representation of polynomial that is
// result of multiplication of polynomial1 and polynomial2
array<double> convol(array<double> &polynomial1, array<double> &polynomial2);

// LagrangeP function: returns lagrange polynomial of order "order", with value
// of unity at given node number "node", after substituting polynomial "subs"
// where independent variable in polynomial goes
/*
...
// multPoly function: multiplies polynomials symbolically by stacking them (puts
// them in different rows)
template <typename T>
array<T> multPoly(array<T> &p1, array<T> &p2);

// nodeFunctionTri function: returns the complete shape function of triangles at
// a specific global node in_index given in_n_spts, the total number of nodes in
// the triangle, // and the index_location_array: first row in
// index_location_array contains indeces of r arranged in ascending global node
// number; second row contains indeces of s arranged in ascending global node
// number; third row contains indeces of t arranged in ascending global node
// number; refer to Hughes pp 169 to see link between r/s indeces ordering and
// global indeces ordering
...
// output from this function is eventually passed as a parameter to the
// nodeFunctionTri function

array<int> linkTriangleNodes(int in_n_spts);

// addPoly function: returns a 3D matrix, the stacking in the 3rd dimension
// represents polynomial addition adds polynomials by placing them in different
// layers

array<double> addPoly(array<double> &p1, array<double> &p2);

// diffPoly function: returns a 3D matrix that represents a polynomial
// differentiated with respect to a dimension.

array<double> diffPoly(array<double> &p, array<int> &term2Diff);

// evalPoly function: returns a double, which is the value of a polynomial "p"
// evaluated at coordinates coords; the height of matrix representing polynomial
// p must equal the number of elements (columns) of array coords

double evalPoly(array<double> p, array<double> coords);

// createEquispacedArray: returns array with nPoints values equispaced between a
// and b (in that order)
...
// eval_dd_nodal_s_basis_new function: new implementation of function that finds
// nth derivatives with respect to r or s at each of the triangle nodes
