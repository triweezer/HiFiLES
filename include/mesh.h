 ...
 *                                by Prof. Jameson. (Aero/Astro Dept. Stanford
 * University). \version 1.0.0
 *
...
class mesh {
 public:
  ...
  array<int> c2v, c2n_v, ctype, bctype_c, ic2icg, iv2ivg, ic2loc_c, f2c,
      f2loc_f, c2f, c2e, f2v, f2n_v, e2v, v2n_e;
  array<array<int>> v2e;
  ...
  std::map<std::string, int> bc_name;

  /** inverse of bc_name */
  std::map<int, std::string> bc_flag;
  ...
  void setup(solution *in_FlowSol, array<double> &in_xv, array<int> &in_c2v,
             array<int> &in_c2n_v, array<int> &in_iv2ivg, array<int> &in_ctype);
 ...
 private:
  ...
  // bool set_2D_StiffMat_ele_tet(array<double> &stiffMat_ele,int ele_id,
  // solution *FlowSol);

  /** create individual-element stiffness matrix - hexahedrons */
  // bool set_2D_StiffMat_ele_hex(array<double> &stiffMat_ele,int ele_id,
  // solution *FlowSol);

  /**
   * transfrom single-element stiffness matrix to nodal contributions in order
   * to add to global stiffness matrix
   */
  ...
  /** Set given/known displacements of vertices on moving boundaries in linear
   * system */
   ...
   * --- This version here has been stripped down to the bare essentials needed
   * for HiFiLES For more details (and additional awesome features), see
   * su2.stanford.edu */

  /* These functions will (eventually) be replaced with pre-existing HiFiLES
   * functions, but
   * for now, a HUGE THANKS to the SU^2 dev team for making this practically
   * plug & play! */

  /*!
   * \brief Add the stiffness matrix for a 2-D triangular element to the global
   * stiffness matrix for the entire mesh (node-based). \param[in]
   * StiffMatrix_Elem - Element stiffness matrix to be filled. \param[in]
   * PointCornders - Element vertex ID's
   */
  void add_FEA_stiffMat(array<double> &stiffMat_ele, array<int> &PointCorners);

  /*!
   * \brief Build the stiffness matrix for a 3-D hexahedron element. The result
   * will be placed in StiffMatrix_Elem. \param[in] geometry - Geometrical
   * definition of the problem. \param[in] StiffMatrix_Elem - Element stiffness
   * matrix to be filled. \param[in] CoordCorners[8][3] - Index value for Node 1
   * of the current hexahedron.
   */
   ...
   */
  double ShapeFunc_Hexa(double Xi, double Eta, double Mu,
                        double CoordCorners[8][3], double DShapeFunction[8][4]);
   ...
   * \param[in] shp[8][4] - Shape function information
   */
  double ShapeFunc_Tetra(double Xi, double Eta, double Mu,
   ...
   * \param[in] shp[8][4] - Shape function information
   */
  double ShapeFunc_Pyram(double Xi, double Eta, double Mu,
                         double CoordCorners[8][3],
                         double DShapeFunction[8][4]);
   ...
   * \param[in] shp[8][4] - Shape function information
   */
  double ShapeFunc_Wedge(double Xi, double Eta, double Mu,
                         double CoordCorners[8][3],
                         double DShapeFunction[8][4]);
   ...
   * \param[in] shp[8][4] - Shape function information
   */
  double ShapeFunc_Triangle(double Xi, double Eta, double CoordCorners[8][3],
                            double DShapeFunction[8][4]);
   ...
   * \param[in] shp[8][4] - Shape function information
   */
  double ShapeFunc_Rectangle(double Xi, double Eta, double CoordCorners[8][3],
                             double DShapeFunction[8][4]);
};
