...
void calc_source_SA_2d(array<double> &in_u, array<double> &in_grad_u, double &d,
  ...
  // (solve for S_bar)
  f_v1 = pow(in_u(4) / mu, 3.0) /
         (pow(in_u(4) / mu, 3.0) + pow(run_input.c_v1, 3.0));
  f_v2 = 1.0 - psi / (1.0 + psi * f_v1);
  S_bar = pow(mu * psi / rho, 2.0) * f_v2 /
          (pow(run_input.Kappa, 2.0) * pow(d, 2.0));

  // (solve for S_tilde)
  if (S_bar >= -run_input.c_v2 * S)
    S_tilde = S + S_bar;
  else
    S_tilde = S + S * (pow(run_input.c_v2, 2.0) * S + run_input.c_v3 * S_bar) /
                      ((run_input.c_v3 - 2.0 * run_input.c_v2) * S - S_bar);

  // (production term)
  nu_t_prod = run_input.c_b1 * S_tilde * mu * psi;

  // solve for non-conservative diffusion term for eddy viscosity
  nu_t_diff = (1.0 / run_input.omega) *
              (run_input.c_b2 * rho *
               (pow(dnu_tilde_dx, 2.0) + pow(dnu_tilde_dy, 2.0)));

  // solve for destruction term for eddy viscosity
  c_w1 = run_input.c_b1 / pow(run_input.Kappa, 2.0) +
         (1.0 / run_input.omega) * (1.0 + run_input.c_b2);
  r = std::min(
      (mu * psi / rho) / (S_tilde * pow(run_input.Kappa, 2.0) * pow(d, 2.0)),
      10.0);
  g = r + run_input.c_w2 * (pow(r, 6.0) - r);
  f_w = g * pow((1.0 + pow(run_input.c_w3, 6.0)) /
                    (pow(g, 6.0) + pow(run_input.c_w3, 6.0)),
                1.0 / 6.0);
