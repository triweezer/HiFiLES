...
double eval_lagrange(double in_r, int in_mode, array<double> &in_loc_pts) {
  ...
  for (i = 0; i < in_loc_pts.get_dim(0); i++) {
    if (i != in_mode) {
      dtemp_0 = dtemp_0 * ((in_r - in_loc_pts(i)) /
                           (in_loc_pts(in_mode) - in_loc_pts(i)));
    }
  }
...
double eval_legendre(double in_r, int in_mode) {
  ...
  if (in_mode == 0) {
    ...
    d_temp = ((2 * in_mode - 1) * in_r * eval_legendre(in_r, in_mode - 1) -
              (in_mode - 1) * eval_legendre(in_r, in_mode - 2)) /
             in_mode;
  }
...
double eval_d_legendre(double in_r, int in_mode) {
  ...
  if (in_mode == 0) {
    ...
    if (in_r > -1.0 && in_r < 1.0) {
      d_temp = (in_mode * ((in_r * eval_legendre(in_r, in_mode)) -
                           eval_legendre(in_r, in_mode - 1))) /
               ((in_r * in_r) - 1.0);
    } else {
      if (in_r == -1.0) {
...
double eval_d_vcjh_1d(double in_r, int in_mode, int in_order, double in_eta) {
  ...
  if (in_mode == 0)  // left correction function
  {
    if (in_order == 0) {
      // if (in_eta != 0)
      //     FatalError("P=0 only compatible with DG. Set VCJH scheme to 1 OR
      //     eta to 0.0")

      dtemp_0 = 0.5 * pow(-1.0, in_order) *
                (eval_d_legendre(in_r, in_order) -
                 ((eval_d_legendre(in_r, in_order + 1)) / (1.0 + in_eta)));
    } else {
      dtemp_0 = 0.5 * pow(-1.0, in_order) *
                (eval_d_legendre(in_r, in_order) -
                 (((in_eta * eval_d_legendre(in_r, in_order - 1)) +
                   eval_d_legendre(in_r, in_order + 1)) /
                  (1.0 + in_eta)));
    }
  } else if (in_mode == 1)  // right correction function
  {
    if (in_order == 0) {
      // if (in_eta != 0)
      //    FatalError("P=0 only compatible with DG. Set VCJH scheme to 1 OR eta
      //    to 0.0")

      dtemp_0 =
          0.5 * (eval_d_legendre(in_r, in_order) +
                 ((eval_d_legendre(in_r, in_order + 1)) / (1.0 + in_eta)));
    } else {
      dtemp_0 = 0.5 * (eval_d_legendre(in_r, in_order) +
                       (((in_eta * eval_d_legendre(in_r, in_order - 1)) +
                         eval_d_legendre(in_r, in_order + 1)) /
                        (1.0 + in_eta)));
    }
...
double eval_d_oesfr_1d(double in_r, int in_mode, int in_order) {
  ...
  aP = (1.0 / pow(2.0, in_order)) * factorial(2 * in_order) /
       (factorial(in_order) * factorial(in_order));
  eta = cVal * (0.5 * (2 * in_order + 1)) * (aP * factorial(in_order)) *
        (aP * factorial(in_order));

  if (in_mode == 0)  // left correction function
    dtemp_0 = 0.5 * pow(-1.0, in_order) *
              (eval_d_legendre(in_r, in_order) -
               (((eta * eval_d_legendre(in_r, in_order - 1)) +
                 eval_d_legendre(in_r, in_order + 1)) /
                (1.0 + eta)));
...
void get_opp_3_tri(array<double> &opp_3, array<double> &loc_upts_tri,
                   array<double> &loc_1d_fpts, array<double> &vandermonde_tri,
                   array<double> &inv_vandermonde_tri, int n_upts_per_tri,
                   int order, double c_tri, int vcjh_scheme_tri) {
  ...
  compute_filt_matrix_tri(Filt, vandermonde_tri, inv_vandermonde_tri,
                          n_upts_per_tri, order, c_tri, vcjh_scheme_tri,
                          loc_upts_tri);
...
void get_opp_3_dg(array<double> &opp_3_dg, array<double> &loc_upts_tri,
                  array<double> &loc_1d_fpts, int n_upts_per_tri, int order) {
  int i, j, k;
  array<double> loc(2);
...
void compute_modal_filter_1d(array<double> &filter_upts,
                             array<double> &vandermonde,
                             array<double> &inv_vandermonde, int N, int order) {
  int i, j, ind = 0;
  ...
  // std::cout<<"modal coeffs:"<<std::endl;
  // modal.print();

#if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS

  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, N, N, N, 1.0,
              vandermonde.get_ptr_cpu(), N, modal.get_ptr_cpu(), N, 0.0,
              mtemp.get_ptr_cpu(), N);

  cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, N, N, N, 1.0,
              mtemp.get_ptr_cpu(), N, inv_vandermonde.get_ptr_cpu(), N, 0.0,
              filter_upts.get_ptr_cpu(), N);
...
// Compute a modal filter matrix for a triangular element, given Vandermonde
// matrix and inverse
void compute_modal_filter_tri(array<double> &filter_upts,
                              array<double> &vandermonde,
                              array<double> &inv_vandermonde, int N,
                              int order) {
  int i, j, ind = 0;
...
// Compute a modal filter matrix for a tetrahedral element, given Vandermonde
// matrix and inverse
void compute_modal_filter_tet(array<double> &filter_upts,
                              array<double> &vandermonde,
                              array<double> &inv_vandermonde, int N,
                              int order) {
  int i, j, k, ind = 0;
...
void compute_filt_matrix_tri(array<double> &Filt,
                             array<double> &vandermonde_tri,
                             array<double> &inv_vandermonde_tri, int n_upts_tri,
                             int order, double c_tri, int vcjh_scheme_tri,
                             array<double> &loc_upts_tri) {
  // -----------------
  // VCJH Filter
  // -----------------
  double ap;
  ...
  // 1D prep
  ap = 1. / pow(2.0, order) * factorial(2 * order) /
       (factorial(order) * factorial(order));

  c_sd_1d = (2 * order) / ((2 * order + 1) * (order + 1) *
                           (factorial(order) * ap) * (factorial(order) * ap));
  c_hu_1d =
      (2 * (order + 1)) / ((2 * order + 1) * order * (factorial(order) * ap) *
                           (factorial(order) * ap));
  ...
  for (int i = 0; i < n_upts_tri; i++) {
    for (int j = 0; j < n_upts_tri; j++) {
      tempr(i, j) = eval_dr_dubiner_basis_2d(loc_upts_tri(0, i),
                                             loc_upts_tri(1, i), j, order);
      temps(i, j) = eval_ds_dubiner_basis_2d(loc_upts_tri(0, i),
                                             loc_upts_tri(1, i), j, order);
    }
  }
  ...
  for (int i = 0; i < n_upts_tri; ++i) Identity(i, i) = 1.;

  // Set array with binomial coefficients multiplied by value of c
  for (int k = 0; k < (order + 1); k++) {
    c_coeff(k) = (1. / n_upts_tri) *
                 (factorial(order) / (factorial(k) * factorial(order - k)));
    // std::cout << "k=" << k << "coeff= " << c_coeff(k) << std::endl;
  }
  ...
  for (int k = 0; k < (order + 1); k++) {
    ...
    // std::cout<<"D_high_order(k)"<<std::endl;
    ...
    // std::cout<<"V^T*D_high_order(k)*V"<<std::endl;
    // mtemp_2.print();
    // std::cout << std::endl;
  ...
  for (int i = 0; i < n_upts_tri; i++)
    for (int j = 0; j < n_upts_tri; j++) mtemp_1(i, j) += Identity(i, j);
  ...
  if (vcjh_scheme_tri==0)
  {
    ...
    double cp = 1./pow(2.0,order)*factorial(2*order)/
  (factorial(order)*factorial(order)); double kappa =
  (2*order+1)/2*(factorial(order)*cp)*(factorial(order)*cp); frac = 1./
  (1+c_1d*kappa);
  }
  else if (vcjh_scheme_tri==1) // DG
  {
...
double eval_div_dg_tri(array<double> &in_loc, int in_edge, int in_edge_fpt,
                       int in_order, array<double> &in_loc_fpts_1d) {
  ...
  for (int i = 0; i < n_upts_tri; i++) {
    ...
    for (int j = 0; j < cub1d.get_n_pts(); j++) {
      ...
      integral += cub1d.get_weight(j) *
                  eval_dubiner_basis_2d(r, s, i, in_order) * gdotn_at_cubpt;
    }
  ...
  div_vcjh_basis = 0.;
  for (int i = 0; i < n_upts_tri; i++)
    div_vcjh_basis += coeff_divg(i, 0) *
                      eval_dubiner_basis_2d(in_loc(0), in_loc(1), i, in_order);

  return div_vcjh_basis;
}

// get intel mkl csr 4 array format
void array_to_mklcsr(array<double> &in_array, array<double> &out_data,
                     array<int> &out_cols, array<int> &out_b,
                     array<int> &out_e) {
  int i, j;
...
void array_to_ellpack(array<double> &in_array, array<double> &out_data,
                      array<int> &out_cols, int &nnz_per_row) {
  ...
  for (int i = 0; i < n_rows; i++) {
    ...
    for (int j = 0; j < n_cols; j++) {
      // std::cout << "in_array=" << in_array(i,j) << std::endl;
      if (abs(in_array(i, j)) >= zero_tol) temp++;
    }
    // std::cout << "temp= " << temp << std::endl;
    if (temp > nnz_per_row) nnz_per_row = temp;
  }
  ...
  // std::cout << "nnz_per_row*n_rows=" << nnz_per_row*n_rows << std::endl;
  int index;
  for (int i = 0; i < n_rows; i++) {
    int count = 0;
    for (int j = 0; j < n_cols; j++) {
...
#ifdef _GPU
/*
void array_to_cusparse_csr(array<double>& in_array, cusparseHybMat_t &hyb_array,
cusparseHandle_t& handle)
{
  int n_rows = in_array.get_dim(0);
  int n_cols = in_array.get_dim(1);

  std::cout << "Converting to hybrid format" << std::endl;
  ...
  if (status != CUSPARSE_STATUS_SUCCESS){
    std::cout << "error create Mat Desc" << std::endl;
    exit(1);
  }
  ...
  cusparseDnnz(handle, CUSPARSE_DIRECTION_ROW, n_rows,
n_cols,mat_description,in_array.get_ptr_gpu(),n_rows,nnz_per_row.get_ptr_cpu(),&nnzTotalDevHostPtr);
...
array<double> rst_to_abc(double in_r, double in_s,
                         double in_t)  // CHECK (PASSING)
{
  array<double> abc(3);
...
double eval_jacobi(double in_r, int in_alpha, int in_beta, int in_mode) {
  ...
  if (in_mode == 0) {
    ...
    double dtemp_0, dtemp_1, dtemp_2, dtemp_3, dtemp_4, dtemp_5, dtemp_6,
        dtemp_7, dtemp_8, dtemp_9, dtemp_10, dtemp_11, dtemp_12, dtemp_13,
        dtemp_14;
...
double eval_dubiner_basis_2d(double in_r, double in_s, int in_mode,
                             int in_basis_order) {
    ...
    for (k = 0; k < in_basis_order + 1; k++) {
      for (j = 0; j < k + 1; j++) {
        ...
        if (mode == in_mode)  // found the correct mode
        {
          ...
          dubiner_basis_2d =
              sqrt(2.0) * jacobi_0 * jacobi_1 * pow(1.0 - ab(1), i);
        }
        mode++;
      }
    }
  } else {
    std::cerr << "ERROR: Invalid mode when evaluating Dubiner basis ...."
              << std::endl;
  }
...
double eval_dr_dubiner_basis_2d(double in_r, double in_s, int in_mode,
                                int in_basis_order) {
  ...
  if (in_mode < n_dof) {
      ...
      for (j = 0; j < k + 1; j++) {
        ...
        if (mode == in_mode)  // found the correct mode
        {
          ...
          if (i == 0)  // to avoid singularity
          {
            ...
            dr_dubiner_basis_2d =
                2.0 * sqrt(2.0) * jacobi_0 * jacobi_1 * pow(1.0 - ab(1), i - 1);
          }
        }
        mode++;
      }
    ...
    std::cerr << "ERROR: Invalid mode when evaluating basis ...." << std::endl;
  }
...
double eval_ds_dubiner_basis_2d(double in_r, double in_s, int in_mode,
                                int in_basis_order) {
    ...
    for (k = 0; k < in_basis_order + 1; k++) {
      for (j = 0; j < k + 1; j++) {
        ...
        if (mode == in_mode)  // find the correct mode
        {
          ...
          jacobi_3 =
              eval_grad_jacobi(ab(1), (2 * i) + 1, 0, j) * pow(1.0 - ab(1), i);
          jacobi_4 = eval_jacobi(ab(1), (2 * i) + 1, 0, j) * i *
                     pow(1.0 - ab(1), i - 1);
          ...
          if (i == 0)  // to avoid singularity
          {
            ...
            ds_dubiner_basis_2d =
                sqrt(2.0) * ((jacobi_0 * jacobi_1 * pow(1.0 - ab(1), i - 1) *
                              (1.0 + ab(0))) +
...
double eval_dubiner_basis_3d(double in_r, double in_s, double in_t, int in_mode,
                             int in_basis_order) {
  double sdubiner_basis_3d;

  int n_dof =
      ((in_basis_order + 1) * (in_basis_order + 2) * (in_basis_order + 3)) / 6;
      ...
      for (n = 0; n < m + 1; n++) {
        for (k = 0; k < n + 1; k++) {
          ...
          if (mode == in_mode)  // found the correct mode
          {
            ...
            sdubiner_basis_3d = 2.0 * sqrt(2.0) * jacobi_0 * jacobi_1 *
                                jacobi_2 * pow(1.0 - abc(1), i) *
                                pow(1 - abc(2), i + j);
          }
          mode++;
        }
...
double eval_grad_dubiner_basis_3d(double in_r, double in_s, double in_t,
                                  int in_mode, int in_basis_order,
                                  int component) {
      ...
      for (n = 0; n < m + 1; n++) {
        for (k = 0; k < n + 1; k++) {
          ...
          if (mode == in_mode)  // found the correct mode
          {
            ...
            dt_sdubiner =
                0.5 * (1. + abc(0)) * dr_sdubiner + 0.5 * (1. + abc(1)) * temp;
            temp = dhc * pow(0.5 * (1. - abc(2)), i + j);

            if (i + j > 0) {
              temp = temp -
                     0.5 * (i + j) * (hc * pow(0.5 * (1 - abc(2)), i + j - 1));
            }
...
double compute_eta(int vcjh_scheme, int order) {
  ...
  } else if (vcjh_scheme == 3)  // hu
  {
    ...
    std::cerr << "ERROR: Invalid VCJH scheme ... " << std::endl;
  }
...
// Method that searches a value in a sorted array without repeated entries and
...
int index_locate_int(int value, int *array, int size) {
  ...
  if (array[ju] <= array[0] && ju != 0) {
    std::cerr << "ERROR, array not sorted, exiting" << std::endl;
    std::cout << "size= " << size << std::endl;
    std::cout << "array[0] = " << array[0] << std::endl;
    std::cout << "array[size-1] = " << array[ju] << std::endl;
    exit(1);
  }
...
void eval_isentropic_vortex(array<double> &pos, double time, double &rho,
  ...
  rho = pow(1.0 - ev_eps_ic * ev_eps_ic * (gamma - 1.0) /
                      (8.0 * gamma * pi * pi) * exp(f),
            1.0 / (gamma - 1.0));
...
void eval_sine_wave_single(array<double> &pos, array<double> &wave_speed,
                           double diff_coeff, double time, double &rho,
                           array<double> &grad_rho, int n_dims) {
  array<double> relative_pos(n_dims);
  ...
  grad_rho(0) = pi * exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
                cos(pi * angle);
  grad_rho(1) = pi * exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
                cos(pi * angle);

  if (n_dims == 3)
    grad_rho(2) = pi * exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
                  cos(pi * angle);
}

void eval_sine_wave_group(array<double> &pos, array<double> &wave_speed,
                          double diff_coeff, double time, double &rho,
                          array<double> &grad_rho, int n_dims) {
  array<double> relative_pos(n_dims);
  relative_pos(0) = pos(0) - wave_speed(0) * time;
  relative_pos(1) = pos(1) - wave_speed(1) * time;
  if (n_dims == 3) relative_pos(2) = pos(2) - wave_speed(2) * time;

  if (n_dims == 2)
    rho = exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
          sin(pi * relative_pos(0)) * sin(pi * relative_pos(1));
  if (n_dims == 3)
    rho = exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
          sin(pi * relative_pos(0)) * sin(pi * relative_pos(1)) *
          sin(pi * relative_pos(2));

  if (n_dims == 2) {
    grad_rho(0) = pi * exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
                  cos(pi * relative_pos(0)) * sin(pi * relative_pos(1));
    grad_rho(1) = pi * exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
                  sin(pi * relative_pos(0)) * cos(pi * relative_pos(1));
  }
  if (n_dims == 3) {
    grad_rho(0) = pi * exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
                  cos(pi * relative_pos(0)) * sin(pi * relative_pos(1)) *
    ...
    grad_rho(2) = pi * exp(-((double)n_dims) * diff_coeff * pi * pi * time) *
                  sin(pi * relative_pos(0)) * sin(pi * relative_pos(1)) *
                  cos(pi * relative_pos(2));
  }
}

void eval_sphere_wave(array<double> &pos, array<double> &wave_speed,
                      double time, double &rho, int n_dims) {
  array<double> relative_pos(n_dims);
  relative_pos(0) = pos(0) - wave_speed(0) * time;
  relative_pos(1) = pos(1) - wave_speed(1) * time;
  relative_pos(2) = pos(2) - wave_speed(2) * time;
...
void eval_couette_flow(array<double> &pos, double in_gamma, double in_R_ref,
                       double in_u_wall, double in_T_wall, double in_p_bound,
                       double in_prandtl, double time, double &ene,
                       array<double> &grad_ene, int n_dims) {
  double x, y, z;
  ...
  ps = p_bound;
  Ts = T_wall + (y / h_channel) * ka +
       kb * (y / h_channel) * (1.0 - (y / h_channel));

  rho = ps / (R_ref * Ts);

  mom_x = rho * vx;
  mom_y = rho * vy;
  if (n_dims == 3) mom_z = rho * vz;
  ...
  rho_dx = 0.0;
  rho_dy = -(ps / R_ref) *
           ((ka / h_channel) - ((kb * y) / (h_channel * h_channel)) +
            (kb / h_channel) * (1.0 - (y / h_channel))) /
           pow(T_wall + ka * (y / h_channel) +
                   kb * (y / h_channel) * (1.0 - (y / h_channel)),
               2.0);
  if (n_dims == 3) rho_dz = 0.0;

  mom_x_dx = 0.0;
  mom_x_dy = rho_dy * vx + rho * (u_wall / h_channel);
  if (n_dims == 3) mom_x_dz = 0.0;

  mom_y_dx = 0.0;
  mom_y_dy = 0.0;
  if (n_dims == 3) mom_y_dz = 0.0;

  ene_dx = 0.0;
  ene_dy = 0.5 * rho_dy * (vx * vx) + mom_x * (u_wall / h_channel);
  if (n_dims == 3) ene_dz = 0.0;

  grad_ene(0) = ene_dx;
  grad_ene(1) = ene_dy;
  if (n_dims == 3) grad_ene(2) = ene_dz;
}

// Set initial momentum as up to 4th order polynomial in each direction
// TODO: allow mixed terms e.g. xy, yz^2
void eval_poly_ic(array<double> &pos, double rho, array<double> &ics,
                  int n_dims) {
  // HACK: do not use profile outside the vertical bounds of the inlet in
  // periodic hill case
  if (pos(1) < 1.0) {
    ics(1) = 0.0;
    ics(2) = 0.0;
    ...
    // Take N user-specified coefficients {a,b,c,...,n} to construct a
    // polynomial of the form u = a + bx + cx^2 + ... + nx^N (1D) In 2D and 3D,
    // add extra coeffs for mixed terms xy, xyz, x^2y etc.
    array<double> c(13);

    for (int i = 0; i < 13; ++i) c(i) = run_input.x_coeffs(i);
    ics(1) = c(0) + c(1) * pos(0) + c(2) * pow(pos(0), 2) +
             c(3) * pow(pos(0), 3) + c(4) * pow(pos(0), 4) + c(5) * pos(1) +
             c(6) * pow(pos(1), 2) + c(7) * pow(pos(1), 3) +
             c(8) * pow(pos(1), 4);

    if (n_dims == 3)
      ics(1) += c(9) * pos(2) + c(10) * pow(pos(2), 2) +
                c(11) * pow(pos(2), 3) + c(12) * pow(pos(2), 4);

    for (int i = 0; i < 13; ++i) c(i) = run_input.y_coeffs(i);
    ics(2) = c(0) + c(1) * pos(0) + c(2) * pow(pos(0), 2) +
             c(3) * pow(pos(0), 3) + c(4) * pow(pos(0), 4) + c(5) * pos(1) +
             c(6) * pow(pos(1), 2) + c(7) * pow(pos(1), 3) +
             c(8) * pow(pos(1), 4);

    if (n_dims == 3) {
      ics(2) += c(9) * pos(2) + c(10) * pow(pos(2), 2) +
                c(11) * pow(pos(2), 3) + c(12) * pow(pos(2), 4);

      for (int i = 0; i < 13; ++i) c(i) = run_input.z_coeffs(i);
      ics(3) = c(0) + c(1) * pos(0) + c(2) * pow(pos(0), 2) +
               c(3) * pow(pos(0), 3) + c(4) * pow(pos(0), 4) + c(5) * pos(1) +
               c(6) * pow(pos(1), 2) + c(7) * pow(pos(1), 3) +
               c(8) * pow(pos(1), 4) + c(9) * pos(2) + c(10) * pow(pos(2), 2) +
               c(11) * pow(pos(2), 3) + c(12) * pow(pos(2), 4);
    }
  }
}

/*! Functions used in evaluation of shape functions and its 1st and 2nd
derivatives BEGIN:*/
...
array<double> LagrangeP(int order, int node, array<double> &subs) {
  ...
  int constInSubs =
      subs.get_dim(1);  // location of the constant term in polynomial subs
  ...
  for (int i = 0; i < order + 1; i++) {
    if (i != node - 1) {
      term = subs;
      term(constInSubs - 1) -= xi(i);  // adds constant from subs to that in
      ...
      den = convol(den, tempConstant);  // update the denominator; this is a
  ...
  return convol(
      num,
      den);  // return the numerator times the reciprocal of the denominator
}
...
array<double> shapePoly4Tri(int in_index, int nNodesSide) {
  ...
  array<double> T_I;  // special lagrange polynomial corresponding to a node in
                      // the triangle
  // this is the result
  ...
  if (in_index == 1) {
    ...
    double r_I =
        xi(in_index - 1);  // get location of node in_index in the range
...
array<double> addPoly(array<double> &p1, array<double> &p2) {
  ...
  // std::cout<<"length p1: "<<lengthp1<<"   height p1: "<<heightp1;
  // std::cout<<"   depth p1: "<<depthp1<<std::endl;
  int lengthp2 = p2.get_dim(1);
  int heightp2 = p2.get_dim(0);
  int depthp2 = p2.get_dim(2);
  // std::cout<<"length p2: "<<lengthp2<<"   height p2: "<<heightp2;
  // std::cout<<"   depth p2: "<<depthp2<<std::endl;

  int lengthp3 = std::max(lengthp1, lengthp2);
  int heightp3 = std::max(heightp1, heightp2);
  ...
  // std::cout<<std::endl<<"Depth of p3: "<<p3.get_dim(2)<<std::endl;
  // p2.print();
  return p3;
}
...
array<T> multPoly(array<T> &p1, array<T> &p2) {
  ...
  if (iszero(p1) || iszero(p2)) {
    ...
    int lengthp1 = p1.get_dim(1);
    int heightp1 = p1.get_dim(0);
    // std::cout<<"length p1: "<<lengthp1<<"   height p1:
    // "<<heightp1<<std::endl;
    int lengthp2 = p2.get_dim(1);
    int heightp2 = p2.get_dim(0);
    // std::cout<<"length p2: "<<lengthp2<<"   height p2:
    // "<<heightp2<<std::endl;

    // Calculate dimensions of p3: needs to accomodate sizes of p1 and p2
    int lengthp3 =
        std::max(lengthp1, lengthp2);    // return largest of the two values
...
bool iszero(array<T> &poly) {
  ...
  for (int i = 0; i < totalElements; i++) {
    check =
        (*poly.get_ptr_cpu(i) * (*poly.get_ptr_cpu(i)) < 1e-12);  // is it zero?
...
array<double> nodeFunctionTri(int in_index, int in_n_spts,
                              array<int> &index_location_array) {
  ...
  // Multiply polynomials (order of multiplication does matter in this case, as
  // the differentiation with respect to t --third row-- is different to that
  // with respec to r or s)
...
array<int> linkTriangleNodes(int in_n_spts) {
  // first row in index_location_array contains indeces of r arranged in
  // ascending global node number; second row contains indeces of s arranged in
  // ascending global node number; third row contains indeces of t arranged in
  // ascending global node number; refer to Hughes pp 169 to see link between
  // r/s indeces ordering and global indeces ordering

  // Calculate number of nodes on each side
  int nNodesSide;
  nNodesSide = calcNumSides(in_n_spts);
  array<int> index_location_array;  // Global node-specific shape function to be
                                    // returned
  ...
  array<int> rind(
      1,
      in_n_spts);  // stores r indeces; location in array is global node number
  array<int> sind(
      1,
      in_n_spts);  // stores s indeces; location in array is global node number
  array<int> tind(
      1,
      in_n_spts);   // stores t indeces; location in array is global node number
  ...
  int nSide;  // counts number of nodes in side of next cycle; recalculated at
              // every cycle
...
array<double> diffPoly(array<double> &p, array<int> &term2Diff) {
  /*
    Returns 3D array; differentiates polynomial p with respect to dimensions
    specified by term2Diff term2Diff: n x 1 array of rows to differentiate
    (negative integer i differentiates row abs(i) and multiplies that row by -1
    ); rows enumerated starting at 1
    */

  int numTerms = term2Diff.get_dim(
      1);  // find number of variables with respect to which to differentiate
  // std::cout<<"numTerms = "<<numTerms<<std::endl;
  int depthp = p.get_dim(2);  // find number of layers of polynomial p (remember
                              // layers represent addition; rows multiplication;
                              // columns coefficients of polynomials)
  ...
  for (int l = 0; l < numTerms; l++) {
    ...
    // Switch used in triangles; when row is negative, row number abs(row) is
    // differentiated and multiplied by -1
    ...
    for (int k = 0; k < depthp; k++) {
      ...
      diffp(row, 0, k) =
          0;  // coefficients shift due to change in power of term

      for (int j = 1; j < lengthrow; j++) {
        // Grab column in row to manipulate
        int power = lengthrow -
                    j;  // power corresponding to coefficient being manipulated
...
double evalPoly(array<double> p, array<double> coords) {
  ...
  if (p.get_dim(0) != coords.get_dim(1)) {
    FatalError(
        "@evalPoly: number of coordinates at which to evaluate polynomial is "
        "not equal to number of polynomial variables");
  }
  ...
  for (int k = 0; k < depthp; k++) {
    ...
    for (int i = 0; i < heightp; i++) {
      ...
      val *= rowval;
      // std::cout<<"k = "<<k<<" ;  rowval = "<<rowval<<" ;  val = "<<val<<" ;
...
void eval_dn_nodal_s_basis(array<double> &dd_nodal_s_basis,
  ...
  /*
    Function that returns the values of the nth derivatives of the shape
    function of nodes (rows in dd_nodal_s_basis) with respect to r and s (dr^n,
    ds^2, drds are in 1st, 2nd, 3rd columns respectively) given: in_loc: 2x1
    array of coordinates where derivatives are evaluated in_n_spts: number of
    nodes used to create triangle function ( n*(n+1)/2 where n is nodes on each
  ...
  if (dd_nodal_s_basis.get_dim(0) != in_n_spts) {
    FatalError(
        "@eval_dd_nodal_s_basis_new: number of nodes must be equal to number "
        "of rows in dd_nodal_s_basis");
  }

  if (dd_nodal_s_basis.get_dim(1) != 0.5 * n_deriv * (n_deriv + 1)) {
    if ((n_deriv == 1 && dd_nodal_s_basis.get_dim(1) == 2) ||
        (n_deriv == 0 && dd_nodal_s_basis.get_dim(1) == 1)) {
    } else {
      FatalError(
          "@eval_dd_nodal_s_basis_new: not enough columns in dd_nodal_s_basis "
          "to store all derivatives");
    }
  }

  // Obtain linked list of node enumeration for triangle given the number of
  // nodes in_n_spts
  array<int> nodeList;
  ...
  // Array that contains variables with respect to which shape function is
  // differentiated
  array<int> diff_coord(1, 2);
  // array that contains values of r,s,t to be plugged in when evaluating
  // differentiated polynomial
  array<double> coords(1, 3);
  coords(0) = in_loc(0);
  coords(1) = in_loc(1);
  coords(2) = -1 - in_loc(0) -
              in_loc(1);  // value of t given values of r, s : t = -1 -r -s
  diff_coord(1) = -3;     // differentiate 3rd row always and multiply by -1
                          // because
  // t = -1 - s - r so
  ...
  int num_deriv_iterations =
      n_deriv * (n_deriv + 1) / 2 + (n_deriv <= 1 ? 1 : 0);
  ...
  for (int a = 0; a < in_n_spts; a++)  // in_n_spts
  {
    ...
    for (int i = 0; i < num_deriv_iterations; i++) {
      ...
      for (int dr_count = 0; dr_count < n_deriv - i; dr_count++) {
        // Differentiate shape function with respect to one variable (r is 1, s
        // is 2)
      ...
      // Evaluate differentiated shape function and store in array
      // d_nodal_s_basis

      dd_nodal_s_basis(
          a, (i == num_deriv_iterations - 1 ? (i == 0 ? 0 : 1)
                                            : (i == 0 ? i : (i + 1)))) =
          evalPoly(diff_N_a, coords);
    }
  }
}
...
void gaussj(int n, array<double> &A, array<double> &b) {
  ...
  for (i = 0; i < n; i++) {
    ...
    indxc(i) = icol;
    if (A(icol, icol) == 0.0) {
      std::cerr << "Error: Singular matrix in gaussj" << std::endl;
      exit(1);
    }
...
double flt_res(int N, array<double> &wf, array<double> &B, double k_0,
               double k_c, int ctype) {
  int i;
  double norm, xm;
  array<double> flt(N);
  double flt_res = 0.0;
  ...
  for (i = 0; i < N; i++) flt(i) /= norm;

  if (ctype == 0)  // Constrain 2nd moment
  {
    xm = 0.0;
    norm = 0.0;
    for (i = 0; i < N; i++) xm += flt(i) * B(i);
    for (i = 0; i < N; i++) norm += flt(i) * pow((B(i) - xm), 2);
    flt_res = sqrt(12.0 * norm) - 1.0 / k_0;
  } else  // Constrain cutoff frequency
  {
    flt_res = -exp((pow((-1.0 * pi), 2)) / 24.0);
    for (i = 0; i < N; i++) flt_res += flt(i) * cos(B(i) * k_0 * pi);
  }
  // std::cout<<"flt_res: "<<flt_res<<std::endl;
  return flt_res;
}
...
array<double> add_arrays(array<double> &M1, array<double> &M2) {
  ...
  if (dim_1_0 == dim_2_0 and dim_1_1 == dim_2_1 and dim_1_2 == dim_2_2 and
      dim_1_3 == dim_2_3) {
...
array<double> mult_arrays(array<double> &M1, array<double> &M2) {
  ...
  if (dim_1_2 == 1 and dim_1_3 == 1 and dim_2_2 == 1 and dim_2_3 == 1) {
    ...
    if (dim_1_1 == dim_2_0) {
      ...
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, dim_1_0, dim_2_1,
                  dim_1_1, 1.0, M1.get_ptr_cpu(), dim_1_0, M2.get_ptr_cpu(),
      ...
      std::cerr
          << "ERROR: array dimensions are not compatible in multiplication "
             "function"
          << std::endl;
      exit(1);
    }
  } else {
    std::cerr << "ERROR: Array multiplication function can only multiply "
                 "2-dimensional arrays together"
              << std::endl;
    exit(1);
  }
}
...
array<double> transpose_array(array<double> &in_array) {
  ...
  if (dim_2 == 1 and dim_3 == 1 and dim_0 == dim_1) {
    ...
    std::cerr << "ERROR: Array transpose function only accepts a 2-dimensional "
                 "square array"
              << std::endl;
    exit(1);
  }
...
array<double> inv_array(array<double> &in_array) {
  ...
  if (dim_2 == 1 and dim_3 == 1) {
    if (dim_0 == dim_1) {
      ...
      // setup input array
      for (i = 0; i < dim_0; i++)
        for (j = 0; j < dim_0; j++) input(i, j) = in_array(i, j);
      ...
      std::cerr << "ERROR: Can only obtain inverse of a square array"
                << std::endl;
      exit(1);
    }
  } else {
    std::cerr << "ERROR: Array you are trying to invert has > 2 dimensions"
              << std::endl;
    exit(1);
  }
}
