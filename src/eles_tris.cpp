...
#include "../data/loc_tri_alpha_pts.dat"
    ...
    std::cout << "ERROR: Unknown solution point location type.... "
              << std::endl;
  }
  // loc_upts.print();
}
...
#include "../data/loc_1d_gauss_lobatto_pts.dat"
    ...
    std::cout << "ERROR: Unknown edge flux point location type.... "
              << std::endl;
  }
...
double eles_tris::compute_inter_detjac_inters_cubpts(int in_inter,
                                                     array<double> d_pos) {
  double output = 0.;
  double xr, xs, yr, ys;
...
void eles_tris::set_vandermonde(void) {
  ...
  for (int i = 0; i < n_upts_per_ele; i++)
    for (int j = 0; j < n_upts_per_ele; j++)
      vandermonde(i, j) =
          eval_dubiner_basis_2d(loc_upts(0, i), loc_upts(1, i), j, order);
...
void eles_tris::set_vandermonde_restart() {
  ...
  for (int i = 0; i < n_upts_per_ele_rest; i++)
    for (int j = 0; j < n_upts_per_ele_rest; j++)
      vandermonde_rest(i, j) = eval_dubiner_basis_2d(
          loc_upts_rest(0, i), loc_upts_rest(1, i), j, order_rest);
...
void eles_tris::write_restart_info(std::ofstream &restart_file) {
  ...
  restart_file << "Number of solution points per triangular element"
               << std::endl;
  restart_file << n_upts_per_ele << std::endl;

  restart_file << "Location of solution points in triangular elements"
               << std::endl;
  for (int i = 0; i < n_upts_per_ele; i++) {
    for (int j = 0; j < n_dims; j++) {
...
double eles_tris::eval_d_nodal_basis(int in_index, int in_cpnt,
                                     array<double> in_loc) {
...
double eles_tris::eval_nodal_s_basis(int in_index, array<double> in_loc,
                                     int in_n_spts) {
...
// double eles_tris::eval_d_nodal_s_basis(int in_index, int in_cpnt,
// array<double> in_loc, int in_n_spts)
void eles_tris::eval_d_nodal_s_basis(array<double> &d_nodal_s_basis,
                                     array<double> in_loc, int in_n_spts) {
...
void eles_tris::fill_opp_3(array<double> &opp_3) {
  get_opp_3_tri(opp_3, loc_upts, loc_1d_fpts, vandermonde, inv_vandermonde,
                n_upts_per_ele, order, run_input.c_tri,
...
void eles_tris::compute_filter_upts(void) {
  ...
  {
    //#if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined
    //_STANDARD_BLAS
    ...
    if (N != n_cubpts_per_ele) {
      FatalError(
          "WARNING: Gaussian filter only currently possible for order 1, 2 or "
          "4. If order 1, set vol_cub_order to 2. If order 2, set "
          "vol_cub_order to 3 or 4. If order 4, set vol_cub_order to 7. "
          "Exiting");
    }
    ...
    for (i = 0; i < N; i++)
      for (j = i; j < N; j++)
        beta(i, j) =
            sqrt(pow(X(0, i) - X(0, j), 2) + pow(X(1, i) - X(1, j), 2)) / dlt;
    for (i = 0; i < N; i++)
      for (j = 0; j < i; j++) beta(i, j) = beta(j, i);

    for (j = 0; j < N; ++j) wf(j) = weight_volume_cubpts(j);
  ...
  } else if (run_input.filter_type == 2)  // Modal coefficient filter
  {
    ...
    compute_modal_filter_tri(filter_upts, vandermonde, inv_vandermonde, N,
                             order);
  } else  // Simple average for low order
  {
    if (rank == 0) std::cout << "Building average filter" << std::endl;
    for (i = 0; i < N; i++)
      for (j = 0; j < N; j++) filter_upts(i, j) = 1.0 / N;
  }
...
double eles_tris::calc_h_ref_specific(int in_ele) {
  ...
  a = sqrt(pow(shape(0, 0, in_ele) - shape(0, 1, in_ele), 2.0) +
           pow(shape(1, 0, in_ele) - shape(1, 1, in_ele), 2.0));
  b = sqrt(pow(shape(0, 1, in_ele) - shape(0, 2, in_ele), 2.0) +
           pow(shape(1, 1, in_ele) - shape(1, 2, in_ele), 2.0));
  c = sqrt(pow(shape(0, 2, in_ele) - shape(0, 0, in_ele), 2.0) +
