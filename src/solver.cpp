...
void CalcResidual(int in_file_num, int in_rk_stage, struct solution *FlowSol) {
  ...
  /*! If at first RK step and using certain LES models, compute some
   * model-related quantities. */
  if (run_input.LES == 1 && in_disu_upts_from == 0) {
    if (run_input.SGS_model == 2 || run_input.SGS_model == 3 ||
        run_input.SGS_model == 4) {
      for (i = 0; i < FlowSol->n_ele_types; i++)
        FlowSol->mesh_eles(i)->calc_sgs_terms(in_disu_upts_from);
    }
  }

  /*! Shock capturing part - only concentration method on GPU and on quads for
   * now */
  /*! TO be added: Persson's method for triangles with artificial viscosity
   * structure */
  ...
  if (run_input.ArtifOn) {
    ...
    if (run_input.artif_type == 1) {
      /*! This routine does shock detection. For concentration method filter is
       * also applied in this routine itself */
      for (i = 0; i < FlowSol->n_ele_types; i++)
  ...
  /*! Compute the inviscid flux at the solution points and store in total flux
   * storage. */
  for (i = 0; i < FlowSol->n_ele_types; i++)
  ...
  if (run_input.forcing == 1 and in_rk_stage == 0 and
      run_input.equation == 0 and FlowSol->n_dims == 3) {
#ifdef _GPU
  ...
  for (i = 0; i < FlowSol->n_bdy_inter_types; i++)
    FlowSol->mesh_bdy_inters(i).evaluate_boundaryConditions_invFlux(
        FlowSol->time);
  ...
  if (FlowSol->viscous) {
    /*! Compute corrected gradient of the solution at the solution and flux
     * points. */
    for (i = 0; i < FlowSol->n_ele_types; i++)
    ...
    /*! Compute discontinuous viscous flux at upts and add to inviscid flux at
     * upts. */
    for (i = 0; i < FlowSol->n_ele_types; i++)
  ...
  /*! For viscous or inviscid, compute the normal discontinuous flux at flux
   * points. */
  for (i = 0; i < FlowSol->n_ele_types; i++)
    FlowSol->mesh_eles(i)->extrapolate_totalFlux();

  /*! For viscous or inviscid, compute the divergence of flux at solution
   * points. */
  for (i = 0; i < FlowSol->n_ele_types; i++)
  ...
  if (FlowSol->viscous) {
    ...
    for (i = 0; i < FlowSol->n_bdy_inter_types; i++)
      FlowSol->mesh_bdy_inters(i).evaluate_boundaryConditions_viscFlux(
          FlowSol->time);
...
double *get_disu_fpts_ptr(int in_ele_type, int in_ele, int in_field,
                          int in_local_inter, int in_fpt,
                          struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_disu_fpts_ptr(in_fpt, in_local_inter, in_field, in_ele);
}

// get pointer to normal continuous transformed inviscid flux at a flux point

double *get_norm_tconf_fpts_ptr(int in_ele_type, int in_ele, int in_field,
                                int in_local_inter, int in_fpt,
                                struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_norm_tconf_fpts_ptr(in_fpt, in_local_inter, in_field, in_ele);
}

// get pointer to subgrid-scale flux at a flux point

double *get_sgsf_fpts_ptr(int in_ele_type, int in_ele, int in_local_inter,
                          int in_field, int in_dim, int in_fpt,
                          struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_sgsf_fpts_ptr(in_fpt, in_local_inter, in_field, in_dim, in_ele);
}

// get pointer to determinant of jacobian at a flux point

double *get_detjac_fpts_ptr(int in_ele_type, int in_ele, int in_ele_local_inter,
                            int in_inter_local_fpt, struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_detjac_fpts_ptr(in_inter_local_fpt, in_ele_local_inter, in_ele);
}

// get pointer to determinant of jacobian at a flux point (dynamic->static)

double *get_detjac_dyn_fpts_ptr(int in_ele_type, int in_ele,
                                int in_ele_local_inter, int in_inter_local_fpt,
                                struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_detjac_dyn_fpts_ptr(in_inter_local_fpt, in_ele_local_inter, in_ele);
}

// get pointer to magntiude of normal dot inverse of (determinant of jacobian
// multiplied by jacobian) at a flux point

double *get_tdA_fpts_ptr(int in_ele_type, int in_ele, int in_ele_local_inter,
                         int in_inter_local_fpt, struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_tdA_fpts_ptr(in_inter_local_fpt, in_ele_local_inter, in_ele);
}

// get pointer to the equivalent of 'dA' (face area) at a flux point in dynamic
// physical space
double *get_ndA_dyn_fpts_ptr(int in_ele_type, int in_ele,
                             int in_ele_local_inter, int in_inter_local_fpt,
                             struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_ndA_dyn_fpts_ptr(in_inter_local_fpt, in_ele_local_inter, in_ele);
}

// get pointer to the normal at a flux point

double *get_norm_fpts_ptr(int in_ele_type, int in_ele, int in_local_inter,
                          int in_fpt, int in_dim, struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_norm_fpts_ptr(in_fpt, in_local_inter, in_dim, in_ele);
}

// get pointer to the normal at a flux point in the dynamic space

double *get_norm_dyn_fpts_ptr(int in_ele_type, int in_ele, int in_local_inter,
                              int in_fpt, int in_dim,
                              struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_norm_dyn_fpts_ptr(in_fpt, in_local_inter, in_dim, in_ele);
}

// get CPU pointer to the coordinates at a flux point.
// See bdy_inters for reasons for this CPU/GPU split.

double *get_loc_fpts_ptr_cpu(int in_ele_type, int in_ele, int in_local_inter,
                             int in_fpt, int in_dim, struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_loc_fpts_ptr_cpu(in_fpt, in_local_inter, in_dim, in_ele);
}

// get GPU pointer to the coordinates at a flux point

double *get_loc_fpts_ptr_gpu(int in_ele_type, int in_ele, int in_local_inter,
                             int in_fpt, int in_dim, struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_loc_fpts_ptr_gpu(in_fpt, in_local_inter, in_dim, in_ele);
}

// get CPU pointer to the physical dynamic coordinates at a flux point.

double *get_pos_dyn_fpts_ptr_cpu(int in_ele_type, int in_ele,
                                 int in_local_inter, int in_fpt, int in_dim,
                                 struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_pos_dyn_fpts_ptr_cpu(in_fpt, in_local_inter, in_dim, in_ele);
}

// get pointer to normal continuous transformed viscous flux at a flux point

// double* get_norm_tconvisf_fpts_ptr(int in_ele_type, int in_ele, int in_field,
// int in_local_inter, int in_fpt)
//{
//	return
//mesh_eles(in_ele_type)->get_norm_tconvisf_fpts_ptr(in_fpt,in_local_inter,in_field,in_ele);
//}

// get pointer to delta of the transformed discontinuous solution at a flux
// point

double *get_delta_disu_fpts_ptr(int in_ele_type, int in_ele, int in_field,
                                int in_local_inter, int in_fpt,
                                struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_delta_disu_fpts_ptr(in_fpt, in_local_inter, in_field, in_ele);
}

// get pointer to gradient of the discontinuous solution at a flux point
double *get_grad_disu_fpts_ptr(int in_ele_type, int in_ele, int in_local_inter,
                               int in_field, int in_dim, int in_fpt,
                               struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_grad_disu_fpts_ptr(in_fpt, in_local_inter, in_dim, in_field,
                               in_ele);
}

// get pointer to the discontinuous solution (close normal) at a flux point
double *get_normal_disu_fpts_ptr(int in_ele_type, int in_ele,
                                 int in_local_inter, int in_field, int in_fpt,
                                 struct solution *FlowSol,
                                 array<double> temp_loc, double temp_pos[3]) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_normal_disu_fpts_ptr(in_fpt, in_local_inter, in_field, in_ele,
                                 temp_loc, temp_pos);
}

// get pointer to the grid velocity at a flux point
double *get_grid_vel_fpts_ptr(int in_ele_type, int in_ele, int in_local_inter,
                              int in_fpt, int in_dim,
                              struct solution *FlowSol) {
  return FlowSol->mesh_eles(in_ele_type)
      ->get_grid_vel_fpts_ptr(in_ele, in_local_inter, in_fpt, in_dim);
}
  ...
  for (int i = 0; i < FlowSol->n_ele_types; i++) {
    if (FlowSol->mesh_eles(i)->get_n_eles() != 0) {
      for (int j = 0; j < in_n_files; j++) {
        ...
        if (!restart_file) FatalError("Could not open restart file ");
