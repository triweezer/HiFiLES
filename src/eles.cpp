...
void eles::setup(int in_n_eles, int in_max_n_spts_per_ele) {
  ...
  if (n_eles != 0) {
    ...
    filter = 0;
    if (LES)
      if (sgs_model == 3 || sgs_model == 2 || sgs_model == 4) filter = 1;
    ...
    if (run_input.adv_type == 0) {
      ...
      std::cerr << "ERROR: Type of time integration scheme not recongized ... "
                << std::endl;
    }
    ...
    if (run_input.dt_type == 1) dt_local.setup(1);
    ...
    // Initialize to zero
    for (int m = 0; m < n_adv_levels; m++) disu_upts(m).initialize_to_zero();
    ...
    // Allocate array for wall distance if using a RANS-based turbulence model
    // or LES wall model
    ...
    if (LES != 0 || wall_model != 0) {
      ...
      if (motion) temp_sgsf_ref.setup(n_fields, n_dims);
    }
    ...
    d_nodal_s_basis_upts.setup(n_dims, in_max_n_spts_per_ele, n_upts_per_ele,
                               n_eles);
    d_nodal_s_basis_fpts.setup(n_dims, in_max_n_spts_per_ele, n_fpts_per_ele,
                               n_eles);

    nodal_s_basis_vol_cubpts.setup(in_max_n_spts_per_ele, n_cubpts_per_ele,
                                   n_eles);
    d_nodal_s_basis_vol_cubpts.setup(n_dims, in_max_n_spts_per_ele,
                                     n_cubpts_per_ele, n_eles);
    ...
    for (int iface = 0; iface < n_inters_per_ele; iface++) {
      nodal_s_basis_inters_cubpts(iface).setup(
          in_max_n_spts_per_ele, n_cubpts_per_inter(iface), n_eles);
      d_nodal_s_basis_inters_cubpts(iface).setup(
          n_dims, in_max_n_spts_per_ele, n_cubpts_per_inter(iface), n_eles);
    }
    ...
    if (run_input.ArtifOn) {
      if (run_input.artif_type == 1) sensor.setup(n_eles);
...
array<int> eles::get_connectivity_plot() { return connectivity_plot; }
    ...
    for (j = 0; j < n_upts_per_ele; j++) {
      ...
      if (run_input.ic_form == 0) {
        ...
        if (n_dims == 2) {
          ...
          std::cerr << "ERROR: Invalid number of dimensions ... " << std::endl;
        }
      ...
      {
        eval_sine_wave_single(pos, run_input.wave_speed, run_input.diff_coeff,
                              time, rho, grad_rho, n_dims);
        ics(0) = rho;
      } else if (run_input.ic_form == 3)  // Sine wave (group)
      {
        eval_sine_wave_group(pos, run_input.wave_speed, run_input.diff_coeff,
                             time, rho, grad_rho, n_dims);
        ics(0) = rho;
      ...
      {
        ics(0) = 1.0;
      } else if (run_input.ic_form ==
                 6)  // Up to 4th order polynomials for u, v, w
      {
        rho = run_input.rho_c_ic;
        p = run_input.p_c_ic;
        ...
        if (n_dims == 2)
          ics(3) = p / (gamma - 1.0) +
                   0.5 * rho * (ics(1) * ics(1) + ics(2) * ics(2));
        else if (n_dims == 3)
          ics(4) =
              p / (gamma - 1.0) +
              0.5 * rho * (ics(1) * ics(1) + ics(2) * ics(2) + ics(3) * ics(3));
      } else if (run_input.ic_form ==
                 7)  // Taylor-Green Vortex initial conditions
      {
        rho = run_input.rho_c_ic;
        ics(0) = rho;
        if (n_dims == 2) {
          // Simple 2D div-free vortex
          p = 100 + rho / 16.0 * (cos(2.0 * pos(0)) + cos(2.0 * pos(1))) *
                        (cos(2.0 * pos(2)) + 2.0);
        ...
        std::cerr << "ERROR: Invalid form of initial condition ... (File: "
      ...
      if (run_input.perturb_ic == 1 and n_dims == 3) {
        ...
        L_y = pi;
        L_z = 2.;
        ics(3) += alpha * exp(-pow((pos(0) - L_x / 2.) / L_x, 2)) *
                  exp(-pow(pos(1) / L_y, 2)) * cos(4. * pi * pos(2) / L_z);
      }
  ...
  for (int i = 0; i < num_eles_to_read; i++) {
    ...
    if (index != -1)  // Ele belongs to processor
    {
      for (int j = 0; j < n_upts_per_ele_rest; j++)
        for (int k = 0; k < n_fields; k++) restart_file >> disu_upts_rest(j, k);
    ...
    } else  // Skip the data (doesn't belong to current processor)
    {
      ...
      getline(restart_file, str);
      for (int j = 0; j < n_upts_per_ele_rest; j++) getline(restart_file, str);
    }
  }
...
void eles::write_restart_data(std::ofstream &restart_file) {
  ...
  for (int i = 0; i < n_eles; i++) restart_file << ele2global_ele(i) << " ";
    ...
    if (adv_type == 0) {
      ...
      if (run_input.dt_type == 1) {
        ...
        for (int ic = 0; ic < n_eles; ic++) {
          ...
          if (dt_local_new < dt_local(0)) dt_local(0) = dt_local_new;
        }
...
#  endif
      ...
      if (run_input.dt_type == 2) {
        for (int ic = 0; ic < n_eles; ic++) dt_local(ic) = calc_dt_local(ic);
      }
        ...
        for (int ic = 0; ic < n_eles; ic++) {
          for (int inp = 0; inp < n_upts_per_ele; inp++) {
            ...
            if (run_input.dt_type != 0) {
              ...
              if (run_input.dt_type == 1) run_input.dt = dt_local(0);
            ...
            disu_upts(0)(inp, ic, i) -=
                run_input.dt *
                (div_tconf_upts(0)(inp, ic, i) / detjac_upts(inp, ic) -
                 run_input.const_src - src_upts(inp, ic, i));
          }
        }
      }
...
#ifdef _GPU
      RK11_update_kernel_wrapper(
          n_upts_per_ele, n_dims, n_fields, n_eles, disu_upts(0).get_ptr_gpu(),
          div_tconf_upts(0).get_ptr_gpu(), detjac_upts.get_ptr_gpu(),
          src_upts.get_ptr_gpu(), h_ref.get_ptr_gpu(), run_input.dt,
          run_input.const_src, run_input.CFL, run_input.gamma, run_input.mu_inf,
...
#  endif
      ...
      for (int ic = 0; ic < n_eles; ic++) {
        for (int i = 0; i < n_fields; i++) {
          for (int inp = 0; inp < n_upts_per_ele; inp++) {
            rhs = -div_tconf_upts(0)(inp, ic, i) / detjac_upts(inp, ic) +
                  run_input.const_src + src_upts(inp, ic, i);
      ...
      RK45_update_kernel_wrapper(
          n_upts_per_ele, n_dims, n_fields, n_eles, disu_upts(0).get_ptr_gpu(),
          disu_upts(1).get_ptr_gpu(), div_tconf_upts(0).get_ptr_gpu(),
          detjac_upts.get_ptr_gpu(), src_upts.get_ptr_gpu(),
          h_ref.get_ptr_gpu(), rk4a, rk4b, run_input.dt, run_input.const_src,
          run_input.CFL, run_input.gamma, run_input.mu_inf, run_input.order,
    ...
    /*! Time integration not implemented. */
      ...
      std::cerr << "ERROR: Time integration type not recognised ... "
                << std::endl;
    }
  }
}
...
double eles::calc_dt_local(int in_ele) {
  ...
  if (n_dims == 2) {
    ...
    for (int i = 0; i < n_upts_per_ele; i++) {
      ...
      p = (run_input.gamma - 1.0) *
          (disu_upts(0)(i, in_ele, 3) -
           0.5 * disu_upts(0)(i, in_ele, 0) * (u * u + v * v));
      ...
      if (lam_inv < lam_inv_new) lam_inv = lam_inv_new;

      if (lam_visc < lam_visc_new) lam_visc = lam_visc_new;
    }

    if (viscous) {
      dt_visc = (run_input.CFL * 0.25 * h_ref(0, in_ele) * h_ref(0, in_ele)) /
                (lam_visc)*1.0 / (2.0 * run_input.order + 1.0);
      dt_inv = run_input.CFL * h_ref(0, in_ele) / lam_inv * 1.0 /
               (2.0 * run_input.order + 1.0);
    } else {
      dt_visc = 1e16;
      dt_inv = run_input.CFL * h_ref(0, in_ele) / lam_inv * 1.0 /
               (2.0 * run_input.order + 1.0);
    }
    out_dt_local = std::min(dt_visc, dt_inv);
  }
...
void eles::extrapolate_solution(int in_disu_upts_from) {
  if (n_eles != 0) {
    /*!
     ...
     (vector of solution values at flux points) = opp_0 * (vector of solution
     values at nodes)
     */
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Arows, Bcols,
                  Acols, 1.0, opp_0.get_ptr_cpu(), Astride,
                  disu_upts(in_disu_upts_from).get_ptr_cpu(), Bstride, 0.0,
                  disu_fpts.get_ptr_cpu(), Cstride);

#  elif defined _NO_BLAS
      dgemm(Arows, Bcols, Acols, 1.0, 0.0, opp_0.get_ptr_cpu(),
            disu_upts(in_disu_upts_from).get_ptr_cpu(),
            disu_fpts.get_ptr_cpu());

#  endif
    } else if (opp_0_sparse == 1)  // mkl blas four-array csr format
    {
#  if defined _MKL_BLAS
      mkl_dcsrmm(&transa, &n_fpts_per_ele, &n_fields_mul_n_eles,
                 &n_upts_per_ele, &one, matdescra, opp_0_data.get_ptr_cpu(),
                 opp_0_cols.get_ptr_cpu(), opp_0_b.get_ptr_cpu(),
                 opp_0_e.get_ptr_cpu(),
                 disu_upts(in_disu_upts_from).get_ptr_cpu(), &n_upts_per_ele,
                 &zero, disu_fpts.get_ptr_cpu(), &n_fpts_per_ele);

#  endif
    } else {
      std::cerr << "ERROR: Unknown storage for opp_0 ... " << std::endl;
    }
...
#ifdef _GPU
    if (opp_0_sparse == 0) {
      cublasDgemm('N', 'N', Arows, Bcols, Acols, 1.0, opp_0.get_ptr_gpu(),
                  Astride, disu_upts(in_disu_upts_from).get_ptr_gpu(), Bstride,
                  0.0, disu_fpts.get_ptr_gpu(), Cstride);
    } else if (opp_0_sparse == 1) {
      bespoke_SPMV(n_fpts_per_ele, n_upts_per_ele, n_fields, n_eles,
                   opp_0_ell_data.get_ptr_gpu(),
                   opp_0_ell_indices.get_ptr_gpu(), opp_0_nnz_per_row,
                   disu_upts(in_disu_upts_from).get_ptr_gpu(),
                   disu_fpts.get_ptr_gpu(), ele_type, order, 0);
    } else {
      std::cerr << "ERROR: Unknown storage for opp_0 ... " << std::endl;
    }
#endif
        ...
        for (k = 0; k < n_fields; k++) {
          for (l = 0; l < n_dims; l++) {
            ...
            for (m = 0; m < n_dims; m++) {
              tdisf_upts(j, i, k, l) +=
                  JGinv_upts(l, m, j, i) *
                  temp_f(k, m);  // JGinv_upts(j,i,l,m)*temp_f(k,m);
            }
          }
        }
      }
...
#ifdef _GPU
    evaluate_invFlux_gpu_kernel_wrapper(
        n_upts_per_ele, n_dims, n_fields, n_eles,
        disu_upts(in_disu_upts_from).get_ptr_gpu(), tdisf_upts.get_ptr_gpu(),
        detjac_upts.get_ptr_gpu(), J_dyn_upts.get_ptr_gpu(),
        JGinv_upts.get_ptr_gpu(), JGinv_dyn_upts.get_ptr_gpu(),
        grid_vel_upts.get_ptr_gpu(), run_input.gamma, motion,
        run_input.equation, run_input.wave_speed(0), run_input.wave_speed(1),
        run_input.wave_speed(2), run_input.turb_model);
#endif
  }
}
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      ...
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_fpts_per_ele,
                  n_fields * n_eles, n_upts_per_ele, 1.0,
                  opp_1(0).get_ptr_cpu(), n_fpts_per_ele,
                  tdisf_upts.get_ptr_cpu(0, 0, 0, 0), n_upts_per_ele, 0.0,
                  norm_tdisf_fpts.get_ptr_cpu(), n_fpts_per_ele);
      for (int i = 1; i < n_dims; i++) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_fpts_per_ele,
                    n_fields * n_eles, n_upts_per_ele, 1.0,
                    opp_1(i).get_ptr_cpu(), n_fpts_per_ele,
                    tdisf_upts.get_ptr_cpu(0, 0, 0, i), n_upts_per_ele, 1.0,
                    norm_tdisf_fpts.get_ptr_cpu(), n_fpts_per_ele);
      }

#  elif defined _NO_BLAS
      dgemm(n_fpts_per_ele, n_fields * n_eles, n_upts_per_ele, 1.0, 0.0,
            opp_1(0).get_ptr_cpu(), tdisf_upts.get_ptr_cpu(0, 0, 0, 0),
            norm_tdisf_fpts.get_ptr_cpu());
      for (int i = 1; i < n_dims; i++) {
        dgemm(n_fpts_per_ele, n_fields * n_eles, n_upts_per_ele, 1.0, 1.0,
              opp_1(i).get_ptr_cpu(), tdisf_upts.get_ptr_cpu(0, 0, 0, i),
              norm_tdisf_fpts.get_ptr_cpu());
      }
#  endif
...
#  if defined _MKL_BLAS
      ...
      mkl_dcsrmm(&transa, &n_fpts_per_ele, &n_fields_mul_n_eles,
                 &n_upts_per_ele, &one, matdescra, opp_1_data(0).get_ptr_cpu(),
                 opp_1_cols(0).get_ptr_cpu(), opp_1_b(0).get_ptr_cpu(),
                 opp_1_e(0).get_ptr_cpu(), tdisf_upts.get_ptr_cpu(0, 0, 0, 0),
                 &n_upts_per_ele, &zero, norm_tdisf_fpts.get_ptr_cpu,
                 &n_fpts_per_ele);

      for (int i = 1; i < n_dims; i++) {
        mkl_dcsrmm(&transa, &n_fpts_per_ele, &n_fields_mul_n_eles,
                   &n_upts_per_ele, &one, matdescra,
                   opp_1_data(i).get_ptr_cpu(), opp_1_cols(i).get_ptr_cpu(),
                   opp_1_b(i).get_ptr_cpu(), opp_1_e(i).get_ptr_cpu(),
                   tdisf_upts.get_ptr_cpu(0, 0, 0, i), &n_upts_per_ele, &one,
                   norm_tdisf_fpts.get_ptr_cpu(), &n_fpts_per_ele);
      }

#  endif
    } else {
      std::cerr << "ERROR: Unknown storage for opp_1 ... " << std::endl;
    }
    ...
    if (opp_1_sparse == 0) {
      cublasDgemm('N', 'N', n_fpts_per_ele, n_fields * n_eles, n_upts_per_ele,
                  1.0, opp_1(0).get_ptr_gpu(), n_fpts_per_ele,
                  tdisf_upts.get_ptr_gpu(0, 0, 0, 0), n_upts_per_ele, 0.0,
                  norm_tdisf_fpts.get_ptr_gpu(), n_fpts_per_ele);
      for (int i = 1; i < n_dims; i++) {
        cublasDgemm('N', 'N', n_fpts_per_ele, n_fields * n_eles, n_upts_per_ele,
                    1.0, opp_1(i).get_ptr_gpu(), n_fpts_per_ele,
                    tdisf_upts.get_ptr_gpu(0, 0, 0, i), n_upts_per_ele, 1.0,
                    norm_tdisf_fpts.get_ptr_gpu(), n_fpts_per_ele);
      }
    } else if (opp_1_sparse == 1) {
      bespoke_SPMV(n_fpts_per_ele, n_upts_per_ele, n_fields, n_eles,
                   opp_1_ell_data(0).get_ptr_gpu(),
                   opp_1_ell_indices(0).get_ptr_gpu(), opp_1_nnz_per_row(0),
                   tdisf_upts.get_ptr_gpu(0, 0, 0, 0),
                   norm_tdisf_fpts.get_ptr_gpu(), ele_type, order, 0);
      for (int i = 1; i < n_dims; i++) {
        bespoke_SPMV(n_fpts_per_ele, n_upts_per_ele, n_fields, n_eles,
                     opp_1_ell_data(i).get_ptr_gpu(),
                     opp_1_ell_indices(i).get_ptr_gpu(), opp_1_nnz_per_row(i),
                     tdisf_upts.get_ptr_gpu(0, 0, 0, i),
                     norm_tdisf_fpts.get_ptr_gpu(), ele_type, order, 1);
      }
    }
#endif
  }
  ...
  /*
   ...
   std::cout << std::setprecision(10)  << i << " " << j<< " " << k << " " <<
   tdisinvf_upts(i,j,k,m) << std::endl;
   */
  ...
  /*
   ...
   for (int k=0;k<n_fields;k++)
   std::cout << std::setprecision(10)  << i << " " << j<< " " << k << " " <<
   norm_tdisinvf_fpts(i,j,k) << std::endl;
   */
}

// calculate the divergence of the transformed discontinuous flux at the
// solution points
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      ...
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_upts_per_ele,
                  n_fields * n_eles, n_upts_per_ele, 1.0,
                  opp_2(0).get_ptr_cpu(), n_upts_per_ele,
                  tdisf_upts.get_ptr_cpu(0, 0, 0, 0), n_upts_per_ele, 0.0,
                  div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu(),
                  n_upts_per_ele);
      for (int i = 1; i < n_dims; i++) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_upts_per_ele,
                    n_fields * n_eles, n_upts_per_ele, 1.0,
                    opp_2(i).get_ptr_cpu(), n_upts_per_ele,
                    tdisf_upts.get_ptr_cpu(0, 0, 0, i), n_upts_per_ele, 1.0,
                    div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu(),
                    n_upts_per_ele);
      }
...
#  elif defined _NO_BLAS
      dgemm(n_upts_per_ele, n_fields * n_eles, n_upts_per_ele, 1.0, 0.0,
            opp_2(0).get_ptr_cpu(), tdisf_upts.get_ptr_cpu(0, 0, 0, 0),
            div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu());
      for (int i = 1; i < n_dims; i++) {
        dgemm(n_upts_per_ele, n_fields * n_eles, n_upts_per_ele, 1.0, 1.0,
              opp_2(i).get_ptr_cpu(), tdisf_upts.get_ptr_cpu(0, 0, 0, i),
              div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu());
      }
...
#  if defined _MKL_BLAS
      ...
      mkl_dcsrmm(
          &transa, &n_upts_per_ele, &n_fields_mul_n_eles, &n_upts_per_ele, &one,
          matdescra, opp_2_data(0).get_ptr_cpu(), opp_2_cols(0).get_ptr_cpu(),
          opp_2_b(0).get_ptr_cpu(), opp_2_e(0).get_ptr_cpu(),
          tdisf_upts.get_ptr_cpu(0, 0, 0, 0), &n_upts_per_ele, &zero,
          div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu(), &n_upts_per_ele);
      for (int i = 1; i < n_dims; i++) {
        mkl_dcsrmm(&transa, &n_upts_per_ele, &n_fields_mul_n_eles,
                   &n_upts_per_ele, &one, matdescra,
                   opp_2_data(i).get_ptr_cpu(), opp_2_cols(i).get_ptr_cpu(),
                   opp_2_b(i).get_ptr_cpu(), opp_2_e(i).get_ptr_cpu(),
                   tdisf_upts.get_ptr_cpu(0, 0, 0, i), &n_upts_per_ele, &one,
                   div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu(),
                   &n_upts_per_ele);
      }
...
#  endif
      ...
      std::cerr << "ERROR: Unknown storage for opp_2 ... " << std::endl;
    }
    ...
    if (opp_2_sparse == 0) {
      cublasDgemm('N', 'N', n_upts_per_ele, n_fields * n_eles, n_upts_per_ele,
                  1.0, opp_2(0).get_ptr_gpu(), n_upts_per_ele,
                  tdisf_upts.get_ptr_gpu(0, 0, 0, 0), n_upts_per_ele, 0.0,
                  div_tconf_upts(in_div_tconf_upts_to).get_ptr_gpu(),
                  n_upts_per_ele);
      for (int i = 1; i < n_dims; i++) {
        cublasDgemm('N', 'N', n_upts_per_ele, n_fields * n_eles, n_upts_per_ele,
                    1.0, opp_2(i).get_ptr_gpu(), n_upts_per_ele,
                    tdisf_upts.get_ptr_gpu(0, 0, 0, i), n_upts_per_ele, 1.0,
                    div_tconf_upts(in_div_tconf_upts_to).get_ptr_gpu(),
                    n_upts_per_ele);
      }
    } else if (opp_2_sparse == 1) {
      bespoke_SPMV(n_upts_per_ele, n_upts_per_ele, n_fields, n_eles,
                   opp_2_ell_data(0).get_ptr_gpu(),
                   opp_2_ell_indices(0).get_ptr_gpu(), opp_2_nnz_per_row(0),
                   tdisf_upts.get_ptr_gpu(0, 0, 0, 0),
                   div_tconf_upts(in_div_tconf_upts_to).get_ptr_gpu(), ele_type,
                   order, 0);
      for (int i = 1; i < n_dims; i++) {
        bespoke_SPMV(n_upts_per_ele, n_upts_per_ele, n_fields, n_eles,
                     opp_2_ell_data(i).get_ptr_gpu(),
                     opp_2_ell_indices(i).get_ptr_gpu(), opp_2_nnz_per_row(i),
                     tdisf_upts.get_ptr_gpu(0, 0, 0, i),
                     div_tconf_upts(in_div_tconf_upts_to).get_ptr_gpu(),
                     ele_type, order, 1);
      }
  ...
  /*
   ...
   for (int i=0;i<n_upts_per_ele;i++)
   //for (int k=0;k<n_fields;k++)
   std::cout << scientific << std::setw(16) << std::setprecision(12) <<
   div_tconf_upts(0)(i,j,0) << std::endl;
   */
}

// calculate divergence of the transformed continuous flux at the solution
// points
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
    ...
    cblas_daxpy(n_eles * n_fields * n_fpts_per_ele, -1.0,
                norm_tdisf_fpts.get_ptr_cpu(), 1, norm_tconf_fpts.get_ptr_cpu(),
                1);

#  elif defined _NO_BLAS

    daxpy(n_eles * n_fields * n_fpts_per_ele, -1.0,
          norm_tdisf_fpts.get_ptr_cpu(), norm_tconf_fpts.get_ptr_cpu());
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      ...
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_upts_per_ele,
                  n_fields * n_eles, n_fpts_per_ele, 1.0, opp_3.get_ptr_cpu(),
                  n_upts_per_ele, norm_tconf_fpts.get_ptr_cpu(), n_fpts_per_ele,
                  1.0, div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu(),
                  n_upts_per_ele);

#  elif defined _NO_BLAS
      dgemm(n_upts_per_ele, n_fields * n_eles, n_fpts_per_ele, 1.0, 1.0,
            opp_3.get_ptr_cpu(), norm_tconf_fpts.get_ptr_cpu(),
            div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu());
...
#  if defined _MKL_BLAS
      ...
      mkl_dcsrmm(
          &transa, &n_upts_per_ele, &n_fields_mul_n_eles, &n_fpts_per_ele, &one,
          matdescra, opp_3_data.get_ptr_cpu(), opp_3_cols.get_ptr_cpu(),
          opp_3_b.get_ptr_cpu(), opp_3_e.get_ptr_cpu(),
          norm_tconf_fpts.get_ptr_cpu(), &n_fpts_per_ele, &one,
          div_tconf_upts(in_div_tconf_upts_to).get_ptr_cpu(), &n_upts_per_ele);

#  endif
    } else {
      std::cerr << "ERROR: Unknown storage for opp_3 ... " << std::endl;
    }
    ...
    cublasDaxpy(n_eles * n_fields * n_fpts_per_ele, -1.0,
                norm_tdisf_fpts.get_ptr_gpu(), 1, norm_tconf_fpts.get_ptr_gpu(),
                1);

    if (opp_3_sparse == 0) {
      cublasDgemm('N', 'N', n_upts_per_ele, n_fields * n_eles, n_fpts_per_ele,
                  1.0, opp_3.get_ptr_gpu(), n_upts_per_ele,
                  norm_tconf_fpts.get_ptr_gpu(), n_fpts_per_ele, 1.0,
                  div_tconf_upts(in_div_tconf_upts_to).get_ptr_gpu(),
                  n_upts_per_ele);
    } else if (opp_3_sparse == 1) {
      bespoke_SPMV(n_upts_per_ele, n_fpts_per_ele, n_fields, n_eles,
                   opp_3_ell_data.get_ptr_gpu(),
                   opp_3_ell_indices.get_ptr_gpu(), opp_3_nnz_per_row,
                   norm_tconf_fpts.get_ptr_gpu(),
                   div_tconf_upts(in_div_tconf_upts_to).get_ptr_gpu(), ele_type,
                   order, 1);
    } else {
      std::cerr << "ERROR: Unknown storage for opp_3 ... " << std::endl;
...
// calculate uncorrected transformed gradient of the discontinuous solution at
// the solution points (mixed derivative)
...
void eles::calculate_gradient(int in_disu_upts_from) {
  if (n_eles != 0) {
    /*!
     ...
     (vector of solution values at solution points in all elements of the same
     type)
     */
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      for (int i = 0; i < n_dims; i++) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Arows, Bcols,
                    Acols, 1.0, opp_4(i).get_ptr_cpu(), Astride,
                    disu_upts(in_disu_upts_from).get_ptr_cpu(), Bstride, 0.0,
                    grad_disu_upts.get_ptr_cpu(0, 0, 0, i), Cstride);
      }

#  elif defined _NO_BLAS
      for (int i = 0; i < n_dims; i++) {
        dgemm(Arows, Bcols, Acols, 1.0, 0.0, opp_4(i).get_ptr_cpu(),
              disu_upts(in_disu_upts_from).get_ptr_cpu(),
...
#  endif
      ...
      std::cerr << "ERROR: Unknown storage for opp_4 ... " << std::endl;
    }
    ...
    if (opp_4_sparse == 0) {
      for (int i = 0; i < n_dims; i++) {
        cublasDgemm('N', 'N', Arows, Bcols, Acols, 1.0, opp_4(i).get_ptr_gpu(),
                    Astride, disu_upts(in_disu_upts_from).get_ptr_gpu(),
                    Bstride, 0.0, grad_disu_upts.get_ptr_gpu(0, 0, 0, i),
                    Cstride);
      }
    } else if (opp_4_sparse == 1) {
      for (int i = 0; i < n_dims; i++) {
        bespoke_SPMV(
            Arows, Acols, n_fields, n_eles, opp_4_ell_data(i).get_ptr_gpu(),
            opp_4_ell_indices(i).get_ptr_gpu(), opp_4_nnz_per_row(i),
            disu_upts(in_disu_upts_from).get_ptr_gpu(),
            grad_disu_upts.get_ptr_gpu(0, 0, 0, i), ele_type, order, 0);
      }
    }
#endif
  }
  ...
  /*
   ...
   std::cout << std::setprecision(10)  << i << " " << ele2global_ele(j) << " "
   << k
   << " "
   << m << " " << grad_disu_upts(i,j,k,m) << std::endl;
   }
   */
}
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      ...
      for (int i = 0; i < n_dims; i++) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Arows, Bcols,
                    Acols, 1.0, opp_5(i).get_ptr_cpu(), Astride,
                    delta_disu_fpts.get_ptr_cpu(), Bstride, 1.0,
                    grad_disu_upts.get_ptr_cpu(0, 0, 0, i), Cstride);
      }

#  elif defined _NO_BLAS
      for (int i = 0; i < n_dims; i++) {
        dgemm(Arows, Bcols, Acols, 1.0, 1.0, opp_5(i).get_ptr_cpu(),
...
#  endif
      ...
      std::cerr << "ERROR: Unknown storage for opp_5 ... " << std::endl;
    }
    ...
    if (opp_5_sparse == 0) {
      for (int i = 0; i < n_dims; i++) {
        cublasDgemm('N', 'N', Arows, Bcols, Acols, 1.0, opp_5(i).get_ptr_gpu(),
                    Astride, delta_disu_fpts.get_ptr_gpu(), Bstride, 1.0,
                    grad_disu_upts.get_ptr_gpu(0, 0, 0, i), Cstride);
      }
    } else if (opp_5_sparse == 1) {
      for (int i = 0; i < n_dims; i++) {
        bespoke_SPMV(
            Arows, Acols, n_fields, n_eles, opp_5_ell_data(i).get_ptr_gpu(),
            opp_5_ell_indices(i).get_ptr_gpu(), opp_5_nnz_per_row(i),
            delta_disu_fpts.get_ptr_gpu(),
            grad_disu_upts.get_ptr_gpu(0, 0, 0, i), ele_type, order, 1);
      }
    }

    transform_grad_disu_upts_kernel_wrapper(
        n_upts_per_ele, n_dims, n_fields, n_eles, grad_disu_upts.get_ptr_gpu(),
        detjac_upts.get_ptr_gpu(), J_dyn_upts.get_ptr_gpu(),
        JGinv_upts.get_ptr_gpu(), JGinv_dyn_upts.get_ptr_gpu(),
        run_input.equation, motion);
  ...
  /*
   ...
   << k
   << " "
   << " " << delta_disu_fpts(i,j,k) << std::endl;
   }
   }
   */
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      ...
      for (int i = 0; i < n_dims; i++) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Arows, Bcols,
                    Acols, 1.0, opp_6.get_ptr_cpu(), Astride,
                    grad_disu_upts.get_ptr_cpu(0, 0, 0, i), Bstride, 0.0,
                    grad_disu_fpts.get_ptr_cpu(0, 0, 0, i), Cstride);
      }

#  elif defined _NO_BLAS
      for (int i = 0; i < n_dims; i++) {
        dgemm(Arows, Bcols, Acols, 1.0, 0.0, opp_6.get_ptr_cpu(),
              grad_disu_upts.get_ptr_cpu(0, 0, 0, i),
              grad_disu_fpts.get_ptr_cpu(0, 0, 0, i));
      }
#  endif
...
#  endif
      ...
      std::cerr << "ERROR: Unknown storage for opp_6 ... " << std::endl;
    }
    ...
    if (opp_6_sparse == 0) {
      for (int i = 0; i < n_dims; i++) {
        cublasDgemm('N', 'N', Arows, Bcols, Acols, 1.0, opp_6.get_ptr_gpu(),
                    Astride, grad_disu_upts.get_ptr_gpu(0, 0, 0, i), Bstride,
                    0.0, grad_disu_fpts.get_ptr_gpu(0, 0, 0, i), Cstride);
      }
    } else if (opp_6_sparse == 1) {
      for (int i = 0; i < n_dims; i++) {
        bespoke_SPMV(
            Arows, Acols, n_fields, n_eles, opp_6_ell_data.get_ptr_gpu(),
            opp_6_ell_indices.get_ptr_gpu(), opp_6_nnz_per_row,
            grad_disu_upts.get_ptr_gpu(0, 0, 0, i),
            grad_disu_fpts.get_ptr_gpu(0, 0, 0, i), ele_type, order, 0);
      }
    }
  ...
  /*
   ...
   for (int k=0;k<n_fields;k++)
   for (int m=0;m<n_dims;m++)
   std::cout << std::setprecision(10)  << i << " " << j<< " " << k << " " << m
   << " "
   << grad_disu_fpts(i,j,k,m) << std::endl;
   */
}

/*! If at first RK step and using certain LES models, compute some model-related
 quantities. If using similarity or WALE-similarity (WSM) models, compute
 filtered solution and Leonard tensors. If using spectral vanishing viscosity
 (SVV) model, compute filtered solution. */
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Arows, Bcols, Acols,
                1.0, filter_upts.get_ptr_cpu(), Astride,
                disu_upts(in_disu_upts_from).get_ptr_cpu(), Bstride, 0.0,
                disuf_upts.get_ptr_cpu(), Cstride);

#  elif defined _NO_BLAS
    dgemm(Arows, Bcols, Acols, 1.0, 0.0, filter_upts.get_ptr_cpu(),
          disu_upts(in_disu_upts_from).get_ptr_cpu(), disuf_upts.get_ptr_cpu());
      ...
      for (j = 0; j < n_eles; j++) {
        for (k = 0; k < n_fields; k++) {
          ...
          for (l = 0; l < n_upts_per_ele; l++) {
            disuf_upts(i, j, k) +=
                filter_upts(i, l) * disu_upts(in_disu_upts_from)(l, j, k);
          }
        }
      }
    }
      ...
      for (i = 0; i < n_upts_per_ele; i++) {
        for (j = 0; j < n_eles; j++) {
          ...
          if (n_dims == 2) {
            ...
            utemp(3) -= 0.5 * (utemp(1) * utemp(1) + utemp(2) * utemp(2)) /
                        utemp(0);  // internal energy*rho
            ...
            /*! velocity-energy product */
            utemp(4) -= 0.5 *
                        (utemp(1) * utemp(1) + utemp(2) * utemp(2) +
                         utemp(3) * utemp(3)) /
                        utemp(0);  // internal energy*rho
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      ...
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Arows, Bcols,
                  Acols, 1.0, filter_upts.get_ptr_cpu(), Astride,
                  uu.get_ptr_cpu(), Bstride, 0.0, Lu.get_ptr_cpu(), Cstride);

      Bcols = n_dims * n_eles;

      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Arows, Bcols,
                  Acols, 1.0, filter_upts.get_ptr_cpu(), Astride,
                  ue.get_ptr_cpu(), Bstride, 0.0, Le.get_ptr_cpu(), Cstride);
...
#  elif defined _NO_BLAS
      ...
      dgemm(Arows, Bcols, Acols, 1.0, 0.0, filter_upts.get_ptr_cpu(),
            uu.get_ptr_cpu(), Lu.get_ptr_cpu());

      Bcols = n_dims * n_eles;

      dgemm(Arows, Bcols, Acols, 1.0, 0.0, filter_upts.get_ptr_cpu(),
            ue.get_ptr_cpu(), Le.get_ptr_cpu());
...
#  endif
      ...
      for (i = 0; i < n_upts_per_ele; i++) {
        for (j = 0; j < n_eles; j++) {
          // filtered solution
          for (k = 0; k < n_fields; k++) utemp(k) = disuf_upts(i, j, k);
          ...
          if (n_dims == 2) {
            ...
            // internal energy*rho
            utemp(3) -=
                0.5 * (utemp(1) * utemp(1) + utemp(2) * utemp(2)) / utemp(0);
    ...
    cublasDgemm('N', 'N', Arows, Bcols, Acols, 1.0, filter_upts.get_ptr_gpu(),
                Astride, disu_upts(in_disu_upts_from).get_ptr_gpu(), Bstride,
                0.0, disuf_upts.get_ptr_gpu(), Cstride);
    ...
    if (sgs_model == 2 || sgs_model == 4) {
      ...
      calc_similarity_model_kernel_wrapper(
          0, n_fields, n_upts_per_ele, n_eles, n_dims,
          disu_upts(in_disu_upts_from).get_ptr_gpu(), disuf_upts.get_ptr_gpu(),
          uu.get_ptr_gpu(), ue.get_ptr_gpu(), Lu.get_ptr_gpu(),
          Le.get_ptr_gpu());
      ...
      cublasDgemm('N', 'N', Arows, Bcols, Acols, 1.0, filter_upts.get_ptr_gpu(),
                  Astride, uu.get_ptr_gpu(), Bstride, 0.0, Lu.get_ptr_gpu(),
                  Cstride);

      Bcols = n_dims * n_eles;

      cublasDgemm('N', 'N', Arows, Bcols, Acols, 1.0, filter_upts.get_ptr_gpu(),
                  Astride, ue.get_ptr_gpu(), Bstride, 0.0, Le.get_ptr_gpu(),
                  Cstride);

      /*! compute Leonard tensors Lu, Le (pass flag=1 to wrapper function) */
      calc_similarity_model_kernel_wrapper(
          1, n_fields, n_upts_per_ele, n_eles, n_dims,
          disu_upts(in_disu_upts_from).get_ptr_gpu(), disuf_upts.get_ptr_gpu(),
          uu.get_ptr_gpu(), ue.get_ptr_gpu(), Lu.get_ptr_gpu(),
          Le.get_ptr_gpu());
...
void eles::evaluate_viscFlux(int in_disu_upts_from) {
  if (n_eles != 0) {
    ...
    for (i = 0; i < n_eles; i++) {
      ...
      for (j = 0; j < n_upts_per_ele; j++) {
        ...
        if (LES != 0 || wall_model != 0) {
          ...
          for (k = 0; k < n_fields; k++)
            for (l = 0; l < n_dims; l++) temp_f(k, l) += temp_sgsf(k, l);
        }
        ...
        if (LES > 0) {
          ...
          if (motion) {
            ...
            for (k = 0; k < n_fields; k++) {
              for (l = 0; l < n_dims; l++) {
                for (m = 0; m < n_dims; m++) {
                  temp_sgsf_ref(k, l) +=
                      JGinv_dyn_upts(l, m, j, i) * temp_sgsf(k, m);
                }
              }
            }
          ...
          for (k = 0; k < n_fields; k++) {
            for (l = 0; l < n_dims; l++) {
              ...
              for (m = 0; m < n_dims; m++) {
                sgsf_upts(j, i, k, l) +=
                    JGinv_upts(l, m, j, i) * temp_sgsf(k, m);
              }
            }
          }
        }
    ...
    evaluate_viscFlux_gpu_kernel_wrapper(
        n_upts_per_ele, n_dims, n_fields, n_eles, ele_type, order,
        run_input.filter_ratio, LES, motion, sgs_model, wall_model,
        run_input.wall_layer_t, wall_distance.get_ptr_gpu(),
        twall.get_ptr_gpu(), Lu.get_ptr_gpu(), Le.get_ptr_gpu(),
        disu_upts(in_disu_upts_from).get_ptr_gpu(), tdisf_upts.get_ptr_gpu(),
        sgsf_upts.get_ptr_gpu(), grad_disu_upts.get_ptr_gpu(),
        detjac_upts.get_ptr_gpu(), J_dyn_upts.get_ptr_gpu(),
        JGinv_upts.get_ptr_gpu(), JGinv_dyn_upts.get_ptr_gpu(), run_input.gamma,
        run_input.prandtl, run_input.rt_inf, run_input.mu_inf, run_input.c_sth,
        run_input.fix_vis, run_input.equation, run_input.diff_coeff,
        run_input.turb_model, run_input.c_v1, run_input.omega,
        run_input.prandtl_t);
...
void eles::calc_sgsf_upts(array<double> &temp_u, array<double> &temp_grad_u,
                          double &detjac, int ele, int upt,
  ...
  mu = (run_input.mu_inf) * pow(rt_ratio, 1.5) * (1 + (run_input.c_sth)) /
       (rt_ratio + (run_input.c_sth));
  ...
  if (wall) {
    ...
    calc_wall_stress(rho, urot, inte, mu, run_input.prandtl, run_input.gamma, y,
                     tw, qw);
    ...
    for (i = 0; i < n_dims; ++i)
      twall(upt, ele, i + 1) = tw(i);  // momentum flux
    ...
    for (i = 0; i < n_dims; ++i)
      for (j = 0; j < n_dims; ++j)
        for (k = 0; k < n_dims; ++k) temp(i, j) += tau(i, k) * Mrot(k, j);

    zero_array(tau);
    for (i = 0; i < n_dims; ++i)
      for (j = 0; j < n_dims; ++j)
        for (k = 0; k < n_dims; ++k) tau(i, j) += Mrot(k, i) * temp(k, j);
  ...
  // Free-stream SGS flux
    ...
    if (wall_model != 0)
      for (i = 0; i < n_dims; ++i) twall(upt, ele, i) = 0.0;
    ...
    if (eddy == 1) {
      ...
      // Strain modulus
      for (i = 0; i < n_dims; i++)
        for (j = 0; j < n_dims; j++) Smod += 2.0 * S(i, j) * S(i, j);
      ...
      //
      //  NICOUD F., DUCROS F.: "Subgrid-Scale Stress Modelling Based on the
      //  Square
      ...
      for (j = 0; j < n_dims; j++) {
        ...
        temp_sgsf(n_fields - 1, j) =
            -1.0 * run_input.gamma * mu_t / Pr * de(j);  // Energy flux
    ...
    if (sim == 1) {
      for (j = 0; j < n_dims; j++) {
        temp_sgsf(0, j) += 0.0;  // Density flux
        temp_sgsf(n_fields - 1, j) +=
            run_input.gamma * rho * Le(upt, ele, j);  // Energy flux
      }
  ...
  if (n_eles != 0) {
    ...
    for (i = 0; i < n_eles; i++) {
      for (j = 0; j < n_upts_per_ele; j++) {
        ...
        // source term
        if (n_dims == 2)
          calc_source_SA_2d(temp_u, temp_grad_u, wall_distance_mag(j, i),
                            src_upts(j, i, n_fields - 1));
        else if (n_dims == 3)
          calc_source_SA_3d(temp_u, temp_grad_u, wall_distance_mag(j, i),
                            src_upts(j, i, n_fields - 1));
        else
          std::cerr << "ERROR: Invalid number of dimensions ... " << std::endl;
      }
...
#ifdef _GPU
    calc_src_upts_SA_gpu_kernel_wrapper(
        n_upts_per_ele, n_dims, n_fields, n_eles,
        disu_upts(in_disu_upts_from).get_ptr_gpu(),
        grad_disu_upts.get_ptr_gpu(), wall_distance_mag.get_ptr_gpu(),
        src_upts.get_ptr_gpu(), run_input.gamma, run_input.prandtl,
        run_input.rt_inf, run_input.mu_inf, run_input.c_sth, run_input.fix_vis,
        run_input.c_v1, run_input.c_v2, run_input.c_v3, run_input.c_b1,
        run_input.c_b2, run_input.c_w2, run_input.c_w3, run_input.omega,
        run_input.Kappa);
#endif
  }
}
...
void eles::calc_wall_distance(int n_seg_noslip_inters, int n_tri_noslip_inters,
                              int n_quad_noslip_inters,
...
void eles::calc_wall_distance_parallel(
    array<int> n_seg_inters_array, array<int> n_tri_inters_array,
    array<int> n_quad_inters_array, array<array<double>> loc_noslip_bdy_global,
    int nproc) {
  if (n_eles != 0) {
    int i, j, k, m, n, p;
    int n_fpts_per_inter_seg = order + 1;
...
void eles::calc_wall_stress(double rho, array<double> &urot, double ene,
                            double mu, double Pr, double gamma, double y,
                            array<double> &tau_wall, double q_wall) {
  double eps = 1.e-10;
  ...
  if (u > eps) {
    ...
    if (run_input.wall_model == 1) {
      ...
      // Wall heat flux
      if (Rey < Rey_c)
        q_wall = ene * gamma * tw / (Pr * u);
      else
        q_wall = ene * gamma * tw /
                 (Pr * (u + utau * sqrt(Rey_c) * (Pr / Pr_t - 1.0)));
    }
     ...
     */
      ...
      Rey0 = u * phi;
      utau = 0.0;
      for (i = 0; i < n_dims; i++) utau += tau_wall(i) * tau_wall(i);
      ...
      if (yplus <= ymatch)
        ...
        q_wall =
            ene * gamma * tw / (Pr * (u + utau * ymatch * (Pr / Pr_t - 1.0)));
    }
  }
...
double eles::wallfn_br(double yplus, double A, double B, double E,
                       double kappa) {
  double Rey;
  ...
  if (yplus < 0.5)
    ...
    Rey = yplus * log(E * yplus) / kappa;
  else
    Rey = yplus * (A * log(yplus) + B);
...
#  if defined _ACCELERATE_BLAS || defined _MKL_BLAS || defined _STANDARD_BLAS
      ...
      for (int i = 0; i < n_dims; i++) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, Arows, Bcols,
                    Acols, 1.0, opp_0.get_ptr_cpu(), Astride,
                    sgsf_upts.get_ptr_cpu(0, 0, 0, i), Bstride, 0.0,
                    sgsf_fpts.get_ptr_cpu(0, 0, 0, i), Cstride);
      }
...
#  if defined _MKL_BLAS
      ...
      for (int i = 0; i < n_dims; i++) {
        mkl_dcsrmm(&transa, &n_fpts_per_ele, &n_fields_mul_n_eles,
                   &n_upts_per_ele, &one, matdescra, opp_0_data.get_ptr_cpu(),
                   opp_0_cols.get_ptr_cpu(), opp_0_b.get_ptr_cpu(),
                   opp_0_e.get_ptr_cpu(), sgsf_upts.get_ptr_cpu(0, 0, 0, i),
                   &n_upts_per_ele, &zero, sgsf_fpts.get_ptr_cpu(0, 0, 0, i),
                   &n_fpts_per_ele);
      }
...
#  endif
    ...
    if (opp_0_sparse == 0) {
      for (int i = 0; i < n_dims; i++) {
        cublasDgemm('N', 'N', Arows, Bcols, Acols, 1.0, opp_0.get_ptr_gpu(),
                    Astride, sgsf_upts.get_ptr_gpu(0, 0, 0, i), Bstride, 0.0,
                    sgsf_fpts.get_ptr_gpu(0, 0, 0, i), Cstride);
      }
    } else if (opp_0_sparse == 1) {
      for (int i = 0; i < n_dims; i++) {
        bespoke_SPMV(n_fpts_per_ele, n_upts_per_ele, n_fields, n_eles,
                     opp_0_ell_data.get_ptr_gpu(),
                     opp_0_ell_indices.get_ptr_gpu(), opp_0_nnz_per_row,
                     sgsf_upts.get_ptr_gpu(0, 0, 0, i),
                     sgsf_fpts.get_ptr_gpu(0, 0, 0, i), ele_type, order, 0);
      }
    } else {
      std::cerr << "ERROR: Unknown storage for opp_0 ... " << std::endl;
    }
...
void eles::shock_capture_concentration(int in_disu_upts_from) {
  if (n_eles != 0) {
    ...
    shock_capture_concentration_gpu_kernel_wrapper(
        n_eles, n_upts_per_ele, n_fields, order, ele_type, run_input.artif_type,
        run_input.s0, run_input.kappa,
        disu_upts(in_disu_upts_from).get_ptr_gpu(),
        inv_vandermonde.get_ptr_gpu(), inv_vandermonde2D.get_ptr_gpu(),
        vandermonde2D.get_ptr_gpu(), concentration_array.get_ptr_gpu(),
        sensor.get_ptr_gpu(), sigma.get_ptr_gpu());
#endif

#ifdef _CPU
    shock_capture_concentration_cpu(
        n_eles, n_upts_per_ele, n_fields, order, ele_type, run_input.artif_type,
        run_input.s0, run_input.kappa,
        disu_upts(in_disu_upts_from).get_ptr_cpu(),
        inv_vandermonde.get_ptr_cpu(), inv_vandermonde2D.get_ptr_cpu(),
        vandermonde2D.get_ptr_cpu(), concentration_array.get_ptr_cpu(),
        sensor.get_ptr_cpu(), sigma.get_ptr_cpu());
#endif
  }
}

void eles::shock_capture_concentration_cpu(
    int in_n_eles, int in_n_upts_per_ele, int in_n_fields, int in_order,
    int in_ele_type, int in_artif_type, double s0, double kappa,
    double *in_disu_upts_ptr, double *in_inv_vandermonde_ptr,
    ...
    for (int m = 0; m < in_n_eles; m++) {
      ...
      for (int i = 0; i < in_order + 1; i++) {
        for (int j = 0; j < in_order + 1; j++) {
          nodal_rho[j] =
              in_disu_upts_ptr[m * in_n_upts_per_ele + i * (in_order + 1) + j];
        }
        ...
        for (int j = 0; j < in_order + 1; j++) {
          ...
          for (int k = 0; k < in_order + 1; k++) {
            modal_rho[j] +=
                in_inv_vandermonde_ptr[j + k * (in_order + 1)] * nodal_rho[k];
          }
        }
        ...
        for (int j = 0; j < in_order + 1; j++) {
          ...
          for (int k = 0; k < in_order + 1; k++)
            uE[j] +=
                modal_rho[k] * concentration_array_ptr[j * (in_order + 1) + k];

          uE[j] = abs((3.1415 / (in_order + 1)) * uE[j]);
          temp = pow(uE[j], p) * pow(in_order + 1, p / 2);

          if (temp >= J) shock_found++;

          if (temp > tmp_sensor) tmp_sensor = temp;
        }
      }
      ...
      for (int i = 0; i < in_order + 1; i++) {
        for (int j = 0; j < in_order + 1; j++) {
          nodal_rho[j] =
              in_disu_upts_ptr[m * in_n_upts_per_ele + j * (in_order + 1) + i];
        }
      ...
      if (tmp_sensor > s0 + kappa && in_artif_type == 1) {
        ...
        for (int k = 0; k < in_n_fields; k++) {
          for (int i = 0; i < in_n_upts_per_ele; i++) {
            nodal_sol[i] =
                in_disu_upts_ptr[m * in_n_upts_per_ele + k * stride + i];
          }
          ...
          for (int i = 0; i < in_n_upts_per_ele; i++) {
            ...
            for (int j = 0; j < in_n_upts_per_ele; j++)
              modal_sol[i] +=
                  in_inv_vandermonde2D_ptr[i + j * in_n_upts_per_ele] *
                  nodal_sol[j];
          ...
          for (int i = 0; i < in_n_upts_per_ele; i++) {
            nodal_sol[i] = 0;
            for (int j = 0; j < in_n_upts_per_ele; j++)
              nodal_sol[i] += in_vandermonde2D_ptr[i + j * in_n_upts_per_ele] *
                              modal_sol[j];
...
int eles::get_ele_type(void) { return ele_type; }

// get number of elements

int eles::get_n_eles(void) { return n_eles; }

// get number of ppts_per_ele
int eles::get_n_ppts_per_ele(void) { return n_ppts_per_ele; }

// get number of peles_per_ele
int eles::get_n_peles_per_ele(void) { return n_peles_per_ele; }

// get number of verts_per_ele
int eles::get_n_verts_per_ele(void) { return n_verts_per_ele; }

// get number of elements

int eles::get_n_dims(void) { return n_dims; }

// get number of solution fields

int eles::get_n_fields(void) { return n_fields; }

// get number of solutions points per element

int eles::get_n_upts_per_ele(void) { return n_upts_per_ele; }
...
void eles::set_shape_node(int in_spt, int in_ele, array<double> &in_pos) {
  for (int i = 0; i < n_dims; i++) {
    ...
    for (int j = 0; j < 5; j++) shape_dyn(i, in_spt, in_ele, j) = in_pos(i);
  }
}

void eles::set_dynamic_shape_node(int in_spt, int in_ele,
                                  array<double> &in_pos) {
  for (int i = 0; i < n_dims; i++) {
...
void eles::set_rank(int in_rank) { rank = in_rank; }
...
// set opp_0 (transformed discontinuous solution at solution points to
// transformed discontinuous solution at flux points)
...
void eles::set_opp_0(int in_sparse) {
  ...
  if (in_sparse == 0) {
...
#ifdef _GPU
    array_to_ellpack(opp_0, opp_0_ell_data, opp_0_ell_indices,
                     opp_0_nnz_per_row);
    ...
    std::cerr << "ERROR: Invalid sparse matrix form ... " << std::endl;
  }
}

// set opp_1 (transformed discontinuous flux at solution points to normal
// transformed discontinuous flux at flux points)
...
void eles::set_opp_1(int in_sparse) {
...
#ifdef _GPU
  for (int i = 0; i < n_dims; i++) opp_1(i).cp_cpu_gpu();
#endif
  ...
  if (in_sparse == 0) {
...
#ifdef _CPU
    for (int i = 0; i < n_dims; i++) {
      array_to_mklcsr(opp_1(i), opp_1_data(i), opp_1_cols(i), opp_1_b(i),
                      opp_1_e(i));
    }
#endif
    ...
    for (int i = 0; i < n_dims; i++) {
      array_to_ellpack(opp_1(i), opp_1_ell_data(i), opp_1_ell_indices(i),
                       opp_1_nnz_per_row(i));
...
// set opp_2 (transformed discontinuous flux at solution points to divergence of
// transformed discontinuous flux at solution points)
...
void eles::set_opp_2(int in_sparse) {
...
#ifdef _GPU
  for (int i = 0; i < n_dims; i++) opp_2(i).cp_cpu_gpu();
#endif
  ...
  if (in_sparse == 0) {
...
#ifdef _CPU
    for (int i = 0; i < n_dims; i++) {
      array_to_mklcsr(opp_2(i), opp_2_data(i), opp_2_cols(i), opp_2_b(i),
                      opp_2_e(i));
    }
#endif
    ...
    for (int i = 0; i < n_dims; i++) {
      array_to_ellpack(opp_2(i), opp_2_ell_data(i), opp_2_ell_indices(i),
                       opp_2_nnz_per_row(i));
...
// set opp_3 (normal transformed correction flux at edge flux points to
// divergence of transformed correction flux at solution points)
...
#ifdef _GPU
    array_to_ellpack(opp_3, opp_3_ell_data, opp_3_ell_indices,
                     opp_3_nnz_per_row);
...
// set opp_4 (transformed solution at solution points to transformed gradient of
// transformed solution at solution points)
...
void eles::set_opp_4(int in_sparse) {
...
#ifdef _GPU
  for (int i = 0; i < n_dims; i++) opp_4(i).cp_cpu_gpu();
#endif
  ...
  if (in_sparse == 0) {
...
#ifdef _CPU
    for (int i = 0; i < n_dims; i++) {
      array_to_mklcsr(opp_4(i), opp_4_data(i), opp_4_cols(i), opp_4_b(i),
                      opp_4_e(i));
    }
#endif
    ...
    for (int i = 0; i < n_dims; i++) {
      array_to_ellpack(opp_4(i), opp_4_ell_data(i), opp_4_ell_indices(i),
                       opp_4_nnz_per_row(i));
...
// transformed solution correction at flux points to transformed gradient
// correction at solution points
...
void eles::set_opp_5(int in_sparse) {
...
#ifdef _GPU
  for (int i = 0; i < n_dims; i++) opp_5(i).cp_cpu_gpu();
#endif
  ...
  if (in_sparse == 0) {
...
#ifdef _CPU
    for (int i = 0; i < n_dims; i++) {
      array_to_mklcsr(opp_5(i), opp_5_data(i), opp_5_cols(i), opp_5_b(i),
                      opp_5_e(i));
    }
#endif
    ...
    for (int i = 0; i < n_dims; i++) {
      array_to_ellpack(opp_5(i), opp_5_ell_data(i), opp_5_ell_indices(i),
                       opp_5_nnz_per_row(i));
...
// transformed gradient at solution points to transformed gradient at flux
// points
...
#ifdef _GPU
    array_to_ellpack(opp_6, opp_6_ell_data, opp_6_ell_indices,
                     opp_6_nnz_per_row);
...
void eles::set_opp_r(void) {
  ...
  for (i = 0; i < n_upts_per_ele_rest; i++) {
    for (j = 0; j < n_upts_per_ele; j++) {
      for (k = 0; k < n_dims; k++) loc(k) = loc_upts(k, j);
...
void eles::calc_pos_ppts(int in_ele, array<double> &out_pos_ppts) {
  ...
  for (i = 0; i < n_ppts_per_ele; i++) {
    ...
    for (j = 0; j < n_dims;
         j++)  // TODO: can this be made more efficient/simpler?
    {
  ...
  if (n_eles != 0) {
    ...
    for (i = 0; i < n_fields; i++) {
      for (j = 0; j < n_upts_per_ele; j++) {
        if (motion) {
          disu_upts_plot(j, i) =
              disu_upts(0)(j, in_ele, i) / J_dyn_upts(j, in_ele);
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_ppts_per_ele,
                n_fields, n_upts_per_ele, 1.0, opp_p.get_ptr_cpu(),
                n_ppts_per_ele, disu_upts_plot.get_ptr_cpu(), n_upts_per_ele,
                0.0, out_disu_ppts.get_ptr_cpu(), n_ppts_per_ele);

#elif defined _NO_BLAS
    dgemm(n_ppts_per_ele, n_fields, n_upts_per_ele, 1.0, 0.0,
          opp_p.get_ptr_cpu(), disu_upts_plot.get_ptr_cpu(),
          out_disu_ppts.get_ptr_cpu());
...
void eles::calc_grad_disu_ppts(int in_ele, array<double> &out_grad_disu_ppts) {
  if (n_eles != 0) {
    ...
    for (i = 0; i < n_dims; i++) {
      cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_ppts_per_ele,
                  n_fields, n_upts_per_ele, 1.0, opp_p.get_ptr_cpu(),
                  n_ppts_per_ele, grad_disu_upts_temp.get_ptr_cpu(0, 0, i),
                  n_upts_per_ele, 0.0, out_grad_disu_ppts.get_ptr_cpu(0, 0, i),
                  n_ppts_per_ele);
    }
    ...
    for (i = 0; i < n_dims; i++) {
      dgemm(n_ppts_per_ele, n_fields, n_upts_per_ele, 1.0, 0.0,
            opp_p.get_ptr_cpu(), grad_disu_upts_temp.get_ptr_cpu(0, 0, i),
            out_grad_disu_ppts.get_ptr_cpu(0, 0, i));
    }
    ...
    for (i = 0; i < n_ppts_per_ele; i++) {
      for (k = 0; k < n_fields; k++) {
        for (l = 0; l < n_dims; l++) {
          ...
          for (j = 0; j < n_upts_per_ele; j++) {
            out_grad_disu_ppts(i, k, l) +=
                opp_p(i, j) * grad_disu_upts_temp(j, k, l);
          }
        }
      }
    }
...
void eles::calc_time_average_ppts(int in_ele,
                                  array<double> &out_disu_average_ppts) {
  if (n_eles != 0) {
    int i, j, k;
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_ppts_per_ele,
                n_average_fields, n_upts_per_ele, 1.0, opp_p.get_ptr_cpu(),
                n_ppts_per_ele, disu_average_upts_plot.get_ptr_cpu(),
                n_upts_per_ele, 0.0, out_disu_average_ppts.get_ptr_cpu(),
                n_ppts_per_ele);

#elif defined _NO_BLAS
    dgemm(n_ppts_per_ele, n_average_fields, n_upts_per_ele, 1.0, 0.0,
          opp_p.get_ptr_cpu(), disu_average_upts_plot.get_ptr_cpu(),
          out_disu_average_ppts.get_ptr_cpu());
    ...
    for (i = 0; i < n_ppts_per_ele; i++) {
      for (k = 0; k < n_average_fields; k++) {
        ...
        for (j = 0; j < n_upts_per_ele; j++) {
          out_disu_average_ppts(i, k) +=
              opp_p(i, j) * disu_average_upts_plot(j, k);
        }
      }
    }
...
void eles::calc_epsilon_ppts(int in_ele, array<double> &out_epsilon_ppts) {
  if (n_eles != 0) {
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n_ppts_per_ele, 1,
                n_upts_per_ele, 1.0, opp_p.get_ptr_cpu(), n_ppts_per_ele,
                epsilon_upts_plot.get_ptr_cpu(), n_upts_per_ele, 0.0,
                out_epsilon_ppts.get_ptr_cpu(), n_ppts_per_ele);
...
void eles::calc_diagnostic_fields_ppts(int in_ele, array<double> &in_disu_ppts,
                                       array<double> &in_grad_disu_ppts,
                                       array<double> &in_sensor_ppts,
                                       array<double> &in_epsilon_ppts,
                                       array<double> &out_diag_field_ppts,
                                       double &time) {
  int i, j, k, m;
  ...
  for (j = 0; j < n_ppts_per_ele; j++) {
    ...
    // Compute pressure
    pressure = (run_input.gamma - 1.0) *
               (in_disu_ppts(j, n_dims + 1) - 0.5 * in_disu_ppts(j, 0) * v_sq);
    ...
    for (k = 0; k < n_diagnostic_fields; k++) {
      ...
      // flow properties
        ...
        diagfield_upt =
            sqrt(v_sq / (run_input.gamma * pressure / in_disu_ppts(j, 0)));
      } else if (run_input.diagnostic_fields(k) == "pressure") {
        diagfield_upt = pressure;
      }
      // turbulence metrics
      else if (run_input.diagnostic_fields(k) == "vorticity" ||
               run_input.diagnostic_fields(k) == "q_criterion") {
        u = in_disu_ppts(j, 1) * irho;
        v = in_disu_ppts(j, 2) * irho;

        dudx = irho *
               (in_grad_disu_ppts(j, 1, 0) - u * in_grad_disu_ppts(j, 0, 0));
        dudy = irho *
               (in_grad_disu_ppts(j, 1, 1) - u * in_grad_disu_ppts(j, 0, 1));
        dvdx = irho *
               (in_grad_disu_ppts(j, 2, 0) - v * in_grad_disu_ppts(j, 0, 0));
        dvdy = irho *
               (in_grad_disu_ppts(j, 2, 1) - v * in_grad_disu_ppts(j, 0, 1));
        ...
        if (n_dims == 2) {
          ...
          dudz = irho *
                 (in_grad_disu_ppts(j, 1, 2) - u * in_grad_disu_ppts(j, 0, 2));
          dvdz = irho *
                 (in_grad_disu_ppts(j, 2, 2) - v * in_grad_disu_ppts(j, 0, 2));

          dwdx = irho *
                 (in_grad_disu_ppts(j, 3, 0) - w * in_grad_disu_ppts(j, 0, 0));
          dwdy = irho *
                 (in_grad_disu_ppts(j, 3, 1) - w * in_grad_disu_ppts(j, 0, 1));
          dwdz = irho *
                 (in_grad_disu_ppts(j, 3, 2) - w * in_grad_disu_ppts(j, 0, 2));
          ...
          if (run_input.diagnostic_fields(k) == "vorticity") {
            ...
            SS = Sxx * Sxx + Syy * Syy + Szz * Szz + 2 * Sxy * Sxy +
                 2 * Sxz * Sxz + 2 * Syz * Syz;
      ...
      // Artificial Viscosity diagnostics
        ...
        std::cout << "plot_quantity = " << run_input.diagnostic_fields(k)
                  << ": " << std::flush;
        FatalError("plot_quantity not recognized");
      }
      if (isnan(diagfield_upt)) {
        std::cout << "In calculation of plot_quantitiy "
                  << run_input.diagnostic_fields(k) << ": " << std::flush;
        FatalError("NaN");
      }
...
void eles::set_transforms(void) {
  if (n_eles != 0) {
    ...
    // Determinant of Jacobian times inverse of Jacobian (Full vector transform
    // from physcial->reference frame)
    ...
    for (i = 0; i < n_eles; i++) {
      if ((i % (std::max(n_eles, 10) / 10)) == 0 && rank == 0)
        std::cout << std::fixed << std::setprecision(2)
                  << (i * 1.0 / n_eles) * 100 << "% " << std::flush;
      ...
      for (j = 0; j < n_upts_per_ele; j++) {
        ...
        if (n_dims == 2) {
          ...
          // store inverse of determinant of jacobian multiplied by jacobian at
          // the solution point
          ...
          detjac_upts(j, i) = xr * (ys * zt - yt * zs) -
                              xs * (yr * zt - yt * zr) +
                              xt * (yr * zs - ys * zr);
    ...
    if (rank == 0) std::cout << std::endl << " at flux points" << std::endl;
    ...
    for (i = 0; i < n_eles; i++) {
      ...
      for (j = 0; j < n_fpts_per_ele; j++) {
        ...
        if (n_dims == 2) {
          ...
          // temporarily store transformed normal dot inverse of determinant of
          // jacobian multiplied by jacobian at the flux point

          tnorm_dot_inv_detjac_mul_jac(0) = (tnorm_fpts(0, j) * d_pos(1, 1)) -
                                            (tnorm_fpts(1, j) * d_pos(1, 0));
          tnorm_dot_inv_detjac_mul_jac(1) = -(tnorm_fpts(0, j) * d_pos(0, 1)) +
                                            (tnorm_fpts(1, j) * d_pos(0, 0));

          // store magnitude of transformed normal dot inverse of determinant of
          // jacobian multiplied by jacobian at the flux point

          tdA_fpts(j, i) = sqrt(tnorm_dot_inv_detjac_mul_jac(0) *
                                    tnorm_dot_inv_detjac_mul_jac(0) +
                                tnorm_dot_inv_detjac_mul_jac(1) *
                                    tnorm_dot_inv_detjac_mul_jac(1));
          ...
          detjac_fpts(j, i) = xr * (ys * zt - yt * zs) -
                              xs * (yr * zt - yt * zr) +
                              xt * (yr * zs - ys * zr);
          ...
          tnorm_dot_inv_detjac_mul_jac(0) =
              ((tnorm_fpts(0, j) *
                (d_pos(1, 1) * d_pos(2, 2) - d_pos(1, 2) * d_pos(2, 1))) +
               (tnorm_fpts(1, j) *
                (d_pos(1, 2) * d_pos(2, 0) - d_pos(1, 0) * d_pos(2, 2))) +
               (tnorm_fpts(2, j) *
                (d_pos(1, 0) * d_pos(2, 1) - d_pos(1, 1) * d_pos(2, 0))));
          tnorm_dot_inv_detjac_mul_jac(1) =
              ((tnorm_fpts(0, j) *
                (d_pos(0, 2) * d_pos(2, 1) - d_pos(0, 1) * d_pos(2, 2))) +
               (tnorm_fpts(1, j) *
                (d_pos(0, 0) * d_pos(2, 2) - d_pos(0, 2) * d_pos(2, 0))) +
               (tnorm_fpts(2, j) *
                (d_pos(0, 1) * d_pos(2, 0) - d_pos(0, 0) * d_pos(2, 1))));
          tnorm_dot_inv_detjac_mul_jac(2) =
              ((tnorm_fpts(0, j) *
                (d_pos(0, 1) * d_pos(1, 2) - d_pos(0, 2) * d_pos(1, 1))) +
               (tnorm_fpts(1, j) *
                (d_pos(0, 2) * d_pos(1, 0) - d_pos(0, 0) * d_pos(1, 2))) +
               (tnorm_fpts(2, j) *
                (d_pos(0, 0) * d_pos(1, 1) - d_pos(0, 1) * d_pos(1, 0))));
...
void eles::set_transforms_dynamic(void) {
  if (n_eles != 0 && motion && first_time) {
    // Determinant of the dynamic -> static reference transformation matrix (
    // |G| )
  ...
  if (n_eles != 0 && motion) {
    ...
    array<double> pos(n_dims);
    array<double> d_pos(n_dims, n_dims);
    array<double> norm_dot_JGinv(
        n_dims);  // un-normalized normal vector in moving-physical domain
    ...
    if (rank == 0 && first_time) {
      std::cout << " Setting up dynamic->static transforms at solution points"
                << std::endl;
    }

    for (i = 0; i < n_eles; i++) {
      if ((i % (std::max(n_eles, 10) / 10)) == 0 && rank == 0 && first_time)
        std::cout << std::fixed << std::setprecision(2)
                  << (i * 1.0 / n_eles) * 100 << "% " << std::flush;
      ...
      for (j = 0; j < n_upts_per_ele; j++) {
        ...
        if (n_dims == 2) {
          ...
          // store determinant of jacobian multiplied by inverse of jacobian at
          // the solution point
    ...
    for (i = 0; i < n_eles; i++) {
      ...
      for (j = 0; j < n_fpts_per_ele; j++) {
        ...
        if (n_dims == 2) {
          ...
          // store determinant of jacobian multiplied by inverse of jacobian at
          // the flux point (dynamic -> static transformation matrix)
          ...
          // temporarily store transformed normal dot determinant of jacobian
          // multiplied by inverse of jacobian at the flux point
          norm_dot_JGinv(0) =
              (norm_fpts(j, i, 0) * ys - norm_fpts(j, i, 1) * yr);
          norm_dot_JGinv(1) =
              (-norm_fpts(j, i, 0) * xs + norm_fpts(j, i, 1) * xr);

          // store magnitude of transformed normal dot determinant of jacobian
          // multiplied by inverse of jacobian at the flux point
          ...
          // temporarily store moving-physical domain interface normal at the
          // flux point [transformed normal dot determinant of jacobian
          // multiplied by inverse of jacobian]
          norm_dot_JGinv(0) = ((norm_fpts(j, i, 0) * (ys * zt - yt * zs)) +
                               (norm_fpts(j, i, 1) * (yt * zr - yr * zt)) +
                               (norm_fpts(j, i, 2) * (yr * zs - ys * zr)));
          norm_dot_JGinv(1) = ((norm_fpts(j, i, 0) * (xt * zs - xs * zt)) +
                               (norm_fpts(j, i, 1) * (xr * zt - xt * zr)) +
                               (norm_fpts(j, i, 2) * (xs * zr - xr * zs)));
          norm_dot_JGinv(2) = ((norm_fpts(j, i, 0) * (xs * yt - xt * ys)) +
                               (norm_fpts(j, i, 1) * (xt * yr - xr * yt)) +
                               (norm_fpts(j, i, 2) * (xr * ys - xs * yr)));
    ...
    // To avoid re-setting up ALL transform arrays in the future (for dynamic
    // grids)
    first_time = false;
  }
#endif
  ...
  if (n_eles != 0 && motion) {
    ...
    if (rank == 0 && first_time)
      std::cout << "Setting dynamic transformations ... " << std::flush;
    set_transforms_dynamic_fpts_kernel_wrapper(
        n_fpts_per_ele, n_eles, n_dims, max_n_spts_per_ele,
        n_spts_per_ele.get_ptr_gpu(), detjac_fpts.get_ptr_gpu(),
        J_dyn_fpts.get_ptr_gpu(), JGinv_fpts.get_ptr_gpu(),
        JGinv_dyn_fpts.get_ptr_gpu(), ndA_dyn_fpts.get_ptr_gpu(),
        norm_fpts.get_ptr_gpu(), norm_dyn_fpts.get_ptr_gpu(),
        d_nodal_s_basis_fpts.get_ptr_gpu(), shape_dyn.get_ptr_gpu());
    set_transforms_dynamic_upts_kernel_wrapper(
        n_upts_per_ele, n_eles, n_dims, max_n_spts_per_ele,
        n_spts_per_ele.get_ptr_gpu(), detjac_upts.get_ptr_gpu(),
        J_dyn_upts.get_ptr_gpu(), JGinv_upts.get_ptr_gpu(),
        JGinv_dyn_upts.get_ptr_gpu(), d_nodal_s_basis_upts.get_ptr_gpu(),
        shape_dyn.get_ptr_gpu());
    ...
    for (i = 0; i < n_bdy_eles; i++) {
      for (int l = 0; l < n_inters_per_ele; l++) {
        for (j = 0; j < n_cubpts_per_inter(l); j++) {
          ...
          // calculate first derivatives of shape functions at the cubature
          // points
          ...
          if (n_dims == 2) {
            ...
            // store determinant of jacobian at cubature point. TODO: what is
            // this for?
            vol_detjac_inters_cubpts(l)(j, i) = xr * ys - xs * yr;

            // temporarily store transformed normal dot inverse of determinant
            // of jacobian multiplied by jacobian at the cubature point
            tnorm_dot_inv_detjac_mul_jac(0) =
                (tnorm_inters_cubpts(l)(0, j) * d_pos(1, 1)) -
                (tnorm_inters_cubpts(l)(1, j) * d_pos(1, 0));
            tnorm_dot_inv_detjac_mul_jac(1) =
                -(tnorm_inters_cubpts(l)(0, j) * d_pos(0, 1)) +
                (tnorm_inters_cubpts(l)(1, j) * d_pos(0, 0));

            // calculate interface area
            mag_tnorm = sqrt(tnorm_dot_inv_detjac_mul_jac(0) *
                                 tnorm_dot_inv_detjac_mul_jac(0) +
                             tnorm_dot_inv_detjac_mul_jac(1) *
                                 tnorm_dot_inv_detjac_mul_jac(1));

            // store normal at cubature point
            norm_inters_cubpts(l)(j, i, 0) =
                tnorm_dot_inv_detjac_mul_jac(0) / mag_tnorm;
            norm_inters_cubpts(l)(j, i, 1) =
                tnorm_dot_inv_detjac_mul_jac(1) / mag_tnorm;

            inter_detjac_inters_cubpts(l)(j, i) =
                compute_inter_detjac_inters_cubpts(l, d_pos);
          } else if (n_dims == 3) {
            xr = d_pos(0, 0);
            xs = d_pos(0, 1);
            xt = d_pos(0, 2);
            ...
            vol_detjac_inters_cubpts(l)(j, i) = xr * (ys * zt - yt * zs) -
                                                xs * (yr * zt - yt * zr) +
                                                xt * (yr * zs - ys * zr);
            ...
            // of jacobian multiplied by jacobian at the cubature point
            tnorm_dot_inv_detjac_mul_jac(0) =
                ((tnorm_inters_cubpts(l)(0, j) *
                  (d_pos(1, 1) * d_pos(2, 2) - d_pos(1, 2) * d_pos(2, 1))) +
                 (tnorm_inters_cubpts(l)(1, j) *
                  (d_pos(1, 2) * d_pos(2, 0) - d_pos(1, 0) * d_pos(2, 2))) +
                 (tnorm_inters_cubpts(l)(2, j) *
                  (d_pos(1, 0) * d_pos(2, 1) - d_pos(1, 1) * d_pos(2, 0))));
            tnorm_dot_inv_detjac_mul_jac(1) =
                ((tnorm_inters_cubpts(l)(0, j) *
                  (d_pos(0, 2) * d_pos(2, 1) - d_pos(0, 1) * d_pos(2, 2))) +
            ...
            norm_inters_cubpts(l)(j, i, 1) =
                tnorm_dot_inv_detjac_mul_jac(1) / mag_tnorm;
            norm_inters_cubpts(l)(j, i, 2) =
                tnorm_dot_inv_detjac_mul_jac(2) / mag_tnorm;
  ...
  if (n_eles != 0) {
    ...
    for (i = 0; i < n_eles; i++) {
      for (j = 0; j < n_cubpts_per_ele; j++) {
        ...
        for (m = 0; m < n_dims; m++) loc(m) = loc_volume_cubpts(m, j);

        calc_pos(loc, i, pos);
        calc_d_pos(loc, i, d_pos);

        if (n_dims == 2) {
          vol_detjac_vol_cubpts(j)(i) =
              d_pos(0, 0) * d_pos(1, 1) - d_pos(0, 1) * d_pos(1, 0);
        } else if (n_dims == 3) {
          vol_detjac_vol_cubpts(j)(i) =
              d_pos(0, 0) *
                  (d_pos(1, 1) * d_pos(2, 2) - d_pos(1, 2) * d_pos(2, 1)) -
              d_pos(0, 1) *
                  (d_pos(1, 0) * d_pos(2, 2) - d_pos(1, 2) * d_pos(2, 0)) +
              d_pos(0, 2) *
                  (d_pos(1, 0) * d_pos(2, 1) - d_pos(1, 1) * d_pos(2, 0));
        }
      }
...
double *eles::get_disu_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                                int in_field, int in_ele) {
  int i;
...
// get a pointer to the normal transformed continuous inviscid flux at a flux
// point

double *eles::get_norm_tconf_fpts_ptr(int in_inter_local_fpt,
                                      int in_ele_local_inter, int in_field,
                                      int in_ele) {
  int i;
...
double *eles::get_detjac_fpts_ptr(int in_inter_local_fpt,
                                  int in_ele_local_inter, int in_ele) {
  int i;
...
double *eles::get_detjac_dyn_fpts_ptr(int in_inter_local_fpt,
                                      int in_ele_local_inter, int in_ele) {
  int i;
...
// get a pointer to the magnitude of normal dot inverse of (determinant of
// jacobian multiplied by jacobian) at flux points

double *eles::get_tdA_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                               int in_ele) {
  int i;
...
// get pointer to the equivalent of 'dA' (face area) at a flux point in dynamic
// physical space */

double *eles::get_ndA_dyn_fpts_ptr(int in_inter_local_fpt,
                                   int in_ele_local_inter, int in_ele) {
  int i;
...
double *eles::get_norm_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                                int in_dim, int in_ele) {
  int i;
...
double *eles::get_norm_dyn_fpts_ptr(int in_inter_local_fpt,
                                    int in_ele_local_inter, int in_dim,
                                    int in_ele) {
  int i;
...
double *eles::get_loc_fpts_ptr_cpu(int in_inter_local_fpt,
                                   int in_ele_local_inter, int in_dim,
                                   int in_ele) {
  int i;
...
double *eles::get_loc_fpts_ptr_gpu(int in_inter_local_fpt,
                                   int in_ele_local_inter, int in_dim,
                                   int in_ele) {
  int i;
...
double *eles::get_pos_dyn_fpts_ptr_cpu(int in_inter_local_fpt,
                                       int in_ele_local_inter, int in_dim,
                                       int in_ele) {
  int i;
...
// get a pointer to delta of the transformed discontinuous solution at a flux
// point

double *eles::get_delta_disu_fpts_ptr(int in_inter_local_fpt,
                                      int in_ele_local_inter, int in_field,
                                      int in_ele) {
  int i;
...
double *eles::get_grad_disu_fpts_ptr(int in_inter_local_fpt,
                                     int in_ele_local_inter, int in_dim,
                                     int in_field, int in_ele) {
  int i;
...
double *eles::get_normal_disu_fpts_ptr(int in_inter_local_fpt,
                                       int in_ele_local_inter, int in_field,
                                       int in_ele, array<double> temp_loc,
                                       double temp_pos[3]) {
  array<double> pos(n_dims);
...
// get a pointer to the normal transformed continuous viscous flux at a flux
// point
/*
 double* eles::get_norm_tconvisf_fpts_ptr(int in_inter_local_fpt, int
 in_ele_local_inter, int in_field, int in_ele)
 {
 int i;
...
double *eles::get_grid_vel_fpts_ptr(int in_ele, int in_ele_local_inter,
                                    int in_inter_local_fpt, int in_dim) {
  int i;
...
double *eles::get_sgsf_fpts_ptr(int in_inter_local_fpt, int in_ele_local_inter,
                                int in_field, int in_dim, int in_ele) {
  int i;
...
void eles::calc_pos(array<double> in_loc, int in_ele, array<double> &out_pos) {
  ...
  for (i = 0; i < n_dims; i++) {
    ...
    for (j = 0; j < n_spts_per_ele(in_ele); j++) {
      out_pos(i) += eval_nodal_s_basis(j, in_loc, n_spts_per_ele(in_ele)) *
                    shape(i, j, in_ele);
    }
  }
}

/** find the position of a point within the element (r,s,t -> xd,yd,zd) (using
 * positions in dynamic grid) */
void eles::calc_pos_dyn(array<double> in_loc, int in_ele,
                        array<double> &out_pos) {
  int i, j;
  ...
  for (i = 0; i < n_dims; i++) {
    ...
    for (j = 0; j < n_spts_per_ele(in_ele); j++) {
      out_pos(i) += eval_nodal_s_basis(j, in_loc, n_spts_per_ele(in_ele)) *
                    shape_dyn(i, j, in_ele);
    }
  }
}

/** find the physical position of a flux point within the element (using
 * positions in dynamic grid) */
void eles::calc_pos_dyn_fpt(int in_fpt, int in_ele, array<double> &out_pos) {
  ...
  for (i = 0; i < n_dims; i++) {
    for (j = 0; j < n_spts_per_ele(in_ele); j++) {
      out_pos(i) +=
          nodal_s_basis_fpts(j, in_fpt, in_ele) * shape_dyn(i, j, in_ele);
    }
  }
}

/** find the physical position of a solution point within the element (using
 * positions in dynamic grid) */
void eles::calc_pos_dyn_upt(int in_upt, int in_ele, array<double> &out_pos) {
  ...
  for (i = 0; i < n_dims; i++) {
    for (j = 0; j < n_spts_per_ele(in_ele); j++) {
      out_pos(i) +=
          nodal_s_basis_fpts(j, in_upt, in_ele) * shape_dyn(i, j, in_ele);
    }
  }
}

/** find the physical position of a plot point within the element (using
 * positions in dynamic grid) */
void eles::calc_pos_dyn_ppt(int in_ppt, int in_ele, array<double> &out_pos) {
  ...
  for (i = 0; i < n_dims; i++) {
    for (j = 0; j < n_spts_per_ele(in_ele); j++) {
      out_pos(i) +=
          nodal_s_basis_ppts(j, in_ppt, in_ele) * shape_dyn(i, j, in_ele);
    }
  }
}

/** find the physical position of a volume cubature point within the element
 * (using positions in dynamic grid) */
void eles::calc_pos_dyn_vol_cubpt(int in_ppt, int in_ele,
                                  array<double> &out_pos) {
  int i, j;
  ...
  for (i = 0; i < n_dims; i++) {
    for (j = 0; j < n_spts_per_ele(in_ele); j++) {
      out_pos(i) +=
          nodal_s_basis_vol_cubpts(j, in_ppt, in_ele) * shape_dyn(i, j, in_ele);
    }
  }
}

/** find the physical position of a interface cubature point within the element
 * (using positions in dynamic grid) */
void eles::calc_pos_dyn_inters_cubpt(int in_cubpt, int in_face, int in_ele,
                                     array<double> &out_pos) {
  int i, j;
  ...
  for (i = 0; i < n_dims; i++) {
    for (j = 0; j < n_spts_per_ele(in_ele); j++) {
      out_pos(i) += nodal_s_basis_inters_cubpts(in_face)(j, in_cubpt, in_ele) *
                    shape_dyn(i, j, in_ele);
    }
  }
}
...
/** Calculate derivative of position wrt computational space (dx/dr, dx/ds,
 * etc.) */
void eles::calc_d_pos(array<double> in_loc, int in_ele,
                      array<double> &out_d_pos) {
  int i, j, k;
...
/**
 * Calculate derivative of static position wrt computational-space position at
 * upt Uses pre-computed nodal shape basis derivatives for efficiency \param[in]
 * in_upt - ID of solution point within element to evaluate at \param[in] in_ele
 * - local element ID \param[out] out_d_pos - array of size (n_dims,n_dims);
 * (i,j) = dx_i / dxi_j
 */
void eles::calc_d_pos_upt(int in_upt, int in_ele, array<double> &out_d_pos) {
  ...
  for (j = 0; j < n_dims; j++) {
    for (k = 0; k < n_dims; k++) {
      for (i = 0; i < n_spts_per_ele(in_ele); i++) {
        out_d_pos(j, k) +=
            d_nodal_s_basis_upts(k, i, in_upt, in_ele) * shape(j, i, in_ele);
...
/**
 * Calculate derivative of static position wrt computational-space position at
 * fpt Uses pre-computed nodal shape basis derivatives for efficiency \param[in]
 * in_fpt - ID of flux point within element to evaluate at \param[in] in_ele -
 * local element ID \param[out] out_d_pos - array of size (n_dims,n_dims); (i,j)
 * = dx_i / dxi_j
 */
void eles::calc_d_pos_fpt(int in_fpt, int in_ele, array<double> &out_d_pos) {
  ...
  for (j = 0; j < n_dims; j++) {
    for (k = 0; k < n_dims; k++) {
      for (i = 0; i < n_spts_per_ele(in_ele); i++) {
        out_d_pos(j, k) +=
            d_nodal_s_basis_fpts(k, i, in_fpt, in_ele) * shape(j, i, in_ele);
...
/**
 * Calculate derivative of dynamic position wrt reference (initial,static)
 * position \param[in] in_loc - position of point in computational space
 * \param[in] in_ele - local element ID
 * \param[out] out_d_pos - array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn(array<double> in_loc, int in_ele,
                          array<double> &out_d_pos) {
  int i, j, k;
...
/**
 * Calculate derivative of dynamic physical position wrt static/reference
 * physical position at fpt Uses pre-computed nodal shape basis derivatives for
 * efficiency \param[in] in_fpt - ID of flux point within element to evaluate at
 * \param[in] in_ele - local element ID
 * \param[out] out_d_pos - array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn_fpt(int in_fpt, int in_ele,
                              array<double> &out_d_pos) {
  if (run_input.motion == 4) {
    // Analytical formula for perturbed motion
    out_d_pos(0, 0) = 1 + 0.2 * pi *
                              cos(pi * pos_fpts(in_fpt, in_ele, 0) / 10) *
                              sin(pi * pos_fpts(in_fpt, in_ele, 1) / 10) *
                              sin(2 * pi * run_input.rk_time / 10);
    out_d_pos(0, 1) = 0.2 * pi * sin(pi * pos_fpts(in_fpt, in_ele, 0) / 10) *
                      cos(pi * pos_fpts(in_fpt, in_ele, 1) / 10) *
                      sin(2 * pi * run_input.rk_time / 10);
    out_d_pos(1, 0) = 0.2 * pi * cos(pi * pos_fpts(in_fpt, in_ele, 0) / 10) *
                      sin(pi * pos_fpts(in_fpt, in_ele, 1) / 10) *
                      sin(2 * pi * run_input.rk_time / 10);
    out_d_pos(1, 1) = 1 + 0.2 * pi *
                              sin(pi * pos_fpts(in_fpt, in_ele, 0) / 10) *
                              cos(pi * pos_fpts(in_fpt, in_ele, 1) / 10) *
                              sin(2 * pi * run_input.rk_time / 10);
  } else {
    // For all cases which do not have an analytical solution (i.e. for
    ...
    for (i = 0; i < n_dims; i++) {
      for (j = 0; j < n_dims; j++) {
        for (k = 0; k < n_spts_per_ele(in_ele); k++) {
          dxdr(i, j) += shape_dyn(i, k, in_ele) *
                        d_nodal_s_basis_fpts(j, k, in_fpt, in_ele);
          // dxdr(i,j) +=
          // shape_dyn(i,k,in_ele)*d_nodal_s_basis_fpts(in_fpt,in_ele,j,k);
        }
      }
    }
    ...
    for (i = 0; i < n_dims; i++) {
      for (j = 0; j < n_dims; j++) {
        for (k = 0; k < n_dims; k++) {
          out_d_pos(i, j) += dxdr(i, k) * JGinv_fpts(k, j, in_fpt, in_ele) /
                             detjac_fpts(in_fpt, in_ele);
        }
      }
    }
  }
...
/**
 * Calculate derivative of dynamic physical position wrt static/reference
 * physical position at upt Uses pre-computed nodal shape basis derivatives for
 * efficiency \param[in] in_upt - ID of solution point within element to
 * evaluate at \param[in] in_ele - local element ID \param[out] out_d_pos -
 * array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn_upt(int in_upt, int in_ele,
                              array<double> &out_d_pos) {
  if (run_input.motion == 4) {
    // Analytical formula for perturbed motion test case
    out_d_pos(0, 0) = 1 + 0.2 * pi *
                              cos(pi * pos_upts(in_upt, in_ele, 0) / 10) *
                              sin(pi * pos_upts(in_upt, in_ele, 1) / 10) *
                              sin(2 * pi * run_input.rk_time / 10);
    out_d_pos(0, 1) = 0.2 * pi * sin(pi * pos_upts(in_upt, in_ele, 0) / 10) *
                      cos(pi * pos_upts(in_upt, in_ele, 1) / 10) *
                      sin(2 * pi * run_input.rk_time / 10);
    out_d_pos(1, 0) = 0.2 * pi * cos(pi * pos_upts(in_upt, in_ele, 0) / 10) *
                      sin(pi * pos_upts(in_upt, in_ele, 1) / 10) *
                      sin(2 * pi * run_input.rk_time / 10);
    out_d_pos(1, 1) = 1 + 0.2 * pi *
                              sin(pi * pos_upts(in_upt, in_ele, 0) / 10) *
                              cos(pi * pos_upts(in_upt, in_ele, 1) / 10) *
                              sin(2 * pi * run_input.rk_time / 10);
  } else {
    // For all cases which do not have an analytical solution (i.e. for
    ...
    for (i = 0; i < n_dims; i++) {
      for (j = 0; j < n_dims; j++) {
        for (k = 0; k < n_spts_per_ele(in_ele); k++) {
          dxdr(i, j) += shape_dyn(i, k, in_ele) *
                        d_nodal_s_basis_upts(j, k, in_upt, in_ele);
          // dxdr(i,j) +=
          // shape_dyn(i,k,in_ele)*d_nodal_s_basis_upts(in_upt,in_ele,j,k);
        }
      }
    }
    ...
    for (i = 0; i < n_dims; i++) {
      for (j = 0; j < n_dims; j++) {
        for (k = 0; k < n_dims; k++) {
          out_d_pos(i, j) += dxdr(i, k) * JGinv_upts(k, j, in_upt, in_ele) /
                             detjac_upts(in_upt, in_ele);
        }
      }
    }
  }
...
/**
 * Calculate derivative of dynamic physical position wrt static/reference
 * physical position at volume cubature point Uses pre-computed nodal shape
 * basis derivatives for efficiency \param[in] in_cubpt - ID of volume cubature
 * point within element to evaluate at \param[in] in_ele - local element ID
 * \param[out] out_d_pos - array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn_vol_cubpt(int in_cubpt, int in_ele,
                                    array<double> &out_d_pos) {
  int i, j, k;
  ...
  for (i = 0; i < n_dims; i++) {
    for (j = 0; j < n_dims; j++) {
      for (k = 0; k < n_spts_per_ele(in_ele); k++) {
        dxdr(i, j) += d_nodal_s_basis_vol_cubpts(j, k, in_cubpt, in_ele) *
                      shape_dyn(i, k, in_ele);
      }
    }
  }
...
/**
 * Calculate derivative of dynamic physical position wrt static/reference
 * physical position at interface cubature point Uses pre-computed nodal shape
 * basis derivatives for efficiency \param[in] in_cubpt - ID of interface
 * cubature point within element to evaluate at \param[in] in_face - Local ID of
 * face within element \param[in] in_ele - local element ID \param[out]
 * out_d_pos - array of size (n_dims,n_dims); (i,j) = dx_i / dX_j
 */
void eles::calc_d_pos_dyn_inters_cubpt(int in_cubpt, int in_face, int in_ele,
                                       array<double> &out_d_pos) {
  int i, j, k;
  ...
  for (i = 0; i < n_dims; i++) {
    for (j = 0; j < n_dims; j++) {
      for (k = 0; k < n_spts_per_ele(in_ele); k++) {
        dxdr(i, j) +=
            d_nodal_s_basis_inters_cubpts(in_face)(j, k, in_cubpt, in_ele) *
            shape_dyn(i, k, in_ele);
      }
    }
  }
  ...
  for (i = 0; i < n_dims; i++) {
    for (j = 0; j < n_dims; j++) {
      for (k = 0; k < n_dims; k++) {
        // out_d_pos(i,j) +=
        // dxdr(i,k)/jac_inters_cubpts(in_face)(in_cubpt,in_ele,j,k);
      }
    }
  }
}
  ...
  for (i = 0; i < n_eles; i++) {
    ...
    for (j = 0; j < n_upts_per_ele; j++) {
      if (in_norm_type == 0) {
        cell_sum =
            std::max(cell_sum,
                     abs(div_tconf_upts(0)(j, i, in_field) / detjac_upts(j, i) -
                         run_input.const_src - src_upts(j, i, in_field)));
      }
      if (in_norm_type == 1) {
        cell_sum += abs(div_tconf_upts(0)(j, i, in_field) / detjac_upts(j, i) -
                        run_input.const_src - src_upts(j, i, in_field));
      } else if (in_norm_type == 2) {
        cell_sum += (div_tconf_upts(0)(j, i, in_field) / detjac_upts(j, i) -
                     run_input.const_src - src_upts(j, i, in_field)) *
                    (div_tconf_upts(0)(j, i, in_field) / detjac_upts(j, i) -
                     run_input.const_src - src_upts(j, i, in_field));
      }
    }
    if (in_norm_type == 0)
      sum = std::max(cell_sum, sum);
    else
      sum += cell_sum;
  }
...
array<double> eles::compute_error(int in_norm_type, double &time) {
  ...
  for (int i = 0; i < n_eles; i++) {
    for (int j = 0; j < n_cubpts_per_ele; j++) {
      ...
      error = get_pointwise_error(disu_cubpt, grad_disu_cubpt, pos, time,
                                  in_norm_type);
...
array<double> eles::get_pointwise_error(array<double> &sol,
                                        array<double> &grad_sol,
                                        array<double> &loc, double &time,
                                        int in_norm_type) {
  ...
  if (run_input.test_case == 1)  // Isentropic vortex
  {
    ...
    error_sol(3) =
        sol(3) - (p / (run_input.gamma - 1) + 0.5 * rho * (vx * vx + vy * vy));
  } else if (run_input.test_case == 2)  // Sine Wave (single)
  {
    ...
    if (viscous) {
      eval_sine_wave_single(loc, run_input.wave_speed, run_input.diff_coeff,
                            time, rho, grad_rho, n_dims);
    } else {
      eval_sine_wave_single(loc, run_input.wave_speed, 0., time, rho, grad_rho,
                            n_dims);
    }
  ...
  } else if (run_input.test_case == 3)  // Sine Wave (group)
  {
    ...
    if (viscous) {
      eval_sine_wave_group(loc, run_input.wave_speed, run_input.diff_coeff,
                           time, rho, grad_rho, n_dims);
    } else {
      eval_sine_wave_group(loc, run_input.wave_speed, 0., time, rho, grad_rho,
                           n_dims);
    }
  ...
  } else if (run_input.test_case == 5)  // Couette flow
  {
    ...
    eval_couette_flow(loc, run_input.gamma, run_input.R_ref, u_wall,
                      run_input.T_wall, run_input.p_bound, run_input.prandtl,
                      time, ene, grad_ene, n_dims);
...
// Calculate body forcing term for periodic channel flow. HARDCODED FOR THE
// CHANNEL AND PERIODIC HILL!
...
void eles::evaluate_body_force(int in_file_num) {
  ...
  if (n_eles != 0) {
        ...
        if (inflowinters(i, l) == 1) {
          for (j = 0; j < n_cubpts_per_inter(l); j++) {
            ...
            for (k = 0; k < n_upts_per_ele; k++) {
              for (m = 0; m < 4; m++) {
                disu_cubpt(m) +=
                    opp_inters_cubpts(l)(j, k) * disu_upts(0)(k, ele, m);
              }
            }
            for (m = 0; m < 4; m++) {
    ...
    for (m = 0; m < 4; m++) {
      integral_global(m) = 0.;
      MPI_Allreduce(&integral(m), &integral_global(m), 1, MPI_DOUBLE, MPI_SUM,
                    MPI_COMM_WORLD);
    ...
    if (run_input.restart_flag == 0 and in_file_num == 0)
      mdot_old = mdot0;
    else if (run_input.restart_flag == 1 and
             in_file_num == run_input.restart_iter)
      mdot_old = mdot0;
    else
      mdot_old = mass_flux;
    ...
    if (run_input.dt_type == 0)
      ...
      FatalError(
          "Not sure what value of timestep to use in body force term when "
    ...
    body_force(0) = 0.;
    // body_force(1) = alpha/area/dt*(mdot0 - mass_flux); // modified SD3D
    // version
    body_force(1) =
        1.0 / area / dt *
        (mdot0 - 2.0 * mass_flux + mdot_old);  // HIOCFD C3.4 version
    body_force(2) = 0.;
    body_force(3) = 0.;
    body_force(4) = body_force(1) * ubulk;  // energy forcing

    if (rank == 0)
      std::cout << "iter, mdot0, mdot_old, mass_flux, body_force(1): "
                << in_file_num << ", " << std::setprecision(8) << mdot0 << ", "
                << mdot_old << ", " << mass_flux << ", " << body_force(1)
                << std::endl;
    ...
    for (i = 0; i < n_eles; i++)
      for (j = 0; j < n_upts_per_ele; j++)
        for (k = 0; k < n_fields; k++) src_upts(j, i, k) += body_force(k);

#endif

#ifdef _GPU
    body_force.cp_cpu_gpu();
    evaluate_body_force_gpu_kernel_wrapper(n_upts_per_ele, n_dims, n_fields,
                                           n_eles, src_upts.get_ptr_gpu(),
                                           body_force.get_ptr_gpu());
#endif
  }
}

// Compute integral quantities
void eles::CalcIntegralQuantities(int n_integral_quantities,
    ...
    for (int j = 0; j < n_cubpts_per_ele; j++) {
      ...
      for (int m = 0; m < n_fields; m++) {
        for (int n = 0; n < n_dims; n++) {
          ...
          for (int k = 0; k < n_upts_per_ele; k++) {
            grad_disu_cubpt(m, n) +=
                opp_volume_cubpts(j, k) * grad_disu_upts(k, i, m, n);
          }
        }
      }
      irho = 1. / disu_cubpt(0);
      dudx = irho * (grad_disu_cubpt(1, 0) -
                     disu_cubpt(1) * irho * grad_disu_cubpt(0, 0));
      dudy = irho * (grad_disu_cubpt(1, 1) -
                     disu_cubpt(1) * irho * grad_disu_cubpt(0, 1));
      dvdx = irho * (grad_disu_cubpt(2, 0) -
                     disu_cubpt(2) * irho * grad_disu_cubpt(0, 0));
      dvdy = irho * (grad_disu_cubpt(2, 1) -
                     disu_cubpt(2) * irho * grad_disu_cubpt(0, 1));

      if (n_dims == 3) {
        dudz = irho * (grad_disu_cubpt(1, 2) -
                       disu_cubpt(1) * irho * grad_disu_cubpt(0, 2));
        dvdz = irho * (grad_disu_cubpt(2, 2) -
                       disu_cubpt(2) * irho * grad_disu_cubpt(0, 2));
        dwdx = irho * (grad_disu_cubpt(3, 0) -
                       disu_cubpt(3) * irho * grad_disu_cubpt(0, 0));
        dwdy = irho * (grad_disu_cubpt(3, 1) -
                       disu_cubpt(3) * irho * grad_disu_cubpt(0, 1));
        dwdz = irho * (grad_disu_cubpt(3, 2) -
                       disu_cubpt(3) * irho * grad_disu_cubpt(0, 2));
      }
      ...
      for (int m = 0; m < n_integral_quantities; ++m) {
        ...
        if (run_input.integral_quantities(m) == "kineticenergy") {
          ...
          // Compute pressure
          pressure =
              (run_input.gamma - 1.0) * (disu_cubpt(n_fields - 1) - irho * tke);
        ...
        } else if (run_input.integral_quantities(m) == "straincolonproduct" ||
                   run_input.integral_quantities(m) ==
                       "devstraincolonproduct") {
          ...
          if (run_input.integral_quantities(m) == "devstraincolonproduct") {
            for (int i = 0; i < n_dims; i++) S(i, i) -= diag;
          }

          for (int i = 0; i < n_dims; i++)
            for (int j = 0; j < n_dims; j++) diagnostic += S(i, j) * S(i, j);
    ...
    for (k = 0; k < n_eles; k++) {
      for (i = 0; i < n_average_fields; ++i) {
        ...
        if (run_input.dt_type == 0)
          ...
          FatalError(
              "Not sure what value of timestep to use in time average "
              "calculation when using local timestepping.");
...
void eles::compute_wall_forces(array<double> &inv_force,
                               array<double> &vis_force, double &temp_cl,
                               double &temp_cd, std::ofstream &coeff_file,
                               bool write_forces) {
  ...
  // one over the dynamic pressure - factor for computing friction coeff,
  // pressure coeff, forces
  factor = 1.0 / (0.5 * run_input.rho_c_ic *
                  (run_input.u_c_ic * run_input.u_c_ic +
                   run_input.v_c_ic * run_input.v_c_ic +
                   run_input.w_c_ic * run_input.w_c_ic));

  // Add a header to the force file
  if (write_forces) {
    coeff_file << std::setw(18) << "x" << std::setw(18) << "Cp" << std::setw(18)
               << "Cf" << std::endl;
  }
  ...
  for (int i = 0; i < n_bdy_eles; i++) {
    ...
    for (int l = 0; l < n_inters_per_ele; l++) {
      if (bctype(ele, l) == 7 || bctype(ele, l) == 11 || bctype(ele, l) == 12 ||
          bctype(ele, l) == 16) {
        ...
        for (int j = n_cubpts_per_inter(l) - 1; j >= 0; j--) {
          ...
          for (int m = 0; m < n_dims; m++) loc(m) = loc_inters_cubpts(l)(m, j);
          ...
          if (viscous == 1) {
            for (int m = 0; m < n_fields; m++) {
              for (int n = 0; n < n_dims; n++) {
                ...
                for (int k = 0; k < n_upts_per_ele; k++) {
                  value +=
                      opp_inters_cubpts(l)(j, k) * grad_disu_upts(k, ele, m, n);
                }
                grad_u_l(m, n) = value;
              }
            }
          ...
          if (bctype(ele, l) != 16) {
            v_sq = 0.;
            for (int m = 0; m < n_dims; m++) v_sq += (u_l(m + 1) * u_l(m + 1));
            p_l = (gamma - 1.0) * (u_l(n_dims + 1) - 0.5 * v_sq / u_l(0));
          } else {
            // Dual consistent approach
            vn_l = 0.;
            for (int m = 0; m < n_dims; m++) vn_l += u_l(m + 1) * norm(m);
            vn_l /= u_l(0);
          ...
          for (int m = 0; m < n_dims; m++) {
            Finv(m) =
                wgt * (p_l - run_input.p_c_ic) * norm(m) * detjac * factor;
          }
          ...
          if (n_dims == 2) {
            ...
            cl = -Finv(0) * sin(aoa) + Finv(1) * cos(aoa);
            cd = Finv(0) * cos(aoa) * cos(aos) + Finv(1) * sin(aoa) +
                 Finv(2) * sin(aoa) * cos(aos);
          }

          // write to file
          if (write_forces) {
            coeff_file << std::scientific << std::setw(18)
                       << std::setprecision(12) << pos(0) << " "
                       << std::setw(18) << std::setprecision(12) << cp;
          }
          ...
          if (viscous == 1) {
            ...
            for (int m = 0; m < n_dims; m++) {
              ...
              for (int n = 0; n < n_dims; n++) {
                dv(n, m) =
                    1.0 / u_l(0) * (grad_u_l(n + 1, m) - drho(m) * u_l(n + 1));
                ;
              }
              de(m) = 1.0 / u_l(0) *
                      (grad_u_l(n_dims + 1, m) - drho(m) * u_l(n_dims + 1));
              ;
            }
            ...
            // get viscosity
            rt_ratio = (run_input.gamma - 1.0) * inte / (run_input.rt_inf);
            mu = (run_input.mu_inf) * pow(rt_ratio, 1.5) *
                 (1 + (run_input.c_sth)) / (rt_ratio + (run_input.c_sth));
            mu = mu + run_input.fix_vis * (run_input.mu_inf - mu);

            // Compute the coefficient of friction and wall shear stress
            if (n_dims == 2) {
              // stresses w.r.t. normal
              taun(0) = mu * (2. * (dv(0, 0) - diag) * norm(0) +
                              (dv(0, 1) + dv(1, 0)) * norm(1));
              taun(1) = mu * (2. * (dv(1, 1) - diag) * norm(1) +
                              (dv(0, 1) + dv(1, 0)) * norm(0));
              ...
              if (write_forces) {
                coeff_file << " " << std::setw(18) << std::setprecision(12)
                           << cf;
              }
            ...
            if (n_dims == 3) {
              ...
              taun(0) = mu * (2. * (dv(0, 0) - diag) * norm(0) +
                              (dv(0, 1) + dv(1, 0)) * norm(1) +
                              (dv(0, 2) + dv(2, 0)) * norm(2));
              taun(1) = mu * (2. * (dv(1, 1) - diag) * norm(1) +
                              (dv(0, 1) + dv(1, 0)) * norm(0) +
                              (dv(1, 2) + dv(2, 1)) * norm(2));
              taun(2) = mu * (2. * (dv(2, 2) - diag) * norm(2) +
                              (dv(0, 2) + dv(2, 0)) * norm(0) +
                              (dv(1, 2) + dv(2, 1)) * norm(1));

              // take dot product with normal
              taundotn =
                  taun(0) * norm(0) + taun(1) * norm(1) + taun(2) * norm(2);
              ...
              cl += -Fvis(0) * sin(aoa) + Fvis(1) * cos(aoa);
              cd += Fvis(0) * cos(aoa) * cos(aos) + Fvis(1) * sin(aoa) +
                    Fvis(2) * sin(aoa) * cos(aos);
            }
          }  // End of if viscous
...
void eles::set_grid_vel_spt(int in_ele, int in_spt, array<double> in_vel) {
  for (int i = 0; i < n_dims; i++) vel_spts(i, in_spt, in_ele) = in_vel(i);
}
  ...
  for (ic = 0; ic < n_eles; ic++) {
    for (fpt = 0; fpt < n_fpts_per_ele; fpt++) {
      ...
      for (j = 0; j < n_spts_per_ele(ic); j++) {
        nodal_s_basis_fpts(j, fpt, ic) =
            eval_nodal_s_basis(j, loc, n_spts_per_ele(ic));
      }
    }
  }
#ifdef _GPU
...
void eles::store_nodal_s_basis_vol_cubpts(void) {
  ...
  for (ic = 0; ic < n_eles; ic++) {
    for (cubpt = 0; cubpt < n_cubpts_per_ele; cubpt++) {
      ...
      for (j = 0; j < n_spts_per_ele(ic); j++) {
        nodal_s_basis_vol_cubpts(j, cubpt, ic) =
            eval_nodal_s_basis(j, loc, n_spts_per_ele(ic));
      }
    }
  }
  ...
  for (ic = 0; ic < n_eles; ic++) {
    for (iface = 0; iface < n_inters_per_ele; iface++) {
      for (cubpt = 0; cubpt < n_cubpts_per_inter(iface); cubpt++) {
        ...
        for (j = 0; j < n_spts_per_ele(ic); j++) {
          nodal_s_basis_inters_cubpts(iface)(j, cubpt, ic) =
              eval_nodal_s_basis(j, loc, n_spts_per_ele(ic));
        }
      }
    }
  ...
  for (ic = 0; ic < n_eles; ic++) {
    for (iface = 0; iface < n_inters_per_ele; iface++) {
      for (cubpt = 0; cubpt < n_cubpts_per_inter(iface); cubpt++) {
        ...
        for (j = 0; j < n_spts_per_ele(ic); j++) {
          for (k = 0; k < n_dims; k++) {
            d_nodal_s_basis_inters_cubpts(iface)(k, j, cubpt, ic) =
                d_nodal_basis(j, k);
          }
        }
      }
...
void eles::initialize_grid_vel(int in_max_n_spts_per_ele) {
  if (motion) {
    ...
    /// ALSO: *_nodal_s_basis data is redundant: same for every element (of same
    /// type)
  }
}
...
void eles::set_grid_vel_fpts(int in_rk_step) {
  ...
  //    for (ic=0; ic<n_eles; ic++) {
  //      for (fpt=0; fpt<n_fpts_per_ele; fpt++) {
  //        grid_vel_fpts(fpt,ic,0) =
  //        4*pi/10*sin(pi*pos_fpts(fpt,ic,0)/10)*sin(pi*pos_fpts(fpt,ic,1)/10)*cos(2*pi*rk_time/10);
  //        grid_vel_fpts(fpt,ic,1) =
  //        4*pi/10*sin(pi*pos_fpts(fpt,ic,0)/10)*sin(pi*pos_fpts(fpt,ic,1)/10)*cos(2*pi*rk_time/10);
  //      }
  //    }
  //  }
  ...
  for (ic = 0; ic < n_eles; ic++) {
    for (fpt = 0; fpt < n_fpts_per_ele; fpt++) {
      for (k = 0; k < n_dims; k++) {
        ...
        for (j = 0; j < n_spts_per_ele(ic); j++) {
          grid_vel_fpts(fpt, ic, k) +=
              nodal_s_basis_fpts(j, fpt, ic) * vel_spts(k, j, ic);
        }
      }
    }
  }
//  }
  ...
  eval_grid_vel_pts_kernel_wrapper(
      n_dims, n_eles, n_fpts_per_ele, max_n_spts_per_ele,
      n_spts_per_ele.get_ptr_gpu(), nodal_s_basis_fpts.get_ptr_gpu(),
      vel_spts.get_ptr_gpu(), grid_vel_fpts.get_ptr_gpu());
#endif
}
...
void eles::set_grid_vel_upts(int in_rk_step) {
  ...
  //    for (ic=0; ic<n_eles; ic++) {
  //      for (upt=0; upt<n_upts_per_ele; upt++) {
  //        grid_vel_upts(upt,ic,0) =
  //        4*pi/10*sin(pi*pos_upts(upt,ic,0)/10)*sin(pi*pos_upts(upt,ic,1)/10)*cos(2*pi*rk_time/10);
  //        grid_vel_upts(upt,ic,1) =
  //        4*pi/10*sin(pi*pos_upts(upt,ic,0)/10)*sin(pi*pos_upts(upt,ic,1)/10)*cos(2*pi*rk_time/10);
  //      }
  //    }
  //  }
  ...
  for (ic = 0; ic < n_eles; ic++) {
    for (upt = 0; upt < n_upts_per_ele; upt++) {
      for (k = 0; k < n_dims; k++) {
        ...
        for (j = 0; j < n_spts_per_ele(ic); j++) {
          grid_vel_upts(upt, ic, k) +=
              nodal_s_basis_upts(j, upt, ic) * vel_spts(k, j, ic);
        }
      }
    }
  }
//  }
  ...
  // grid_vel_upts.cp_cpu_gpu();
  eval_grid_vel_pts_kernel_wrapper(
      n_dims, n_eles, n_upts_per_ele, max_n_spts_per_ele,
      n_spts_per_ele.get_ptr_gpu(), nodal_s_basis_upts.get_ptr_gpu(),
      vel_spts.get_ptr_gpu(), grid_vel_upts.get_ptr_gpu());
#endif
}
  ...
  for (ic = 0; ic < n_eles; ic++) {
    for (ppt = 0; ppt < n_ppts_per_ele; ppt++) {
      for (k = 0; k < n_dims; k++) {
        ...
        for (j = 0; j < n_spts_per_ele(ic); j++) {
          vel_ppts(k, ppt, ic) +=
              nodal_s_basis_ppts(j, ppt, ic) * vel_spts(k, j, ic);
        }
      }
    }
  }
...
array<double> eles::get_grid_vel_ppts(void) { return vel_ppts; }

#ifdef _GPU
void eles::rigid_move(double rk_time) {
  if (n_eles != 0) {
    rigid_motion_kernel_wrapper(
        n_dims, n_eles, max_n_spts_per_ele, n_spts_per_ele.get_ptr_gpu(),
        shape.get_ptr_gpu(), shape_dyn.get_ptr_gpu(),
        run_input.bound_vel_simple(0).get_ptr_gpu(), rk_time);
  }
}

void eles::perturb_shape(double rk_time) {
  if (n_eles != 0) {
    push_back_shape_dyn_kernel_wrapper(n_dims, n_eles, max_n_spts_per_ele, 5,
                                       n_spts_per_ele.get_ptr_gpu(),
                                       shape_dyn.get_ptr_gpu());
    perturb_shape_kernel_wrapper(
        n_dims, n_eles, max_n_spts_per_ele, n_spts_per_ele.get_ptr_gpu(),
        shape.get_ptr_gpu(), shape_dyn.get_ptr_gpu(), rk_time);
  }
}

void eles::rigid_grid_velocity(double rk_time) {
  if (n_eles != 0) {
    calc_rigid_grid_vel_spts_kernel_wrapper(
        n_dims, n_eles, max_n_spts_per_ele, n_spts_per_ele.get_ptr_gpu(),
        run_input.bound_vel_simple(0).get_ptr_gpu(), vel_spts.get_ptr_gpu(),
        rk_time);
    eval_grid_vel_pts_kernel_wrapper(
        n_dims, n_eles, n_upts_per_ele, max_n_spts_per_ele,
        n_spts_per_ele.get_ptr_gpu(), nodal_s_basis_upts.get_ptr_gpu(),
...
void eles::perturb_grid_velocity(double rk_time) {
  if (n_eles != 0) {
    calc_perturb_grid_vel_spts_kernel_wrapper(
        n_dims, n_eles, max_n_spts_per_ele, n_spts_per_ele.get_ptr_gpu(),
        shape.get_ptr_gpu(), vel_spts.get_ptr_gpu(), rk_time);
    eval_grid_vel_pts_kernel_wrapper(
        n_dims, n_eles, n_upts_per_ele, max_n_spts_per_ele,
        n_spts_per_ele.get_ptr_gpu(), nodal_s_basis_upts.get_ptr_gpu(),
        vel_spts.get_ptr_gpu(), grid_vel_upts.get_ptr_gpu());
...
void eles::calc_grid_velocity(void) {
  if (n_eles != 0) {
    calc_grid_vel_spts_kernel_wrapper(
        n_dims, n_eles, max_n_spts_per_ele, n_spts_per_ele.get_ptr_gpu(),
        shape_dyn.get_ptr_gpu(), vel_spts.get_ptr_gpu(), run_input.dt);
    eval_grid_vel_pts_kernel_wrapper(
        n_dims, n_eles, n_upts_per_ele, max_n_spts_per_ele,
        n_spts_per_ele.get_ptr_gpu(), nodal_s_basis_upts.get_ptr_gpu(),
        vel_spts.get_ptr_gpu(), grid_vel_upts.get_ptr_gpu());
