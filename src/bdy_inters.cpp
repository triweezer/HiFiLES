...
void bdy_inters::set_boundary(int in_inter, int bdy_type, int in_ele_type_l,
                              int in_ele_l, int in_local_inter_l,
                              struct solution *FlowSol) {
  boundary_type(in_inter) = bdy_type;

  for (int i = 0; i < n_fields; i++) {
    for (int j = 0; j < n_fpts_per_inter; j++) {
      disu_fpts_l(j, in_inter, i) = get_disu_fpts_ptr(
          in_ele_type_l, in_ele_l, i, in_local_inter_l, j, FlowSol);

      norm_tconf_fpts_l(j, in_inter, i) = get_norm_tconf_fpts_ptr(
          in_ele_type_l, in_ele_l, i, in_local_inter_l, j, FlowSol);

      if (viscous) {
        delta_disu_fpts_l(j, in_inter, i) = get_delta_disu_fpts_ptr(
            in_ele_type_l, in_ele_l, i, in_local_inter_l, j, FlowSol);
      }
    }
  }
  ...
  for (int i = 0; i < n_fields; i++) {
    for (int j = 0; j < n_fpts_per_inter; j++) {
      for (int k = 0; k < n_dims; k++) {
        if (viscous) {
          grad_disu_fpts_l(j, in_inter, i, k) = get_grad_disu_fpts_ptr(
              in_ele_type_l, in_ele_l, in_local_inter_l, i, k, j, FlowSol);
        }

        // Subgrid-scale flux
        if (LES) {
          sgsf_fpts_l(j, in_inter, i, k) = get_sgsf_fpts_ptr(
              in_ele_type_l, in_ele_l, in_local_inter_l, i, k, j, FlowSol);
        }
      }
    }
  ...
  for (int j = 0; j < n_fpts_per_inter; j++) {
    tdA_fpts_l(j, in_inter) =
        get_tdA_fpts_ptr(in_ele_type_l, in_ele_l, in_local_inter_l, j, FlowSol);

    if (motion) {
      ndA_dyn_fpts_l(j, in_inter) = get_ndA_dyn_fpts_ptr(
          in_ele_type_l, in_ele_l, in_local_inter_l, j, FlowSol);
      J_dyn_fpts_l(j, in_inter) = get_detjac_dyn_fpts_ptr(
          in_ele_type_l, in_ele_l, in_local_inter_l, j, FlowSol);
      // if (run_input.GCL)
    ...
    for (int k = 0; k < n_dims; k++) {
      norm_fpts(j, in_inter, k) = get_norm_fpts_ptr(
          in_ele_type_l, in_ele_l, in_local_inter_l, j, k, FlowSol);

      if (motion) {
        norm_dyn_fpts(j, in_inter, k) = get_norm_dyn_fpts_ptr(
            in_ele_type_l, in_ele_l, in_local_inter_l, j, k, FlowSol);
        grid_vel_fpts(j, in_inter, k) = get_grid_vel_fpts_ptr(
            in_ele_type_l, in_ele_l, in_local_inter_l, j, k, FlowSol);
        pos_dyn_fpts(j, in_inter, k) = get_pos_dyn_fpts_ptr_cpu(
            in_ele_type_l, in_ele_l, in_local_inter_l, j, k, FlowSol);
      }

#ifdef _CPU
      pos_fpts(j, in_inter, k) = get_loc_fpts_ptr_cpu(
          in_ele_type_l, in_ele_l, in_local_inter_l, j, k, FlowSol);
#endif
#ifdef _GPU
      pos_fpts(j, in_inter, k) = get_loc_fpts_ptr_gpu(
          in_ele_type_l, in_ele_l, in_local_inter_l, j, k, FlowSol);
#endif
    }
  }
  ...
  //        for (int k=0;k<n_dims;k++)
  //          temp_loc(k) =
  //          *get_loc_fpts_ptr_cpu(in_ele_type_l,in_ele_l,in_local_inter_l,j,k,FlowSol);
  ...
  //        if(viscous) {
  //          for(int i=0;i<n_fields;i++)
  //            normal_disu_fpts_l(j,in_inter,i) =
  //            get_normal_disu_fpts_ptr(in_ele_type_l,in_ele_l,in_local_inter_l,i,j,FlowSol,
  //            temp_loc, temp_pos);
...
/*! Calculate normal transformed continuous inviscid flux at the flux points on
 * the boundaries.*/
...
void bdy_inters::evaluate_boundaryConditions_invFlux(double time_bound) {
  ...
  for (int i = 0; i < n_inters; i++) {
    for (int j = 0; j < n_fpts_per_inter; j++) {
      ...
      if (motion) {
        for (int m = 0; m < n_dims; m++) norm(m) = *norm_dyn_fpts(j, i, m);
      } else {
        for (int m = 0; m < n_dims; m++) norm(m) = *norm_fpts(j, i, m);
      }

      /*! calculate discontinuous solution at flux points */
      for (int k = 0; k < n_fields; k++) temp_u_l(k) = (*disu_fpts_l(j, i, k));
      ...
      if (motion) {
        ...
        for (int m = 0; m < n_dims; m++) temp_loc(m) = *pos_dyn_fpts(j, i, m);
      } else {
        // Get static-physical flux point location
        for (int m = 0; m < n_dims; m++) temp_loc(m) = *pos_fpts(j, i, m);

        temp_v.initialize_to_zero();
      }

      set_inv_boundary_conditions(
          boundary_type(i), temp_u_l.get_ptr_cpu(), temp_u_r.get_ptr_cpu(),
          temp_v.get_ptr_cpu(), norm.get_ptr_cpu(), temp_loc.get_ptr_cpu(),
          bdy_params.get_ptr_cpu(), n_dims, n_fields, run_input.gamma,
          run_input.R_ref, time_bound, run_input.equation);
      ...
      } else  // Call Riemann solver
      {
        ...
        if (run_input.riemann_solve_type == 0) {  // Rusanov
          convective_flux_boundary(temp_f_l, temp_f_r, norm, fn, n_dims,
                                   n_fields);
        } else if (run_input.riemann_solve_type == 1) {  // Lax-Friedrich
          lax_friedrich(temp_u_l, temp_u_r, norm, fn, n_dims, n_fields,
                        run_input.lambda, run_input.wave_speed);
        } else if (run_input.riemann_solve_type == 2) {  // ROE
          roe_flux(temp_u_l, temp_u_r, temp_v, norm, fn, n_dims, n_fields,
                   run_input.gamma);
        } else
      ...
      if (motion) {
        for (int k = 0; k < n_fields; k++) {
          (*norm_tconf_fpts_l(j, i, k)) =
              fn(k) * (*ndA_dyn_fpts_l(j, i)) * (*tdA_fpts_l(j, i));
        }
      } else {
        for (int k = 0; k < n_fields; k++) {
      ...
      if (viscous) {
        ...
        if (run_input.vis_riemann_solve_type == 0)
          ldg_solution(flux_spec, temp_u_l, temp_u_r, u_c, run_input.pen_fact,
                       norm);
        else
        ...
        if (motion) {
          ...
          for (int k = 0; k < n_fields; k++) {
            *delta_disu_fpts_l(j, i, k) =
                (u_c(k) - temp_u_l(k)) * (*J_dyn_fpts_l(j, i));
          }
        } else {
          for (int k = 0; k < n_fields; k++) {
...
#ifdef _GPU
  if (n_inters != 0)
    evaluate_boundaryConditions_invFlux_gpu_kernel_wrapper(
        n_fpts_per_inter, n_dims, n_fields, n_inters, disu_fpts_l.get_ptr_gpu(),
        norm_tconf_fpts_l.get_ptr_gpu(), tdA_fpts_l.get_ptr_gpu(),
        ndA_dyn_fpts_l.get_ptr_gpu(), J_dyn_fpts_l.get_ptr_gpu(),
        norm_fpts.get_ptr_gpu(), norm_dyn_fpts.get_ptr_gpu(),
        pos_fpts.get_ptr_gpu(), pos_dyn_fpts.get_ptr_gpu(),
        grid_vel_fpts.get_ptr_gpu(), boundary_type.get_ptr_gpu(),
        bdy_params.get_ptr_gpu(), run_input.riemann_solve_type,
        delta_disu_fpts_l.get_ptr_gpu(), run_input.gamma, run_input.R_ref,
        viscous, motion, run_input.vis_riemann_solve_type, time_bound,
        run_input.wave_speed(0), run_input.wave_speed(1),
        run_input.wave_speed(2), run_input.lambda, run_input.equation,
        run_input.turb_model);
#endif
}

void bdy_inters::set_inv_boundary_conditions(
    int bdy_type, double *u_l, double *u_r, double *v_g, double *norm,
    double *loc, double *bdy_params, int n_dims, int n_fields, double gamma,
    double R_ref, double time_bound, int equation) {
  double rho_l, rho_r;
  ...
  if (equation == 0) {
    ...
    rho_l = u_l[0];
    for (int i = 0; i < n_dims; i++) v_l[i] = u_l[i + 1] / u_l[0];
    e_l = u_l[n_dims + 1];

    // Compute pressure on left side
    v_sq = 0.;
    for (int i = 0; i < n_dims; i++) v_sq += (v_l[i] * v_l[i]);
    ...
    if (bdy_type == 1) {
      ...
      rho_r = rho_bound;
      for (int i = 0; i < n_dims; i++) v_r[i] = v_bound[i];
      ...
      // compute energy
      v_sq = 0.;
      for (int i = 0; i < n_dims; i++) v_sq += (v_r[i] * v_r[i]);
    ...
    // Subsonic outflow simple (fixed pressure) //CONSIDER DELETING
      ...
      rho_r = rho_l;
      for (int i = 0; i < n_dims; i++) v_r[i] = v_l[i];
    ...
    // SU2.
      ...
      vn_l = 0.;
      for (int i = 0; i < n_dims; i++) vn_l += v_l[i] * norm[i];
      ...
      for (int i = 0; i < n_dims; i++) v_sq += v_l[i] * v_l[i];
      c_total_sq =
          (gamma - 1.0) * (h_total - (e_l / rho_l + p_l / rho_l) + 0.5 * v_sq) +
          c_l * c_l;

      // Dot product of normal flow velocity
      alpha = 0.;
      for (int i = 0; i < n_dims; i++) alpha += norm[i] * n_free_stream[i];

      // Coefficients of quadratic equation
      aa = 1.0 + 0.5 * (gamma - 1.0) * alpha * alpha;
      bb = -(gamma - 1.0) * alpha * R_plus;
      cc = 0.5 * (gamma - 1.0) * R_plus * R_plus -
           2.0 * c_total_sq / (gamma - 1.0);
      ...
      Mach_sq = v_sq / (c_r_sq);
      Mach_sq = std::min(Mach_sq, 1.0);
      v_sq = Mach_sq * c_r_sq;
      V_r = sqrt(v_sq);
      c_r_sq = c_total_sq - 0.5 * (gamma - 1.0) * v_sq;

      // Compute velocity (based on free stream direction)
      for (int i = 0; i < n_dims; i++) v_r[i] = V_r * n_free_stream[i];
    ...
    // Slip wall
      ...
      // Compute normal velocity on left side
      vn_l = 0.;
      for (int i = 0; i < n_dims; i++) vn_l += (v_l[i] - v_g[i]) * norm[i];

      // reflect normal velocity
      for (int i = 0; i < n_dims; i++) v_r[i] = v_l[i] - 2.0 * vn_l * norm[i];
    ...
    // Isothermal, no-slip wall (fixed)
      ...
      // no-slip
      for (int i = 0; i < n_dims; i++) v_r[i] = v_g[i];
    ...
    // Isothermal, no-slip wall (moving)
      ...
      // no-slip
      for (int i = 0; i < n_dims; i++) v_r[i] = v_wall[i] + v_g[i];
    ...
    // Characteristic
      ...
      vn_bound = 0;
      for (int i = 0; i < n_dims; i++) vn_bound += v_bound[i] * norm[i];

      r_plus = vn_l + 2. / (gamma - 1.) * sqrt(gamma * p_l / rho_l);
      r_minus =
          vn_bound - 2. / (gamma - 1.) * sqrt(gamma * p_bound / rho_bound);
      ...
      if (vn_l < 0) {
        ...
        v_sq = 0.;
        for (int i = 0; i < n_dims; i++) v_sq += v_bound[i] * v_bound[i];
        h_free_stream = gamma / (gamma - 1.) * p_bound / rho_bound + 0.5 * v_sq;

        rho_r =
            pow(1. / gamma * (one_over_s * c_star * c_star), 1. / (gamma - 1.));
    ...
    // Dual consistent BC (see SD++ for more comments)
      ...
      // set v = v - (vn_l)ny
      // set w = w - (vn_l)nz
      for (int i = 0; i < n_dims; i++) v_r[i] = v_l[i] - vn_l * norm[i];
    ...
    u_r[0] = rho_r;
    for (int i = 0; i < n_dims; i++) u_r[i + 1] = rho_r * v_r[i];
    u_r[n_dims + 1] = e_r;
  }
...
/*! Calculate normal transformed continuous viscous flux at the flux points on
 * the boundaries. */
...
void bdy_inters::evaluate_boundaryConditions_viscFlux(double time_bound) {
  ...
  for (int i = 0; i < n_inters; i++) {
    ...
    for (int j = 0; j < n_fpts_per_inter; j++) {
      if (motion) {
        /*! obtain discontinuous solution at flux points (transform to dynamic
         * physical domain) */
        for (int k = 0; k < n_fields; k++)
      ...
      set_inv_boundary_conditions(
          bdy_spec, temp_u_l.get_ptr_cpu(), temp_u_r.get_ptr_cpu(),
          temp_v.get_ptr_cpu(), norm.get_ptr_cpu(), temp_loc.get_ptr_cpu(),
          bdy_params.get_ptr_cpu(), n_dims, n_fields, run_input.gamma,
          run_input.R_ref, time_bound, run_input.equation);
      ...
      if (flux_spec == 2) {
        ...
        set_vis_boundary_conditions(
            bdy_spec, temp_u_l.get_ptr_cpu(), temp_u_r.get_ptr_cpu(),
            temp_grad_u_r.get_ptr_cpu(), norm.get_ptr_cpu(),
            temp_loc.get_ptr_cpu(), bdy_params.get_ptr_cpu(), n_dims, n_fields,
            run_input.gamma, run_input.R_ref, time_bound, run_input.equation);
      }
      ...
      /*! Calling viscous riemann solver */
      if (run_input.vis_riemann_solve_type == 0)
        ldg_flux(flux_spec, temp_u_l, temp_u_r, temp_f_l, temp_f_r, norm, fn,
                 n_dims, n_fields, run_input.tau, run_input.pen_fact);
      else
        FatalError("Viscous Riemann solver not implemented");
      ...
      if (motion) {
        for (int k = 0; k < n_fields; k++)
          (*norm_tconf_fpts_l(j, i, k)) +=
              fn(k) * (*tdA_fpts_l(j, i)) * (*ndA_dyn_fpts_l(j, i));
      } else {
        for (int k = 0; k < n_fields; k++)
          (*norm_tconf_fpts_l(j, i, k)) += fn(k) * (*tdA_fpts_l(j, i));
      }
  ...
  if (n_inters != 0)
    evaluate_boundaryConditions_viscFlux_gpu_kernel_wrapper(
        n_fpts_per_inter, n_dims, n_fields, n_inters, disu_fpts_l.get_ptr_gpu(),
        grad_disu_fpts_l.get_ptr_gpu(), norm_tconf_fpts_l.get_ptr_gpu(),
        tdA_fpts_l.get_ptr_gpu(), ndA_dyn_fpts_l.get_ptr_gpu(),
        J_dyn_fpts_l.get_ptr_gpu(), norm_fpts.get_ptr_gpu(),
        norm_dyn_fpts.get_ptr_gpu(), grid_vel_fpts.get_ptr_gpu(),
        pos_fpts.get_ptr_gpu(), pos_dyn_fpts.get_ptr_gpu(),
        sgsf_fpts_l.get_ptr_gpu(), boundary_type.get_ptr_gpu(),
        bdy_params.get_ptr_gpu(), delta_disu_fpts_l.get_ptr_gpu(),
        run_input.riemann_solve_type, run_input.vis_riemann_solve_type,
        run_input.R_ref, run_input.pen_fact, run_input.tau, run_input.gamma,
        run_input.prandtl, run_input.rt_inf, run_input.mu_inf, run_input.c_sth,
        run_input.fix_vis, time_bound, run_input.equation, run_input.diff_coeff,
        LES, motion, run_input.turb_model, run_input.c_v1, run_input.omega,
        run_input.prandtl_t);
#endif
}

void bdy_inters::set_vis_boundary_conditions(
    int bdy_type, double *u_l, double *u_r, double *grad_u, double *norm,
    double *loc, double *bdy_params, int n_dims, int n_fields, double gamma,
    double R_ref, double time_bound, int equation) {
  int cpu_flag;
  cpu_flag = 1;
  ...
  if (bdy_type == 12 || bdy_type == 14) {
    v_sq = 0.;
    for (int i = 0; i < n_dims; i++) v_sq += (u_l[i + 1] * u_l[i + 1]);
    ...
    if (cpu_flag)  // execute always
    {
      ...
      for (int j = 0; j < n_dims; j++) {
        for (int i = 0; i < n_dims; i++)
          grad_vel[j * n_dims + i] =
              (grad_u[i * n_fields + (j + 1)] -
               grad_u[i * n_fields + 0] * u_r[j + 1] / u_r[0]) /
              u_r[0];
      }
      ...
      if (n_dims == 2) {
        for (int i = 0; i < n_dims; i++)
          grad_u[i * n_fields + 3] =
              inte * grad_u[i * n_fields + 0] +
              0.5 * ((u_r[1] * u_r[1] + u_r[2] * u_r[2]) / (u_r[0] * u_r[0])) *
                  grad_u[i * n_fields + 0] +
              u_r[0] * ((u_r[1] / u_r[0]) * grad_vel[0 * n_dims + i] +
                        (u_r[2] / u_r[0]) * grad_vel[1 * n_dims + i]);
      } else if (n_dims == 3) {
        for (int i = 0; i < n_dims; i++)
          grad_u[i * n_fields + 4] =
              inte * grad_u[i * n_fields + 0] +
              0.5 *
                  ((u_r[1] * u_r[1] + u_r[2] * u_r[2] + u_r[3] * u_r[3]) /
                   (u_r[0] * u_r[0])) *
                  grad_u[i * n_fields + 0] +
              u_r[0] * ((u_r[1] / u_r[0]) * grad_vel[0 * n_dims + i] +
                        (u_r[2] / u_r[0]) * grad_vel[1 * n_dims + i] +
                        (u_r[3] / u_r[0]) * grad_vel[2 * n_dims + i]);
      }
    } else {
      // Velocity gradients
      for (int j = 0; j < n_dims; j++) {
        for (int i = 0; i < n_dims; i++)
          grad_vel[j * n_dims + i] =
              (grad_u[(j + 1) * n_dims + i] -
               grad_u[0 * n_dims + i] * u_r[j + 1] / u_r[0]) /
              u_r[0];
      }
