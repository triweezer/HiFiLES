  ...
  for (int l = 0; l < p_res - 1; ++l) {
    for (int j = 0; j < p_res - 1; ++j) {  // look to right from each point
      for (int k = 0; k < p_res - j - 1; ++k) {
        ...
        vertex_1 = vertex_0 + 1;
        vertex_2 =
            k + ((j + 1) * (p_res + 1)) - (((j + 1) * (j + 2)) / 2) + l * temp;
  ...
  for (int l = 0; l < p_res - 1; ++l) {
    for (int j = 0; j < p_res - 2; ++j) {  //  look to left from each point
      for (int k = 1; k < p_res - j - 1; ++k) {
        ...
        vertex_1 =
            k + ((j + 1) * (p_res + 1)) - (((j + 1) * (j + 2)) / 2) + l * temp;
        vertex_2 = k - 1 + ((j + 1) * (p_res + 1)) - (((j + 1) * (j + 2)) / 2) +
                   l * temp;
  ...
  for (int face = 0; face < 3; face++) {
    for (int i = 0; i < order + 1; i++) {
      for (int j = 0; j < order + 1; j++) {
        if (face == 0) {
          tloc_fpts(0, offset + face * (order + 1) * (order + 1) +
                           i * (order + 1) + j) = loc_1d_fpts(j);
          tloc_fpts(1, offset + face * (order + 1) * (order + 1) +
                           i * (order + 1) + j) = -1;
          ;
        } else if (face == 1) {
          tloc_fpts(0, offset + face * (order + 1) * (order + 1) +
                           i * (order + 1) + j) = loc_1d_fpts(order - j);
          tloc_fpts(1, offset + face * (order + 1) * (order + 1) +
                           i * (order + 1) + j) = loc_1d_fpts(j);
        } else if (face == 2) {
          tloc_fpts(0, offset + face * (order + 1) * (order + 1) +
        ...
        tloc_fpts(2, offset + face * (order + 1) * (order + 1) +
                         i * (order + 1) + j) = loc_1d_fpts(i);
      }
    }
  }
}
...
double eles_pris::compute_inter_detjac_inters_cubpts(int in_inter,
                                                     array<double> d_pos) {
  double output = 0.;
  double xr, xs, xt;
  double yr, ys, yt;
...
void eles_pris::set_vandermonde_tri() {
  ...
  for (int i = 0; i < n_upts_tri; i++)
    for (int j = 0; j < n_upts_tri; j++)
      vandermonde_tri(i, j) = eval_dubiner_basis_2d(
          loc_upts_pri_tri(0, i), loc_upts_pri_tri(1, i), j, order);
...
void eles_pris::set_vandermonde_tri_restart() {
  ...
  for (int i = 0; i < n_upts_tri_rest; i++)
    for (int j = 0; j < n_upts_tri_rest; j++)
      vandermonde_tri_rest(i, j) =
          eval_dubiner_basis_2d(loc_upts_pri_tri_rest(0, i),
                                loc_upts_pri_tri_rest(1, i), j, order_rest);
...
void eles_pris::write_restart_info(std::ofstream &restart_file) {
  ...
  restart_file << "Number of solution points per prismatic element"
               << std::endl;
...
double eles_pris::eval_nodal_basis(int in_index, array<double> in_loc) {
  ...
  for (int i = 0; i < n_upts_tri; i++)
    dubiner_basis_at_loc(i) =
        eval_dubiner_basis_2d(in_loc(0), in_loc(1), i, order);

  // From Hesthaven, equation 3.3, V^T * l = P, or l = (V^-1)^T P
  tri_nodal_basis_at_loc = 0.;
  for (int i = 0; i < n_upts_tri; i++)
    tri_nodal_basis_at_loc +=
        inv_vandermonde_tri(i, index_tri) * dubiner_basis_at_loc(i);
...
double eles_pris::eval_nodal_basis_restart(int in_index, array<double> in_loc) {
  ...
  for (int i = 0; i < n_upts_tri_rest; i++)
    dubiner_basis_at_loc(i) =
        eval_dubiner_basis_2d(in_loc(0), in_loc(1), i, order_rest);

  // From Hesthaven, equation 3.3, V^T * l = P, or l = (V^-1)^T P
  tri_nodal_basis_at_loc = 0.;
  for (int i = 0; i < n_upts_tri_rest; i++)
    tri_nodal_basis_at_loc +=
        inv_vandermonde_tri_rest(i, index_tri) * dubiner_basis_at_loc(i);

  // 2. Now evaluate the 1D lagrange basis at loc(2)
  oned_nodal_basis_at_loc =
      eval_lagrange(in_loc(2), index_1d, loc_upts_pri_1d_rest);
...
double eles_pris::eval_d_nodal_basis(int in_index, int in_cpnt,
                                     array<double> in_loc) {
  ...
  if (in_cpnt == 0 || in_cpnt == 1) {
    ...
    for (int i = 0; i < n_upts_tri; i++) {
      if (in_cpnt == 0)
        d_dubiner_basis_at_loc(i) =
            eval_dr_dubiner_basis_2d(in_loc(0), in_loc(1), i, order);
      else if (in_cpnt == 1)
        d_dubiner_basis_at_loc(i) =
            eval_ds_dubiner_basis_2d(in_loc(0), in_loc(1), i, order);
    }

    // From Hesthaven, equation 3.3, V^T * l = P, or l = (V^-1)^T P
    d_tri_nodal_basis_at_loc = 0.;
    for (int i = 0; i < n_upts_tri; i++)
      d_tri_nodal_basis_at_loc +=
          inv_vandermonde_tri(i, index_tri) * d_dubiner_basis_at_loc(i);

    // 2. Evaluate the 1d nodal basis at loc(2)
    oned_nodal_basis_at_loc =
        eval_lagrange(in_loc(2), index_1d, loc_upts_pri_1d);

    out_d_nodal_basis_at_loc =
        d_tri_nodal_basis_at_loc * oned_nodal_basis_at_loc;
  } else if (in_cpnt == 2) {
    double tri_nodal_basis_at_loc;
    ...
    d_oned_nodal_basis_at_loc =
        eval_d_lagrange(in_loc(2), index_1d, loc_upts_pri_1d);

    out_d_nodal_basis_at_loc =
        tri_nodal_basis_at_loc * d_oned_nodal_basis_at_loc;
  }
...
double eles_pris::eval_nodal_s_basis(int in_index, array<double> in_loc,
                                     int in_n_spts) {
  ...
  if (in_n_spts == 6) {
    ...
    if (in_index == 0)
      nodal_s_basis = (1. / 4 * (in_loc(0) + in_loc(1))) *
                      (in_loc(0) + in_loc(1) + 1.) * in_loc(2) *
                      (in_loc(2) - 1.);
    else if (in_index == 1)
      nodal_s_basis = (1. / 4) * in_loc(0) * (in_loc(0) + 1.) * in_loc(2) *
                      (in_loc(2) - 1.);
    else if (in_index == 2)
      nodal_s_basis = (1. / 4) * in_loc(1) * (in_loc(1) + 1.) * in_loc(2) *
                      (in_loc(2) - 1.);
    else if (in_index == 3)
      nodal_s_basis = (1. / 4 * (in_loc(0) + in_loc(1))) *
                      (in_loc(0) + in_loc(1) + 1.) * in_loc(2) *
      ...
      nodal_s_basis = -(1. / 2 * (in_loc(0) + in_loc(1))) * (in_loc(0) + 1.) *
                      in_loc(2) * (in_loc(2) - 1.);
    else if (in_index == 7)
      nodal_s_basis = (1. / 2 * (in_loc(0) + 1.)) * (in_loc(1) + 1.) *
                      in_loc(2) * (in_loc(2) - 1.);
    else if (in_index == 8)
      nodal_s_basis = -(1. / 2 * (in_loc(0) + in_loc(1))) * (in_loc(1) + 1.) *
                      in_loc(2) * (in_loc(2) - 1.);
    else if (in_index == 9)
      nodal_s_basis =
          (1. / 2 * (in_loc(0) + in_loc(1))) * (in_loc(2) * in_loc(2) - 1.);
    else if (in_index == 10)
      nodal_s_basis =
          -(1. / 2 * (in_loc(0) + 1.)) * (in_loc(2) * in_loc(2) - 1.);
    else if (in_index == 11)
      nodal_s_basis =
          -(1. / 2 * (in_loc(1) + 1.)) * (in_loc(2) * in_loc(2) - 1.);
    else if (in_index == 12)
      nodal_s_basis = -(1. / 2 * (in_loc(0) + in_loc(1))) * (in_loc(0) + 1.) *
                      in_loc(2) * (in_loc(2) + 1.);
    else if (in_index == 13)
...
void eles_pris::eval_d_nodal_s_basis(array<double> &d_nodal_s_basis,
  ...
  if (in_n_spts == 6) {
    ...
    d_nodal_s_basis(0, 0) = (1. / 4) * in_loc(2) * (in_loc(2) - 1.) *
                            (2 * in_loc(0) + 2 * in_loc(1) + 1.);
    d_nodal_s_basis(1, 0) =
        (1. / 4) * in_loc(2) * (in_loc(2) - 1.) * (2 * in_loc(0) + 1.);
    d_nodal_s_basis(2, 0) = 0.;
    d_nodal_s_basis(3, 0) = (1. / 4) * in_loc(2) * (in_loc(2) + 1.) *
                            (2 * in_loc(0) + 2 * in_loc(1) + 1.);
    d_nodal_s_basis(4, 0) =
        (1. / 4) * in_loc(2) * (in_loc(2) + 1.) * (2 * in_loc(0) + 1.);
    d_nodal_s_basis(5, 0) = 0.;
    d_nodal_s_basis(6, 0) = -(1. / 2) * in_loc(2) * (in_loc(2) - 1.) *
                            (2 * in_loc(0) + 1. + in_loc(1));
    d_nodal_s_basis(7, 0) =
        (1. / 2 * (in_loc(1) + 1.)) * in_loc(2) * (in_loc(2) - 1.);
    d_nodal_s_basis(8, 0) =
        -(1. / 2 * (in_loc(1) + 1.)) * in_loc(2) * (in_loc(2) - 1.);
    ...
    d_nodal_s_basis(12, 0) = -(1. / 2) * in_loc(2) * (in_loc(2) + 1.) *
                             (2 * in_loc(0) + 1. + in_loc(1));
    ...
    d_nodal_s_basis(0, 1) = (1. / 4) * in_loc(2) * (in_loc(2) - 1.) *
                            (2 * in_loc(0) + 2 * in_loc(1) + 1.);
    d_nodal_s_basis(1, 1) = 0.;
    d_nodal_s_basis(2, 1) =
        (1. / 4) * in_loc(2) * (in_loc(2) - 1.) * (2 * in_loc(1) + 1.);
    d_nodal_s_basis(3, 1) = (1. / 4) * in_loc(2) * (in_loc(2) + 1.) *
                            (2 * in_loc(0) + 2 * in_loc(1) + 1.);
    d_nodal_s_basis(4, 1) = 0.;
    d_nodal_s_basis(5, 1) =
        (1. / 4) * in_loc(2) * (in_loc(2) + 1.) * (2 * in_loc(1) + 1.);
    ...
    d_nodal_s_basis(7, 1) =
        (1. / 2 * (in_loc(0) + 1.)) * in_loc(2) * (in_loc(2) - 1.);
    d_nodal_s_basis(8, 1) = -(1. / 2) * in_loc(2) * (in_loc(2) - 1.) *
                            (2 * in_loc(1) + 1. + in_loc(0));
    ...
    d_nodal_s_basis(13, 1) =
        (1. / 2 * (in_loc(0) + 1.)) * in_loc(2) * (in_loc(2) + 1.);
    d_nodal_s_basis(14, 1) = -(1. / 2) * in_loc(2) * (in_loc(2) + 1.) *
                             (2 * in_loc(1) + 1. + in_loc(0));

    d_nodal_s_basis(0, 2) = (1. / 4 * (in_loc(0) + in_loc(1) + 1.)) *
                            (in_loc(0) + in_loc(1)) * (2 * in_loc(2) - 1.);
    d_nodal_s_basis(1, 2) =
        (1. / 4) * in_loc(0) * (2 * in_loc(2) - 1.) * (in_loc(0) + 1.);
    d_nodal_s_basis(2, 2) =
        (1. / 4) * in_loc(1) * (2 * in_loc(2) - 1.) * (in_loc(1) + 1.);
    d_nodal_s_basis(3, 2) = (1. / 4 * (in_loc(0) + in_loc(1) + 1.)) *
                            (in_loc(0) + in_loc(1)) * (2 * in_loc(2) + 1.);
    d_nodal_s_basis(4, 2) =
        (1. / 4) * in_loc(0) * (2 * in_loc(2) + 1.) * (in_loc(0) + 1.);
    d_nodal_s_basis(5, 2) =
        (1. / 4) * in_loc(1) * (2 * in_loc(2) + 1.) * (in_loc(1) + 1.);
    d_nodal_s_basis(6, 2) = -(1. / 2 * (2 * in_loc(2) - 1.)) *
                            (in_loc(0) + 1.) * (in_loc(0) + in_loc(1));
    d_nodal_s_basis(7, 2) =
        (1. / 2 * (2 * in_loc(2) - 1.)) * (in_loc(1) + 1.) * (in_loc(0) + 1.);
    d_nodal_s_basis(8, 2) = -(1. / 2 * (2 * in_loc(2) - 1.)) *
                            (in_loc(1) + 1.) * (in_loc(0) + in_loc(1));
    ...
    d_nodal_s_basis(12, 2) = -(1. / 2 * (2 * in_loc(2) + 1.)) *
                             (in_loc(0) + 1.) * (in_loc(0) + in_loc(1));
    d_nodal_s_basis(13, 2) =
        (1. / 2 * (2 * in_loc(2) + 1.)) * (in_loc(1) + 1.) * (in_loc(0) + 1.);
    d_nodal_s_basis(14, 2) = -(1. / 2 * (2 * in_loc(2) + 1.)) *
                             (in_loc(1) + 1.) * (in_loc(0) + in_loc(1));
  } else {
    FatalError("Shape order not implemented yet, exiting");
  }
...
void eles_pris::fill_opp_3(array<double> &opp_3) {
  ...
  get_opp_3_tri(opp_3_tri, loc_upts_pri_tri, loc_1d_fpts, vandermonde_tri,
                inv_vandermonde_tri, n_upts_tri, order, run_input.c_tri,
                run_input.vcjh_scheme_tri);
    ...
    for (int in_index = 0; in_index < n_fpts_per_ele; in_index++) {
      ...
      // face 2
        ...
        if (face_fpt / (order + 1) == upt_1d)
          // opp_3(upt,in_index)=
          // eval_div_dg_tri(loc,edge,edge_fpt,order,loc_upts_pri_1d);
          opp_3(upt, in_index) =
              opp_3_tri(upt_tri, edge * (order + 1) + edge_fpt);
        else
          opp_3(upt, in_index) = 0.;
      }
      // face 3
      else if (in_index < n_fpts_per_inter(0) + n_fpts_per_inter(1) +
                              n_fpts_per_inter(2) + n_fpts_per_inter(3)) {
...
double eles_pris::eval_div_vcjh_basis(int in_index, array<double> &loc) {
  ...
  // Check that loc is at one of the solution points, otherwise procedure
  // doesn't work
  int flag = 1;
  int upt;
  ...
  if (flag == 1)
    FatalError("eval_div_vcjh_basis is not at solution point, exiting");
  ...
  // face 2
           ...
           n_fpts_per_inter(0) + n_fpts_per_inter(1) + n_fpts_per_inter(2)) {
    int face_fpt = in_index - 2 * n_fpts_per_inter(0);
    int edge_fpt = face_fpt % (order + 1);
    int edge = 0;
