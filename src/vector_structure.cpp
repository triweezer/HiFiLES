...
CSysVector::CSysVector(void) { vec_val = NULL; }
...
CSysVector::CSysVector(const unsigned long &size, const double &val) {
  ...
  for (unsigned int i = 0; i < nElm; i++) vec_val[i] = val;
...
CSysVector::CSysVector(const unsigned long &numBlk,
                       ...
                       const unsigned short &numVar, const double &val) {
  nElm = numBlk * numVar;
  ...
  if ((nElm <= 0) || (nElm >= ULONG_MAX)) {
    std::cerr << "CSysVector::CSysVector(unsigned int,unsigned int,double): "
              << "invalid input: numBlk, numVar = " << numBlk << "," << numVar
              << std::endl;
    throw(-1);
  }
...
CSysVector::CSysVector(const CSysVector &u) {
  ...
  vec_val = new double[nElm];
  for (unsigned long i = 0; i < nElm; i++) vec_val[i] = u.vec_val[i];
...
CSysVector::CSysVector(const unsigned long &size, const double *u_array) {
  ...
  vec_val = new double[nElm];
  for (unsigned long i = 0; i < nElm; i++) vec_val[i] = u_array[i];
...
void CSysVector::Initialize(const unsigned long &numBlk,
  ...
  vec_val = new double[nElm];
  for (unsigned long i = 0; i < nElm; i++) vec_val[i] = val;
...
void CSysVector::Equals_AX(const double &a, CSysVector &x) {
  ...
  if (nElm != x.nElm) {
    std::cerr << "CSysVector::Equals_AX(): "
              << "sizes do not match";
    throw(-1);
  }
  for (unsigned long i = 0; i < nElm; i++) vec_val[i] = a * x.vec_val[i];
}

void CSysVector::Plus_AX(const double &a, CSysVector &x) {
  /*--- check that *this and x are compatible ---*/
  if (nElm != x.nElm) {
    std::cerr << "CSysVector::Plus_AX(): "
              << "sizes do not match";
    throw(-1);
  }
  for (unsigned long i = 0; i < nElm; i++) vec_val[i] += a * x.vec_val[i];
...
void CSysVector::Equals_AX_Plus_BY(const double &a, CSysVector &x,
                                   const double &b, CSysVector &y) {
  /*--- check that *this, x and y are compatible ---*/
  if ((nElm != x.nElm) || (nElm != y.nElm)) {
    std::cerr << "CSysVector::Equals_AX_Plus_BY(): "
              << "sizes do not match";
    throw(-1);
  }
...
CSysVector &CSysVector::operator+=(const CSysVector &u) {
  ...
  if (nElm != u.nElm) {
    std::cerr << "CSysVector::operator+=(CSysVector): "
              << "sizes do not match";
    throw(-1);
  }
  for (unsigned long i = 0; i < nElm; i++) vec_val[i] += u.vec_val[i];
...
void CSysVector::CopyToArray(double *u_array) {
  for (unsigned long i = 0; i < nElm; i++) u_array[i] = vec_val[i];
}
...
void CSysVector::SetBlock(unsigned long val_ipoint, unsigned short val_var,
                          double val_residual) {
...
void CSysVector::SetBlock_Zero(unsigned long val_ipoint) {
  ...
  for (iVar = 0; iVar < nVar; iVar++) vec_val[val_ipoint * nVar + iVar] = 0.0;
}
