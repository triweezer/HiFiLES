...
void inters::get_lut(int in_rot_tag) {
  ...
  } else if (inters_type == 1)  // triangle face
  {
    ...
    } else if (in_rot_tag == 1)  // Example face 0 with 3
    {
      for (j = 0; j < order + 1; j++) {
        for (i = 0; i < order + 1 - j; i++) {
          index0 = j * (order + 1) - (j - 1) * j / 2 + i;
          index1 =
              (order + 1) * (order + 2) / 2 - 1 - (i + j) * (i + j + 1) / 2 - j;
          lut(index0) = index1;
        }
      }
    } else if (in_rot_tag == 2)  // Example face 0 with 2
    {
      ...
      std::cerr << "ERROR: Unknown rotation of triangular face..." << std::endl;
    }
  } else if (inters_type == 2)  // quad face
  {
    if (in_rot_tag == 0) {
      ...
      for (i = 0; i < (order + 1); i++) {
        for (j = 0; j < (order + 1); j++) {
          lut((i * (order + 1)) + j) =
              n_fpts_per_inter - ((order + 1) - 1 - j) - ((order + 1) * i) - 1;
        }
      }
      ...
      for (i = 0; i < (order + 1); i++) {
        for (j = 0; j < (order + 1); j++) {
          lut((i * (order + 1)) + j) =
              n_fpts_per_inter - ((order + 1) * j) - i - 1;
        }
      }
    } else {
      std::cerr << "ERROR: Unknown rotation tag ... " << std::endl;
    }
  } else {
    FatalError("ERROR: Invalid interface type ... ");
  }
...
void inters::right_flux(array<double> &f_r, array<double> &norm,
                        array<double> &fn, int n_dims, int n_fields,
                        double gamma) {
...
// Rusanov inviscid numerical flux
void inters::rusanov_flux(array<double> &u_l, array<double> &u_r,
                          array<double> &v_g, array<double> &f_l,
                          array<double> &f_r, array<double> &norm,
                          array<double> &fn, int n_dims, int n_fields,
                          double gamma) {
  double vx_l, vy_l, vx_r, vy_r, vz_l, vz_r, vn_l, vn_r, p_l, p_r, vn_g,
      vn_av_mag, c_av, eig;
  ...
  if (n_dims == 2) {
    ...
    p_l = (gamma - 1.0) *
          (u_l(3) - (0.5 * u_l(0) * ((vx_l * vx_l) + (vy_l * vy_l))));
    p_r = (gamma - 1.0) *
          (u_r(3) - (0.5 * u_r(0) * ((vx_r * vx_r) + (vy_r * vy_r))));
  } else if (n_dims == 3) {
    vz_l = u_l(3) / u_l(0);
    vz_r = u_r(3) / u_r(0);
    ...
    p_l = (gamma - 1.0) *
          (u_l(4) -
           (0.5 * u_l(0) * ((vx_l * vx_l) + (vy_l * vy_l) + (vz_l * vz_l))));
    p_r = (gamma - 1.0) *
          (u_r(4) -
           (0.5 * u_r(0) * ((vx_r * vx_r) + (vy_r * vy_r) + (vz_r * vz_r))));
  } else
...
void inters::convective_flux_boundary(array<double> &f_l, array<double> &f_r,
                                      array<double> &norm, array<double> &fn,
                                      int n_dims, int n_fields) {
  array<double> fn_l(n_fields), fn_r(n_fields);
  ...
  for (int k = 0; k < n_fields; k++) fn(k) = 0.5 * (fn_l(k) + fn_r(k));
}

// Roe inviscid numerical flux
void inters::roe_flux(array<double> &u_l, array<double> &u_r,
                      array<double> &v_g, array<double> &norm,
                      array<double> &fn, int n_dims, int n_fields,
                      double gamma) {
  double p_l, p_r;
  double h_l, h_r;
  ...
  if (n_dims == 2) {
    p_l = (gamma - 1.0) *
          (u_l(3) - (0.5 * u_l(0) * ((v_l(0) * v_l(0)) + (v_l(1) * v_l(1)))));
    p_r = (gamma - 1.0) *
          (u_r(3) - (0.5 * u_r(0) * ((v_r(0) * v_r(0)) + (v_r(1) * v_r(1)))));
  } else
  ...
  for (int i = 0; i < n_dims; i++) um(i) = rrho * (v_l(i) + sq_rho * v_r(i));

  hm = rrho * (h_l + sq_rho * h_r);

  usq = 0.;
  for (int i = 0; i < n_dims; i++) usq += 0.5 * um(i) * um(i);
  ...
  // Entropy fix
  eps = 0.5 * (abs(rhoun_l / u_l(0) - rhoun_r / u_r(0)) +
               abs(sqrt(gamma * p_l / u_l(0)) - sqrt(gamma * p_r / u_r(0))));
  if (lambda0 < 2. * eps) lambda0 = 0.25 * lambda0 * lambda0 / eps + eps;
  if (lambdaP < 2. * eps) lambdaP = 0.25 * lambdaP * lambdaP / eps + eps;
  if (lambdaM < 2. * eps) lambdaM = 0.25 * lambdaM * lambdaM / eps + eps;
...
// Rusanov inviscid numerical flux
void inters::lax_friedrich(array<double> &u_l, array<double> &u_r,
                           array<double> &norm, array<double> &fn, int n_dims,
                           int n_fields, double lambda,
...
void inters::ldg_flux(int flux_spec, array<double> &u_l, array<double> &u_r,
                      array<double> &f_l, array<double> &f_r,
                      array<double> &norm, array<double> &fn, int n_dims,
                      int n_fields, double tau, double pen_fact) {
  array<double> f_c(n_fields, n_dims);
  double norm_x, norm_y, norm_z;

  if (n_dims == 2)  // needs to be reviewed and understood
  {
    if ((norm(0) + norm(1)) < 0.) pen_fact = -pen_fact;
  }
  if (n_dims == 3) {
    if ((norm(0) + norm(1) + sqrt(2.) * norm(2)) < 0.) pen_fact = -pen_fact;
  }

  norm_x = norm(0);
  norm_y = norm(1);

  if (n_dims == 3) norm_z = norm(2);

  if (flux_spec == 0)  // Interior and mpi
  {
    for (int k = 0; k < n_fields; k++) {
      if (n_dims == 2) {
        f_c(k, 0) = 0.5 * (f_l(k, 0) + f_r(k, 0)) +
                    pen_fact * norm_x *
                        (norm_x * (f_l(k, 0) - f_r(k, 0)) +
                         norm_y * (f_l(k, 1) - f_r(k, 1))) +
                    tau * norm_x * (u_l(k) - u_r(k));
        f_c(k, 1) = 0.5 * (f_l(k, 1) + f_r(k, 1)) +
                    pen_fact * norm_y *
                        (norm_x * (f_l(k, 0) - f_r(k, 0)) +
                         norm_y * (f_l(k, 1) - f_r(k, 1))) +
                    tau * norm_y * (u_l(k) - u_r(k));
      }
        ...
        f_c(k, 1) = 0.5 * (f_l(k, 1) + f_r(k, 1)) +
                    pen_fact * norm_y *
                        (norm_x * (f_l(k, 0) - f_r(k, 0)) +
                         ...
                         norm_z * (f_l(k, 2) - f_r(k, 2))) +
                    tau * norm_y * (u_l(k) - u_r(k));
        f_c(k, 2) = 0.5 * (f_l(k, 2) + f_r(k, 2)) +
                    pen_fact * norm_z *
                        (norm_x * (f_l(k, 0) - f_r(k, 0)) +
                         norm_y * (f_l(k, 1) - f_r(k, 1)) +
                         norm_z * (f_l(k, 2) - f_r(k, 2))) +
                    tau * norm_z * (u_l(k) - u_r(k));
      }
    }
...
// LDG common solution
void inters::ldg_solution(int flux_spec, array<double> &u_l, array<double> &u_r,
                          array<double> &u_c, double pen_fact,
                          array<double> &norm) {
  if (flux_spec == 0)  // Interior and mpi
  {
  ...
  {
    for (int k = 0; k < n_fields; k++) u_c(k) = 0.5 * (u_r(k) + u_l(k));
  } else if (flux_spec == 2)  // von Neumann
  {
    for (int k = 0; k < n_fields; k++) u_c(k) = 0.5 * (u_r(k) + u_l(k));
  } else
