...
void int_inters::set_interior(int in_inter, int in_ele_type_l,
                              int in_ele_type_r, int in_ele_l, int in_ele_r,
                              int in_local_inter_l, int in_local_inter_r,
                              int rot_tag, struct solution *FlowSol) {
  int i, j, k;
  int i_rhs, j_rhs;
  ...
  for (i = 0; i < n_fields; i++) {
    for (j = 0; j < n_fpts_per_inter; j++) {
      ...
      disu_fpts_r(j, in_inter, i) = get_disu_fpts_ptr(
          in_ele_type_r, in_ele_r, i, in_local_inter_r, j_rhs, FlowSol);

      norm_tconf_fpts_l(j, in_inter, i) = get_norm_tconf_fpts_ptr(
          in_ele_type_l, in_ele_l, i, in_local_inter_l, j, FlowSol);
      norm_tconf_fpts_r(j, in_inter, i) = get_norm_tconf_fpts_ptr(
          in_ele_type_r, in_ele_r, i, in_local_inter_r, j_rhs, FlowSol);
      ...
      for (int k = 0; k < n_dims; k++) {
        if (viscous) {
          delta_disu_fpts_l(j, in_inter, i) = get_delta_disu_fpts_ptr(
              in_ele_type_l, in_ele_l, i, in_local_inter_l, j, FlowSol);
          delta_disu_fpts_r(j, in_inter, i) = get_delta_disu_fpts_ptr(
              in_ele_type_r, in_ele_r, i, in_local_inter_r, j_rhs, FlowSol);

          grad_disu_fpts_l(j, in_inter, i, k) = get_grad_disu_fpts_ptr(
              in_ele_type_l, in_ele_l, in_local_inter_l, i, k, j, FlowSol);
          grad_disu_fpts_r(j, in_inter, i, k) = get_grad_disu_fpts_ptr(
              in_ele_type_r, in_ele_r, in_local_inter_r, i, k, j_rhs, FlowSol);
        }
        ...
        if (LES) {
          sgsf_fpts_l(j, in_inter, i, k) = get_sgsf_fpts_ptr(
              in_ele_type_l, in_ele_l, in_local_inter_l, i, k, j, FlowSol);
          sgsf_fpts_r(j, in_inter, i, k) = get_sgsf_fpts_ptr(
              in_ele_type_r, in_ele_r, in_local_inter_r, i, k, j_rhs, FlowSol);
        }
      }
    }
  ...
  if (motion) {
    for (j = 0; j < n_fpts_per_inter; j++) {
      ...
      ndA_dyn_fpts_r(j, in_inter) = get_ndA_dyn_fpts_ptr(
          in_ele_type_r, in_ele_r, in_local_inter_r, j_rhs, FlowSol);
      ...
      J_dyn_fpts_l(j, in_inter) = get_detjac_dyn_fpts_ptr(
          in_ele_type_l, in_ele_l, in_local_inter_l, j, FlowSol);
      J_dyn_fpts_r(j, in_inter) = get_detjac_dyn_fpts_ptr(
          in_ele_type_r, in_ele_r, in_local_inter_r, j_rhs, FlowSol);
  ...
  for (i = 0; i < n_fpts_per_inter; i++) {
    ...
    tdA_fpts_r(i, in_inter) = get_tdA_fpts_ptr(
        in_ele_type_r, in_ele_r, in_local_inter_r, i_rhs, FlowSol);
    ...
    for (int j = 0; j < n_fpts_per_inter; j++) {
      ...
      if (motion) {
        for (int k = 0; k < n_fields; k++) {
          ...
          (*norm_tconf_fpts_r(j, i, k)) =
              -fn(k) * (*ndA_dyn_fpts_r(j, i)) * (*tdA_fpts_r(j, i));
        }
      } else {
      ...
      if (viscous) {
        ...
        if (motion)  // include transformation back to static space
        {
          for (int k = 0; k < n_fields; k++) {
            ...
            *delta_disu_fpts_r(j, i, k) =
                (u_c(k) - temp_u_r(k)) * (*J_dyn_fpts_r(j, i));
          }
        } else {
          for (int k = 0; k < n_fields; k++) {
...
#ifdef _GPU
  if (n_inters != 0) {
    calculate_common_invFlux_gpu_kernel_wrapper(
        n_fpts_per_inter, n_dims, n_fields, n_inters, disu_fpts_l.get_ptr_gpu(),
        disu_fpts_r.get_ptr_gpu(), norm_tconf_fpts_l.get_ptr_gpu(),
        norm_tconf_fpts_r.get_ptr_gpu(), tdA_fpts_l.get_ptr_gpu(),
        tdA_fpts_r.get_ptr_gpu(), ndA_dyn_fpts_l.get_ptr_gpu(),
        ndA_dyn_fpts_r.get_ptr_gpu(), J_dyn_fpts_l.get_ptr_gpu(),
        J_dyn_fpts_r.get_ptr_gpu(), norm_fpts.get_ptr_gpu(),
        norm_dyn_fpts.get_ptr_gpu(), grid_vel_fpts.get_ptr_gpu(),
        run_input.riemann_solve_type, delta_disu_fpts_l.get_ptr_gpu(),
        delta_disu_fpts_r.get_ptr_gpu(), run_input.gamma, run_input.pen_fact,
        viscous, motion, run_input.vis_riemann_solve_type,
        run_input.wave_speed(0), run_input.wave_speed(1),
        run_input.wave_speed(2), run_input.lambda, run_input.turb_model);
    ...
    for (int j = 0; j < n_fpts_per_inter; j++) {
      ...
      if (motion) {
        for (int k = 0; k < n_fields; k++) {
          ...
          (*norm_tconf_fpts_r(j, i, k)) +=
              -fn(k) * (*tdA_fpts_r(j, i)) * (*ndA_dyn_fpts_r(j, i));
        }
      } else {
        for (int k = 0; k < n_fields; k++) {
          (*norm_tconf_fpts_l(j, i, k)) += fn(k) * (*tdA_fpts_l(j, i));
...
#ifdef _GPU
  if (n_inters != 0)
    calculate_common_viscFlux_gpu_kernel_wrapper(
        n_fpts_per_inter, n_dims, n_fields, n_inters, disu_fpts_l.get_ptr_gpu(),
        ...
        run_input.c_sth, run_input.fix_vis, run_input.equation,
        run_input.diff_coeff, LES, motion, run_input.turb_model, run_input.c_v1,
        run_input.omega, run_input.prandtl_t);
#endif
}
