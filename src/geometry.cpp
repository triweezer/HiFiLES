...
#    ifdef _YOSEMITESAM
  if (FlowSol->rank == 0) {
    std::cout << "setting CUDA devices on yosemitesam ..." << std::endl;
  }
  if ((FlowSol->rank % 2) == 0) {
    cudaSetDevice(0);
  }
...
#    ifdef _ENRICO
  if (FlowSol->rank == 0) {
    std::cout << "setting CUDA devices on enrico ..." << std::endl;
  }
  if (FlowSol->rank == 0) {
    cudaSetDevice(2);
...
#    ifndef _ENRICO
#      ifndef _YOSEMITESAM
  // NOTE: depening on system arcihtecture, this may not be the GPU device you
  // want i.e. one of the devices may be a (non-scientific-computing) graphics
  // card
...
int get_bc_number(std::string &bcname) {
  int bcflag;

  std::transform(bcname.begin(), bcname.end(), bcname.begin(), ::tolower);

  if (!bcname.compare("sub_in_simp"))
    bcflag = 1;  // Subsonic inflow simple (free pressure) //
  else if (!bcname.compare("sub_out_simp"))
    bcflag = 2;  // Subsonic outflow simple (fixed pressure) //
  else if (!bcname.compare("sub_in_char"))
    bcflag = 3;  // Subsonic inflow characteristic //
  else if (!bcname.compare("sub_out_char"))
    bcflag = 4;  // Subsonic outflow characteristic //
  else if (!bcname.compare("sup_in"))
    bcflag = 5;  // Supersonic inflow //
  else if (!bcname.compare("sup_out"))
    bcflag = 6;  // Supersonic outflow //
  else if (!bcname.compare("slip_wall"))
    bcflag = 7;  // Slip wall //
  else if (!bcname.compare("cyclic"))
    bcflag = 9;
  else if (!bcname.compare("isotherm_fix"))
    bcflag = 11;  // Isothermal, no-slip wall //
  else if (!bcname.compare("adiabat_fix"))
    bcflag = 12;  // Adiabatic, no-slip wall //
  else if (!bcname.compare("isotherm_move"))
    bcflag = 13;  // Isothermal, no-slip moving wall //
  else if (!bcname.compare("adiabat_move"))
    bcflag = 14;  // Adiabatic, no-slip moving wall //
  else if (!bcname.compare("char"))
    bcflag = 15;  // Characteristic //
  else if (!bcname.compare("slip_wall_dual"))
    bcflag = 16;  // Dual consistent BC //
  else if (!bcname.compare("ad_wall"))
    bcflag = 50;  // Advection, Advection-Diffusion Boundary Conditions //
  else {
...
void GeoPreprocess(struct solution *FlowSol, mesh &Mesh) {
  ...
  ReadMesh(run_input.mesh_file, xv, c2v, c2n_v, ctype, ic2icg, iv2ivg,
           FlowSol->num_eles, FlowSol->num_verts, Mesh.n_verts_global, FlowSol);
  ...
  f2v.setup(max_inters,
            MAX_V_PER_F);  // each edge/face cannot have more than 4 vertices
  f2nv.setup(max_inters);
  f2loc_f.setup(max_inters, 2);
  c2f.setup(FlowSol->num_eles,
            MAX_F_PER_C);  // one cell cannot have more than 8 faces
  c2e.setup(FlowSol->num_eles,
            MAX_E_PER_C);  // one cell cannot have more than 8 faces
  ...
  // CompConnectivity(c2v, c2n_v, ctype, c2f, c2e, f2c, f2loc_f, f2v, f2nv,
  // rot_tag, unmatched_inters, n_unmatched_inters, icvsta, icvert,
  // FlowSol->num_inters, FlowSol->num_edges, FlowSol);
  CompConnectivity(c2v, c2n_v, ctype, c2f, c2e, f2c, f2loc_f, f2v, f2nv,
                   Mesh.e2v, Mesh.v2n_e, Mesh.v2e, rot_tag, unmatched_inters,
                   n_unmatched_inters, icvsta, icvert, FlowSol->num_inters,
                   FlowSol->num_edges, FlowSol);

  if (FlowSol->rank == 0) std::cout << "Done setting up mesh connectivity" << std::endl;

  // Reading boundaries
  // ReadBound(run_input.mesh_file,c2v,c2n_v,ctype,bctype_c,ic2icg,icvsta,icvert,iv2ivg,FlowSol->num_eles,FlowSol->num_verts,
  // FlowSol);
  ReadBound(run_input.mesh_file, c2v, c2n_v, c2f, f2v, f2nv, ctype, bctype_c,
            Mesh.boundPts, Mesh.bc_list, Mesh.bound_flags, ic2icg, icvsta,
            icvert, iv2ivg, FlowSol->num_eles, FlowSol->num_verts, FlowSol);
  ...
  // Error checking
  if (FlowSol->n_dims == 2 &&
      (num_tets != 0 || num_pris != 0 || num_hexas != 0)) {
  ...
  if (FlowSol->rank == 0)
    std::cout << "pre-computing nodal shape-basis functions ... " << std::flush;
  for (int i = 0; i < FlowSol->n_ele_types; i++) {
  ...
  for (int i = 0; i < FlowSol->n_ele_types; i++) {
    if (FlowSol->mesh_eles(i)->get_n_eles() != 0) {
      ...
      if (run_input.motion) FlowSol->mesh_eles(i)->set_transforms_dynamic();
    }
  }
  if (FlowSol->rank == 0) std::cout << "done." << std::endl;
  ...
  if (FlowSol->rank == 0)
    std::cout << "initializing grid velocity to 0 ... " << std::flush;
  for (int i = 0; i < FlowSol->n_ele_types; i++) {
    if (FlowSol->mesh_eles(i)->get_n_eles() != 0) {
  ...
  if (FlowSol->rank == 0)
    std::cout << "setting element transforms at interface cubpts ... ";
  for (int i = 0; i < FlowSol->n_ele_types; i++) {
    if (FlowSol->mesh_eles(i)->get_n_eles() != 0) {
  ...
  // Set metrics at volume cubpts. Only needed for computing error and integral
  // diagnostic quantities.
  if (run_input.test_case != 0 || run_input.monitor_integrals_freq != 0) {
    if (FlowSol->rank == 0)
      std::cout << "setting element transforms at volume cubpts ... " << std::endl;
    for (int i = 0; i < FlowSol->n_ele_types; i++) {
      if (FlowSol->mesh_eles(i)->get_n_eles() != 0) {
  ...
  // set on gpu (important - need to do this before we set connectivity, so that
  ...
  array<double> loc_center_inter_0(FlowSol->n_dims),
      loc_center_inter_1(FlowSol->n_dims);
  array<double> loc_vert_0(MAX_V_PER_F, FlowSol->n_dims),
      loc_vert_1(MAX_V_PER_F, FlowSol->n_dims);
  ...
  for (int i = 0; i < FlowSol->num_inters; i++) {
    ...
    if (bctype_f == 9) {
      for (int m = 0; m < FlowSol->n_dims; m++) loc_center_inter_0(m) = 0.;
      ...
      for (int j = 0; j < n_unmatched_inters; j++) {
        ...
        for (int m = 0; m < FlowSol->n_dims; m++) loc_center_inter_1(m) = 0.;

        for (int k = 0; k < f2nv(i2); k++)
          for (int m = 0; m < FlowSol->n_dims; m++)
            loc_center_inter_1(m) += xv(f2v(i2, k), m) / f2nv(i2);

        if (check_cyclic(delta_cyclic, loc_center_inter_0, loc_center_inter_1,
                         tol, FlowSol)) {
          found = 1;
          f2c(i, 1) = f2c(i2, 0);
          bctype_c(f2c(i, 0), f2loc_f(i, 0)) = 0;
          // Change the flag of matching cyclic inter so that it's not counted
          // as interior inter
          ...
          compare_cyclic_faces(loc_vert_0, loc_vert_1, f2nv(i), rtag,
                               delta_cyclic, tol, FlowSol);
          rot_tag(i) = rtag;
          break;
        }
      }
      if (found ==
          0)  // Corresponding cyclic edges belongs to another processsor
      {
        f2c(i, 1) = -1;
        bctype_c(f2c(i, 0), f2loc_f(i, 0)) = 0;
      }
  ...
  // Call function that takes in f_mpi2f,f2v and returns a new array f_mpi2f,
  // and an array mpiface_part that contains the number of faces to send to each
  // processor the new array f_mpi2f is in good order i.e. proc1,proc2,....

  match_mpifaces(f2v, f2nv, xv, f_mpi2f, mpifaces_part, delta_cyclic,
                 FlowSol->n_mpi_inters, tol, FlowSol);

  array<int> rot_tag_mpi(FlowSol->n_mpi_inters);
  find_rot_mpifaces(f2v, f2nv, xv, f_mpi2f, rot_tag_mpi, mpifaces_part,
                    delta_cyclic, FlowSol->n_mpi_inters, tol, FlowSol);
  ...
  for (int i_mpi = 0; i_mpi < FlowSol->n_mpi_inters; i_mpi++) {
    ...
    if (f2nv(i) == 2) {
      FlowSol->mesh_mpi_inters(0).set_mpi(i_seg_mpi, ctype(ic_l), local_c(ic_l),
                                          f2loc_f(i, 0), rot_tag_mpi(i_mpi),
                                          FlowSol);
      i_seg_mpi++;
    } else if (f2nv(i) == 3) {
      FlowSol->mesh_mpi_inters(1).set_mpi(i_tri_mpi, ctype(ic_l), local_c(ic_l),
                                          f2loc_f(i, 0), rot_tag_mpi(i_mpi),
                                          FlowSol);
      i_tri_mpi++;
    } else if (f2nv(i) == 4) {
      FlowSol->mesh_mpi_inters(2).set_mpi(i_quad_mpi, ctype(ic_l),
                                          local_c(ic_l), f2loc_f(i, 0),
                                          rot_tag_mpi(i_mpi), FlowSol);
      i_quad_mpi++;
    }
  }
...
#  endif
  ...
  for (int i = 0; i < FlowSol->num_inters; i++) {
    ...
    if (bctype_f != 10) {
      if (bctype_f == 0)  // internal interface
      {
        if (ic_r == -1) {
          FatalError(
              "Error: Interior interface has i_cell_right=-1. Should not be "
              "here, exiting");
        }
        n_int_inters++;
  ...
  for (int i = 0; i < FlowSol->num_inters; i++) {
    ...
    if (bctype_f != 10)  // internal or boundary edge
    {
      if (bctype_f == 0) {
        if (f2nv(i) == 2) {
          FlowSol->mesh_int_inters(0).set_interior(
              i_seg_int, ctype(ic_l), ctype(ic_r), local_c(ic_l), local_c(ic_r),
              f2loc_f(i, 0), f2loc_f(i, 1), rot_tag(i), FlowSol);
          i_seg_int++;
        }
        if (f2nv(i) == 3) {
          FlowSol->mesh_int_inters(1).set_interior(
              i_tri_int, ctype(ic_l), ctype(ic_r), local_c(ic_l), local_c(ic_r),
              f2loc_f(i, 0), f2loc_f(i, 1), rot_tag(i), FlowSol);
          i_tri_int++;
        }
        if (f2nv(i) == 4) {
          FlowSol->mesh_int_inters(2).set_interior(
              i_quad_int, ctype(ic_l), ctype(ic_r), local_c(ic_l),
              local_c(ic_r), f2loc_f(i, 0), f2loc_f(i, 1), rot_tag(i), FlowSol);
          i_quad_int++;
        }
      } else  // boundary face other than cyclic face
      {
        ...
        {
          if (f2nv(i) == 2) {
            FlowSol->mesh_bdy_inters(0).set_boundary(i_seg_bdy, bctype_f,
                                                     ctype(ic_l), local_c(ic_l),
                                                     f2loc_f(i, 0), FlowSol);
            i_seg_bdy++;
          } else if (f2nv(i) == 3) {
            FlowSol->mesh_bdy_inters(1).set_boundary(i_tri_bdy, bctype_f,
                                                     ctype(ic_l), local_c(ic_l),
                                                     f2loc_f(i, 0), FlowSol);
            i_tri_bdy++;
          } else if (f2nv(i) == 4) {
            FlowSol->mesh_bdy_inters(2).set_boundary(i_quad_bdy, bctype_f,
                                                     ctype(ic_l), local_c(ic_l),
                                                     f2loc_f(i, 0), FlowSol);
            i_quad_bdy++;
          }
  ...
  if (run_input.motion) Mesh.ic2loc_c = local_c;
  ...
  if (run_input.wall_model > 0 or run_input.turb_model > 0) {
    ...
    for (int i = 0; i < FlowSol->num_inters; i++) {
      ...
      if (bctype_f == 11 || bctype_f == 12 || bctype_f == 13 ||
          bctype_f == 14) {
        // segs
    ...
    MPI_Allreduce(&n_seg_noslip_inters, &n_global_seg_noslip_inters, 1, MPI_INT,
                  MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&n_tri_noslip_inters, &n_global_tri_noslip_inters, 1, MPI_INT,
                  MPI_SUM, MPI_COMM_WORLD);
    MPI_Allreduce(&n_quad_noslip_inters, &n_global_quad_noslip_inters, 1,
                  MPI_INT, MPI_SUM, MPI_COMM_WORLD);

    MPI_Allreduce(&n_seg_noslip_inters, &max_seg_noslip_inters, 1, MPI_INT,
                  MPI_MAX, MPI_COMM_WORLD);
    MPI_Allreduce(&n_tri_noslip_inters, &max_tri_noslip_inters, 1, MPI_INT,
                  MPI_MAX, MPI_COMM_WORLD);
    MPI_Allreduce(&n_quad_noslip_inters, &max_quad_noslip_inters, 1, MPI_INT,
                  MPI_MAX, MPI_COMM_WORLD);

    MPI_Allgather(&n_seg_noslip_inters, 1, MPI_INT,
                  n_seg_inters_array.get_ptr_cpu(), 1, MPI_INT, MPI_COMM_WORLD);
    MPI_Allgather(&n_tri_noslip_inters, 1, MPI_INT,
                  n_tri_inters_array.get_ptr_cpu(), 1, MPI_INT, MPI_COMM_WORLD);
    MPI_Allgather(&n_quad_noslip_inters, 1, MPI_INT,
                  n_quad_inters_array.get_ptr_cpu(), 1, MPI_INT,
                  MPI_COMM_WORLD);
    ...
    FlowSol->loc_noslip_bdy(0).setup(n_fpts_per_inter_seg, n_seg_noslip_inters,
                                     FlowSol->n_dims);
    FlowSol->loc_noslip_bdy(1).setup(n_fpts_per_inter_tri, n_tri_noslip_inters,
                                     FlowSol->n_dims);
    FlowSol->loc_noslip_bdy(2).setup(n_fpts_per_inter_quad,
                                     n_quad_noslip_inters, FlowSol->n_dims);
      ...
      if (bctype_f == 11 || bctype_f == 12 || bctype_f == 13 ||
        ...
        if (f2nv(i) == 2) {
          for (int j = 0; j < n_fpts_per_inter_seg; j++) {
            for (int k = 0; k < FlowSol->n_dims; k++) {
              // find coordinates
              FlowSol->loc_noslip_bdy(0)(j, n_seg_noslip_inters, k) =
                  *get_loc_fpts_ptr_cpu(ctype(ic_l), local_c(ic_l),
                                        f2loc_f(i, 0), j, k, FlowSol);
            }
          }
          n_seg_noslip_inters++;
        }
        ...
        if (f2nv(i) == 3) {
          for (int j = 0; j < n_fpts_per_inter_tri; j++) {
            for (int k = 0; k < FlowSol->n_dims; k++) {
              FlowSol->loc_noslip_bdy(1)(j, n_tri_noslip_inters, k) =
                  *get_loc_fpts_ptr_cpu(ctype(ic_l), local_c(ic_l),
                                        f2loc_f(i, 0), j, k, FlowSol);
            }
          }
        ...
        if (f2nv(i) == 4) {
          for (int j = 0; j < n_fpts_per_inter_quad; j++) {
            for (int k = 0; k < FlowSol->n_dims; k++) {
              FlowSol->loc_noslip_bdy(2)(j, n_quad_noslip_inters, k) =
                  *get_loc_fpts_ptr_cpu(ctype(ic_l), local_c(ic_l),
                                        f2loc_f(i, 0), j, k, FlowSol);
            }
          }
    ...
    FlowSol->loc_noslip_bdy_global(0).setup(n_fpts_per_inter_seg,
                                            max_seg_noslip_inters,
                                            FlowSol->nproc * FlowSol->n_dims);

    FlowSol->loc_noslip_bdy_global(1).setup(n_fpts_per_inter_tri,
                                            max_tri_noslip_inters,
                                            FlowSol->nproc * FlowSol->n_dims);

    FlowSol->loc_noslip_bdy_global(2).setup(n_fpts_per_inter_quad,
                                            max_quad_noslip_inters,
                                            FlowSol->nproc * FlowSol->n_dims);
    ...
    MPI_Allgather(FlowSol->loc_noslip_bdy(0).get_ptr_cpu(), buf, MPI_DOUBLE,
                  FlowSol->loc_noslip_bdy_global(0).get_ptr_cpu(), buf,
                  MPI_DOUBLE, MPI_COMM_WORLD);

    buf = max_tri_noslip_inters * n_fpts_per_inter_tri * FlowSol->n_dims;

    MPI_Allgather(FlowSol->loc_noslip_bdy(1).get_ptr_cpu(), buf, MPI_DOUBLE,
                  FlowSol->loc_noslip_bdy_global(1).get_ptr_cpu(), buf,
                  MPI_DOUBLE, MPI_COMM_WORLD);

    buf = max_quad_noslip_inters * n_fpts_per_inter_quad * FlowSol->n_dims;

    MPI_Allgather(FlowSol->loc_noslip_bdy(2).get_ptr_cpu(), buf, MPI_DOUBLE,
                  FlowSol->loc_noslip_bdy_global(2).get_ptr_cpu(), buf,
                  MPI_DOUBLE, MPI_COMM_WORLD);

    // Calculate distance of every solution point to nearest point on no-slip
    // boundary for every partition
    for (int i = 0; i < FlowSol->n_ele_types; i++)
      FlowSol->mesh_eles(i)->calc_wall_distance_parallel(
          n_seg_inters_array, n_tri_inters_array, n_quad_inters_array,
          FlowSol->loc_noslip_bdy_global, FlowSol->nproc);
    ...
    // boundary
    for (int i = 0; i < FlowSol->n_ele_types; i++)
      FlowSol->mesh_eles(i)->calc_wall_distance(
          n_seg_noslip_inters, n_tri_noslip_inters, n_quad_noslip_inters,
          FlowSol->loc_noslip_bdy);
...
void ReadMesh(std::string &in_file_name, array<double> &out_xv, array<int> &out_c2v,
              array<int> &out_c2n_v, array<int> &out_ctype,
              array<int> &out_ic2icg, array<int> &out_iv2ivg, int &out_n_cells,
              int &out_n_verts, int &out_n_verts_global,
  ...
  if (run_input.mesh_format == 0) {  // Gambit
    read_connectivity_gambit(in_file_name, out_n_cells, out_c2v, out_c2n_v,
                             out_ctype, out_ic2icg, FlowSol);
  } else if (run_input.mesh_format == 1) {  // Gmsh
    read_connectivity_gmsh(in_file_name, out_n_cells, out_c2v, out_c2n_v,
                           out_ctype, out_ic2icg, FlowSol);
  } else {
  ...
  if (FlowSol->nproc != 1)
    repartition_mesh(out_n_cells, out_c2v, out_c2n_v, out_ctype, out_ic2icg,
                     FlowSol);
#endif

  if (FlowSol->rank == 0) std::cout << "reading vertices" << std::endl;

  // Call method to create array iv2ivg and modify c2v using local vertex
  // indices
  array<int> iv2ivg;
  int n_verts;
  ...
  if (run_input.mesh_format == 0) {
    read_vertices_gambit(in_file_name, n_verts, out_n_verts_global, out_iv2ivg,
                         out_xv, FlowSol);
  } else if (run_input.mesh_format == 1) {
    read_vertices_gmsh(in_file_name, n_verts, out_n_verts_global, out_iv2ivg,
                       out_xv, FlowSol);
  } else {
    FatalError("Mesh format not recognized");
  }
...
void ReadBound(std::string &in_file_name, array<int> &in_c2v, array<int> &in_c2n_v,
               array<int> &in_c2f, array<int> &in_f2v, array<int> &in_f2nv,
               array<int> &in_ctype, array<int> &out_bctype,
               array<array<int>> &out_boundpts, array<int> &out_bc_list,
               array<int> &out_bound_flag, array<int> &in_ic2icg,
               array<int> &in_icvsta, array<int> &in_icvert,
               array<int> &in_iv2ivg, int &in_n_cells, int &in_n_verts,
               struct solution *FlowSol) {
  ...
  if (run_input.mesh_format == 0) {
    ...
    read_boundary_gambit(in_file_name, in_n_cells, in_ic2icg, out_bctype,
                         out_bc_list, bccells, bcfaces);
    if (run_input.motion != 0)
      create_boundpts(out_boundpts, out_bc_list, out_bound_flag, bccells,
                      bcfaces, in_c2f, in_f2v, in_f2nv);
  } else if (run_input.mesh_format == 1) {
    read_boundary_gmsh(in_file_name, in_n_cells, in_ic2icg, in_c2v, in_c2n_v,
                       out_bctype, out_bc_list, out_bound_flag, out_boundpts,
                       in_iv2ivg, in_n_verts, in_ctype, in_icvsta, in_icvert,
                       FlowSol);
  } else {
    FatalError("Mesh format not recognized");
  }
...
void create_boundpts(array<array<int>> &out_boundpts, array<int> &in_bclist,
                     array<int> &out_bound_flag, array<array<int>> &in_bccells,
                     array<array<int>> &in_bcfaces, array<int> &in_c2f,
                     array<int> &in_f2v, array<int> &in_f2nv) {
  ...
  for (int i = 0; i < n_bcs; i++) {
    ...
    int j = 0;
    for (std::set<int>::iterator it = Bounds(i).begin(); it != Bounds(i).end();
         it++) {
      out_boundpts(i)(j) = (*it);
      j++;
    }
...
void read_boundary_gambit(std::string &in_file_name, int &in_n_cells,
                          array<int> &in_ic2icg, array<int> &out_bctype,
                          array<int> &out_bclist,
                          array<array<int>> &out_bccells,
                          array<array<int>> &out_bcfaces) {
  // input: ic2icg
  // output: bctype
  ...
  for (int i = 0; i < in_n_cells; i++) cell_list(i) = in_ic2icg(i);
  ...
  if (!mesh_file) FatalError("Unable to open mesh file");
  ...
  // std::cout << "Gambit mesh specs from header: " << ", " << n_verts_global << ", "
  // << n_cells_global << ", " << n_mats << ", " << n_bcs << ", " << dummy <<
  // std::endl;
  ...
  for (int i = 0; i < n_verts_global; i++) mesh_file.getline(buf, BUFSIZ);
  ...
  for (int i = 0; i < n_bcs; i++) {
    ...
    for (int bf = 0; bf < bcNF; bf++) {
      ...
      if (eleType == 2 || eleType == 3) real_face = k - 1;
      // Hex
      else if (eleType == 4) {
        if (k == 1)
          real_face = 0;
...
void read_boundary_gmsh(std::string &in_file_name, int &in_n_cells,
                        array<int> &in_ic2icg, array<int> &in_c2v,
                        array<int> &in_c2n_v, array<int> &out_bctype,
                        array<int> &out_bclist, array<int> &out_bound_flag,
                        array<array<int>> &out_boundpts, array<int> &in_iv2ivg,
                        int in_n_verts, array<int> &in_ctype,
                        array<int> &in_icvsta, array<int> &in_icvert,
                        struct solution *FlowSol) {
  std::string str;
  ...
  for (int i = 0; i < n_entities; i++) {
    ...
    mesh_file >> bcid;
    for (int tag = 0; tag < ntags - 1; tag++) mesh_file >> dummy;
    ...
    {
      num_face_vert = 4;
      mesh_file >> vlist_bound(0) >> vlist_bound(1) >> vlist_bound(3) >>
          vlist_bound(2);
    ...
    {
      num_face_vert = 9;
      mesh_file >> vlist_bound(0) >> vlist_bound(2) >> vlist_bound(8) >>
          vlist_bound(6);
      mesh_file >> vlist_bound(1) >> vlist_bound(5) >> vlist_bound(7) >>
          vlist_bound(3) >> vlist_bound(4);
    } else {
    ...
    for (int j = 0; j < num_face_vert; j++) {
      vlist_local(j) =
          index_locate_int(vlist_bound(j), in_iv2ivg.get_ptr_cpu(), in_n_verts);
      if (vlist_local(j) == -1) belong_to_proc = false;
    ...
    if (belong_to_proc) {
      ...
      for (int ind = sta_ind; ind <= end_ind; ind++) {
        ...
        for (int k = 0; k < FlowSol->num_f_per_c(in_ctype(ic)); k++) {
          ...
          get_vlist_loc_face(in_ctype(ic), in_c2n_v(ic), k, vlist_cell,
                             num_v_per_f);

          if (num_v_per_f != num_face_vert) continue;
      ...
      if (found == 0) {
        std::cout << "vlist_bound(2)=" << vlist_bound(2)
             << " vlist_bound(3)=" << vlist_bound(3) << std::endl;
        FatalError(
            "All nodes of boundary face belong to processor but could not find "
            "the coresponding faces");
      }
...
void read_vertices_gambit(std::string &in_file_name, int in_n_verts,
                          int &out_n_verts_global, array<int> &in_iv2ivg,
                          array<double> &out_xv, solution *FlowSol) {
  ...
  if (!mesh_file) FatalError("Could not open mesh file");
...
void read_vertices_gmsh(std::string &in_file_name, int in_n_verts,
                        int &out_n_verts_global, array<int> &in_iv2ivg,
                        array<double> &out_xv, struct solution *FlowSol) {
  std::string str;
...
void create_iv2ivg(array<int> &inout_iv2ivg, array<int> &inout_c2v,
                   int &out_n_verts, int in_n_cells) {
  array<int> vrtlist(in_n_cells * MAX_V_PER_C);
  array<int> temp(in_n_cells * MAX_V_PER_C);

  int icount = 0;
  for (int i = 0; i < in_n_cells; i++)
    for (int j = 0; j < MAX_V_PER_C; j++) vrtlist(icount++) = inout_c2v(i, j);

  // Sort the vertices
  qsort(vrtlist.get_ptr_cpu(), in_n_cells * MAX_V_PER_C, sizeof(int),
        compare_ints);
  ...
  for (int i = 0; i < out_n_verts; i++) inout_iv2ivg(i) = temp(i);
  ...
  for (int i = 0; i < in_n_cells; i++) {
    for (int j = 0; j < MAX_V_PER_C; j++) {
      if (inout_c2v(i, j) != -1) {
        int index = index_locate_int(inout_c2v(i, j),
                                     inout_iv2ivg.get_ptr_cpu(), out_n_verts);
        if (index == -1) {
...
void read_connectivity_gambit(std::string &in_file_name, int &out_n_cells,
                              array<int> &out_c2v, array<int> &out_c2n_v,
                              array<int> &out_ctype, array<int> &out_ic2icg,
                              struct solution *FlowSol) {
  int n_verts_global, n_cells_global;
  int dummy, dummy2;
  ...
  out_c2v.setup(out_n_cells,
                MAX_V_PER_C);    // stores the vertices making that cell
  ...
  for (int i = 0; i < out_n_cells; i++) {
    ...
    out_ctype(i) = -1;
    out_ic2icg(i) = -1;
    for (int k = 0; k < MAX_V_PER_C; k++) out_c2v(i, k) = -1;
  }
  ...
  int eleType;
  for (int i = 0; i < out_n_cells; i++) {
    //  ctype is the element type:  1=edge, 2=quad, 3=tri, 4=brick, 5=wedge,
    //  6=tet, 7=pyramid
    ...
    if (out_ctype(i) == TRI) {
      if (out_c2n_v(i) == 3)  // linear triangle
        ...
        mesh_file >> out_c2v(i, 0) >> out_c2v(i, 3) >> out_c2v(i, 1) >>
            out_c2v(i, 4) >> out_c2v(i, 2) >> out_c2v(i, 5);
      else
    ...
    // quad
      ...
      if (out_c2n_v(i) == 4)  // linear quadrangle
        mesh_file >> out_c2v(i, 0) >> out_c2v(i, 1) >> out_c2v(i, 3) >>
            out_c2v(i, 2);
      else if (out_c2n_v(i) == 8)  // quadratic quad
        mesh_file >> out_c2v(i, 0) >> out_c2v(i, 4) >> out_c2v(i, 1) >>
            out_c2v(i, 5) >> out_c2v(i, 2) >> out_c2v(i, 6) >> out_c2v(i, 3) >>
            out_c2v(i, 7);
      else
    ...
    // tet
      ...
      {
        mesh_file >> out_c2v(i, 0) >> out_c2v(i, 4) >> out_c2v(i, 1) >>
            out_c2v(i, 5) >> out_c2v(i, 7);
        mesh_file >> out_c2v(i, 2) >> out_c2v(i, 6) >> out_c2v(i, 9) >>
            out_c2v(i, 8) >> out_c2v(i, 3);
      } else
    ...
    // prisms
      ...
      if (out_c2n_v(i) == 6)  // linear prism
        mesh_file >> out_c2v(i, 0) >> out_c2v(i, 1) >> out_c2v(i, 2) >>
            out_c2v(i, 3) >> out_c2v(i, 4) >> out_c2v(i, 5);
      else if (out_c2n_v(i) == 15)  // quadratic prism
        mesh_file >> out_c2v(i, 0) >> out_c2v(i, 6) >> out_c2v(i, 1) >>
            out_c2v(i, 8) >> out_c2v(i, 7) >> out_c2v(i, 2) >> out_c2v(i, 9) >>
            out_c2v(i, 10) >> out_c2v(i, 11) >> out_c2v(i, 3) >>
            out_c2v(i, 12) >> out_c2v(i, 4) >> out_c2v(i, 14) >>
            out_c2v(i, 13) >> out_c2v(i, 5);
      else
    ...
    // hexa
      ...
      if (out_c2n_v(i) == 8)  // linear hexas
        mesh_file >> out_c2v(i, 0) >> out_c2v(i, 2) >> out_c2v(i, 4) >>
            out_c2v(i, 6) >> out_c2v(i, 1) >> out_c2v(i, 3) >> out_c2v(i, 5) >>
            out_c2v(i, 7);
      else if (out_c2n_v(i) == 20)  // quadratic hexas
        mesh_file >> out_c2v(i, 0) >> out_c2v(i, 11) >> out_c2v(i, 3) >>
            out_c2v(i, 12) >> out_c2v(i, 15) >> out_c2v(i, 4) >>
            out_c2v(i, 19) >> out_c2v(i, 7) >> out_c2v(i, 8) >>
            out_c2v(i, 10) >> out_c2v(i, 16) >> out_c2v(i, 18) >>
            out_c2v(i, 1) >> out_c2v(i, 9) >> out_c2v(i, 2) >> out_c2v(i, 13) >>
            out_c2v(i, 14) >> out_c2v(i, 5) >> out_c2v(i, 17) >> out_c2v(i, 6);
      else
        FatalError("Hexa element type not implemented");
    } else {
      std::cout << "Element Type = " << out_ctype(i) << std::endl;
      FatalError(
          "Haven't implemented this element type in gambit_meshreader3, "
          "exiting ");
    }
    mesh_file.getline(buf, BUFSIZ);  // skip end of line
...
void read_connectivity_gmsh(std::string &in_file_name, int &out_n_cells,
                            array<int> &out_c2v, array<int> &out_c2n_v,
                            array<int> &out_ctype, array<int> &out_ic2icg,
                            struct solution *FlowSol) {
  ...
  for (int i = 0; i < n_entities; i++) {
    ...
    if (strstr(bcTXT[bcid], "FLUID")) icount++;
  ...
  // ctype is the element type:  for HiFiLES: 0=tri, 1=quad, 2=tet, 3=prism,
  // 4=hex For Gmsh node ordering, see:
  ...
  for (int k = 0; k < n_entities; k++) {
    ...
    if (strstr(bcTXT[bcid], "FLUID")) {
      if (icount >= kstart && i < out_n_cells)  // Read this cell
      {
        ...
        } else if (elmtype == 3 || elmtype == 16 || elmtype == 10)  // Quad
        {
          ...
          } else if (elmtype == 16)  // quadratic quadrangle
          {
            ...
            mesh_file >> out_c2v(i, 0) >> out_c2v(i, 1) >> out_c2v(i, 2) >>
                out_c2v(i, 3) >> out_c2v(i, 4) >> out_c2v(i, 5) >>
                out_c2v(i, 6) >> out_c2v(i, 7);
          } else if (elmtype == 10)  // quadratic quadrangle
          {
            out_c2n_v(i) = 9;
            // not sure this is correct
            mesh_file >> out_c2v(i, 0) >> out_c2v(i, 2) >> out_c2v(i, 8) >>
                out_c2v(i, 6) >> out_c2v(i, 1) >> out_c2v(i, 5) >>
                out_c2v(i, 7) >> out_c2v(i, 3) >> out_c2v(i, 4);
          }
        } else if (elmtype == 4 || elmtype == 11)  // Tetrahedron
        {
          ...
          {
            out_c2n_v(i) = 10;
            // mesh_file >> out_c2v(i,0) >> out_c2v(i,8) >> out_c2v(i,5) >>
            // out_c2v(i,2) >> out_c2v(i,3); mesh_file >> out_c2v(i,6) >>
            // out_c2v(i,7) >> out_c2v(i,4) >> out_c2v(i,9) >> out_c2v(i,1);
            mesh_file >> out_c2v(i, 0) >> out_c2v(i, 5) >> out_c2v(i, 4) >>
                out_c2v(i, 2) >> out_c2v(i, 8);
            mesh_file >> out_c2v(i, 1) >> out_c2v(i, 7) >> out_c2v(i, 3) >>
                out_c2v(i, 9) >> out_c2v(i, 6);
          }
        } else if (elmtype == 5 || elmtype == 12)  // Hexahedron
        {
          ...
          if (elmtype == 5)  // linear quadrangle
          {
            ...
            mesh_file >> out_c2v(i, 0) >> out_c2v(i, 1) >> out_c2v(i, 3) >>
                out_c2v(i, 2);
            mesh_file >> out_c2v(i, 4) >> out_c2v(i, 5) >> out_c2v(i, 7) >>
                out_c2v(i, 6);
          } else if (elmtype == 12)  // 27-node quadratic hexahedron
          {
            ...
            mesh_file >> out_c2v(i, 0) >> out_c2v(i, 1) >> out_c2v(i, 2) >>
                ...
                out_c2v(i, 6) >> out_c2v(i, 7);
            // edges
            mesh_file >> out_c2v(i, 8) >> out_c2v(i, 9) >> out_c2v(i, 10) >>
                out_c2v(i, 11) >> out_c2v(i, 12) >> out_c2v(i, 13);
            mesh_file >> out_c2v(i, 14) >> out_c2v(i, 15) >> out_c2v(i, 16) >>
                out_c2v(i, 17) >> out_c2v(i, 18) >> out_c2v(i, 19);
            // faces
            mesh_file >> out_c2v(i, 20) >> out_c2v(i, 21) >> out_c2v(i, 22) >>
                out_c2v(i, 23) >> out_c2v(i, 24) >> out_c2v(i, 25);
            // volume
      ...
      {
        mesh_file.getline(
            buf, BUFSIZ);  // skip line, cell doesn't belong to this processor
      }
...
#ifdef _MPI
void repartition_mesh(int &out_n_cells, array<int> &out_c2v,
                      array<int> &out_c2n_v, array<int> &out_ctype,
                      array<int> &out_ic2icg, struct solution *FlowSol) {
  ...
  MPI_Allgather(&klocal, 1, MPI_INT, kprocs.get_ptr_cpu(), 1, MPI_INT,
                MPI_COMM_WORLD);

  int kstart = 0;
  for (int p = 0; p < FlowSol->nproc - 1; p++) kstart += kprocs(p);
  ...
  for (int i = 0; i < klocal; i++) tpwgts[i] = 1. / (float)FlowSol->nproc;
  ...
  ParMETIS_V3_PartMeshKway(elmdist, eptr, eind, elmwgt, &wgtflag, &numflag,
                           &ncon, &ncommonnodes, &nparts, (real_t *)tpwgts,
                           (real_t *)ubvec, options, &edgecut, part, &comm);
  ...
  for (int i = 0; i < klocal; i++) ++outK[part[i]];
  ...
  MPI_Allreduce(outK, newkprocs, FlowSol->nproc, MPI_INT, MPI_SUM,
                MPI_COMM_WORLD);

  int totalinK = 0;
  for (int p = 0; p < FlowSol->nproc; p++) totalinK += inK[p];

  // declare new array c2v
  int **new_c2v = (int **)calloc(totalinK, sizeof(int *));
  new_c2v[0] = (int *)calloc(totalinK * MAX_V_PER_C, sizeof(int));
  for (int i = 1; i < totalinK; i++) new_c2v[i] = new_c2v[i - 1] + MAX_V_PER_C;
  ...
  MPI_Request *inrequests =
      (MPI_Request *)calloc(FlowSol->nproc, sizeof(MPI_Request));
  MPI_Request *inrequests_c2n_v =
      (MPI_Request *)calloc(FlowSol->nproc, sizeof(MPI_Request));
  MPI_Request *inrequests_ctype =
      (MPI_Request *)calloc(FlowSol->nproc, sizeof(MPI_Request));
  MPI_Request *inrequests_ic2icg =
      (MPI_Request *)calloc(FlowSol->nproc, sizeof(MPI_Request));

  MPI_Request *outrequests =
      (MPI_Request *)calloc(FlowSol->nproc, sizeof(MPI_Request));
  MPI_Request *outrequests_c2n_v =
      (MPI_Request *)calloc(FlowSol->nproc, sizeof(MPI_Request));
  MPI_Request *outrequests_ctype =
      (MPI_Request *)calloc(FlowSol->nproc, sizeof(MPI_Request));
  MPI_Request *outrequests_ic2icg =
      (MPI_Request *)calloc(FlowSol->nproc, sizeof(MPI_Request));

  MPI_Status *instatus =
      (MPI_Status *)calloc(FlowSol->nproc, sizeof(MPI_Status));
  ...
  int cnt = 0;
  for (int p = 0; p < FlowSol->nproc; p++) {
    MPI_Irecv(new_c2v[cnt], MAX_V_PER_C * inK[p], MPI_INT, p, 666 + p,
              MPI_COMM_WORLD, inrequests + p);
    MPI_Irecv(&new_c2n_v[cnt], inK[p], MPI_INT, p, 666 + p, MPI_COMM_WORLD,
              inrequests_c2n_v + p);
    MPI_Irecv(&new_ctype[cnt], inK[p], MPI_INT, p, 666 + p, MPI_COMM_WORLD,
              inrequests_ctype + p);
    MPI_Irecv(&new_ic2icg[cnt], inK[p], MPI_INT, p, 666 + p, MPI_COMM_WORLD,
              inrequests_ic2icg + p);
    cnt = cnt + inK[p];
  }
  ...
  for (int p = 0; p < FlowSol->nproc; p++) {
    ...
    MPI_Isend(outlist[p], MAX_V_PER_C * outK[p], MPI_INT, p,
              666 + FlowSol->rank, MPI_COMM_WORLD, outrequests + p);
    MPI_Isend(outlist_c2n_v[p], outK[p], MPI_INT, p, 666 + FlowSol->rank,
              MPI_COMM_WORLD, outrequests_c2n_v + p);
    MPI_Isend(outlist_ctype[p], outK[p], MPI_INT, p, 666 + FlowSol->rank,
              MPI_COMM_WORLD, outrequests_ctype + p);
    MPI_Isend(outlist_ic2icg[p], outK[p], MPI_INT, p, 666 + FlowSol->rank,
              MPI_COMM_WORLD, outrequests_ic2icg + p);
  }
  ...
  for (int i = 0; i < totalinK; i++) {
    for (int j = 0; j < MAX_V_PER_C; j++) out_c2v(i, j) = new_c2v[i][j];
...
void CompConnectivity(array<int> &in_c2v, array<int> &in_c2n_v,
                      array<int> &in_ctype, array<int> &out_c2f,
                      array<int> &out_c2e, array<int> &out_f2c,
                      array<int> &out_f2loc_f, array<int> &out_f2v,
                      array<int> &out_f2nv, array<int> &out_e2v,
                      array<int> &out_v2n_e, array<array<int>> &out_v2e,
                      array<int> &out_rot_tag, array<int> &out_unmatched_faces,
                      int &out_n_unmatched_faces, array<int> &out_icvsta,
                      array<int> &out_icvert, int &out_n_faces,
                      int &out_n_edges, struct solution *FlowSol) {
  // inputs:   in_c2v (clls to vertex) , in_ctype (type of cell)
  // outputs:  f2c (face to cell), c2f (cell to face), f2loc_f (face to local
  // face index of right and left cells), rot_tag,  n_faces (number of faces in
  // the mesh)
  ...
  array<int> vlist_loc(MAX_V_PER_F), vlist_loc2(MAX_V_PER_F),
      vlist_glob(MAX_V_PER_F),
      vlist_glob2(MAX_V_PER_F);  // faces cannot have more than 4 vertices

  array<int> v2n_c;

  /**
   * "Moving pointer" that points to next unassigned entry in icvert (for each
   * vertex) (see descriptions of icvsta & icvert)
   */
  array<int> icvsta2;
  ...
  for (int iv = 0; iv < n_verts; iv++) {
    if (v2n_c(iv) > max_nc) max_nc = v2n_c(iv);
  ...
  if (max_nc > max_cells_per_vert)
    FatalError(
        "ERROR: some vertices are shared by more than max_cells_per_vert");
  ...
  if (FlowSol->n_dims == 3 || run_input.motion != 0) {
    std::vector<int> e2v;
    std::vector<std::set<int>> v2e(n_verts);
    ...
    for (int ic = 0; ic < n_cells; ic++) {
      for (int k = 0; k < num_e_per_c(in_ctype(ic)); k++) {
        found = 0;
        if (out_c2e(ic, k) != -1)
          continue;  // we have counted that face already
        ...
        if (FlowSol->n_dims == 3) {
          ...
          get_vlist_loc_face(in_ctype(ic), in_c2n_v(ic), k, vlist_loc,
                             num_v_per_f);
        }
        ...
        for (int ind = sta_ind; ind <= end_ind; ind++) {
          ...
          for (int k2 = 0; k2 < num_e_per_c(in_ctype(ic2)); k2++) {
            ...
            if (FlowSol->n_dims == 3) {
              ...
              get_vlist_loc_face(in_ctype(ic2), in_c2n_v(ic2), k2, vlist_loc2,
                                 num_v_per_f);
            }
            ...
            if ((vlist_glob(0) == vlist_glob2(0) &&
                 vlist_glob(1) == vlist_glob2(1)) ||
                (vlist_glob(0) == vlist_glob2(1) &&
                 vlist_glob(1) == vlist_glob2(0))) {
    ...
    if (n_verts != out_v2e.get_dim(0))
      FatalError("n_verts & out_v2e not same size!!");
    for (int iv = 0; iv < n_verts; iv++) {
  ...
  for (int ic = 0; ic < n_cells; ic++) {
    ...
    for (int k = 0; k < FlowSol->num_f_per_c(in_ctype(ic)); k++) {
      ...
      if (found == 1) {
        ...
        for (int i = 0; i < num_v_per_f; i++) out_f2v(iface, i) = vlist_glob(i);

        out_f2nv(iface) = num_v_per_f;
        out_rot_tag(iface) = rtag;
        iface++;
      } else {
        // If loops over ic2 and k2 were unsuccesful, it means that face is not
        // shared by two cells Then continue here and set f2c( ,1) = -1
...
void get_vlist_loc_edge(int &in_ctype, int &in_n_spts, int &in_edge,
                        array<int> &out_vlist_loc) {
...
void get_vlist_loc_face(int &in_ctype, int &in_n_spts, int &in_face,
                        array<int> &out_vlist_loc, int &num_v_per_f) {
...
void compare_faces(array<int> &vlist1, array<int> &vlist2, int &num_v_per_f,
                   int &found, int &rtag) {
  /* Looking at a face from *inside* the cell, the nodes *must* be numbered in
   * *CW* order (this is in agreement with Gambit; Gmsh does not care about
   * local face numberings)
   *
   * The variable 'rtag' matches up the local node numbers of two overlapping
   * faces from different cells (specifically, it is which node from face 2
   * matches node 0 from face 1)
   */
  ...
  {
    // rot_tag==0
    if (vlist1(0) == vlist2(0) && vlist1(1) == vlist2(2) &&
        vlist1(2) == vlist2(1)) {
      rtag = 0;
      found = 1;
    }
    // rot_tag==1
    else if (vlist1(0) == vlist2(2) && vlist1(1) == vlist2(1) &&
             vlist1(2) == vlist2(0)) {
      rtag = 1;
      found = 1;
    }
    // rot_tag==2
    else if (vlist1(0) == vlist2(1) && vlist1(1) == vlist2(0) &&
             vlist1(2) == vlist2(2)) {
      rtag = 2;
      found = 1;
    } else
  ...
  {
    // rot_tag==0
    if (vlist1(0) == vlist2(1) && vlist1(1) == vlist2(0) &&
        vlist1(2) == vlist2(3) && vlist1(3) == vlist2(2)) {
      rtag = 0;
      found = 1;
    }
    // rot_tag==1
    else if (vlist1(0) == vlist2(3) && vlist1(1) == vlist2(2) &&
             vlist1(2) == vlist2(1) && vlist1(3) == vlist2(0)) {
      rtag = 1;
      found = 1;
    }
    // rot_tag==2
    else if (vlist1(0) == vlist2(0) && vlist1(1) == vlist2(3) &&
             vlist1(2) == vlist2(2) && vlist1(3) == vlist2(1)) {
      rtag = 2;
      found = 1;
    }
    // rot_tag==3
    else if (vlist1(0) == vlist2(2) && vlist1(1) == vlist2(1) &&
             vlist1(2) == vlist2(0) && vlist1(3) == vlist2(3)) {
      rtag = 3;
      found = 1;
    } else
      found = 0;
    ...
    FatalError(
        "ERROR: Haven't implemented this face type in compare_face yet....");
  }
}

void compare_faces_boundary(array<int> &vlist1, array<int> &vlist2,
                            int &num_v_per_f, int &found) {
  if (!(num_v_per_f == 2 || num_v_per_f == 3 || num_v_per_f == 4))
    FatalError(
        "Boundary face type not recognized (expecting a linear edge, tri, or "
        "quad)");
...
void compare_cyclic_faces(array<double> &xvert1, array<double> &xvert2,
                          int &num_v_per_f, int &rtag,
                          array<double> &delta_cyclic, double tol,
                          struct solution *FlowSol) {
  int found = 0;
  if (FlowSol->n_dims == 2) {
    found = 1;
    ...
    if (num_v_per_f == 4)  // quad face
    {
      ...
      if ((abs(abs(xvert1(0, 0) - xvert2(1, 0)) - delta_cyclic(0)) < tol &&
      ...
      // vert1(0) matches vert2(3), rot_tag=1
      else if ((abs(abs(xvert1(0, 0) - xvert2(3, 0)) - delta_cyclic(0)) < tol &&
               ...
               (abs(abs(xvert1(0, 0) - xvert2(3, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(3, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(3, 2)) - delta_cyclic(2)) <
                    tol)) {
        rtag = 1;
        found = 1;
      }
      // vert1(0) matches vert2(0), rot_tag=2
      else if ((abs(abs(xvert1(0, 0) - xvert2(0, 0)) - delta_cyclic(0)) < tol &&
               ...
               (abs(abs(xvert1(0, 0) - xvert2(0, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(0, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(0, 2)) - delta_cyclic(2)) <
                    tol)) {
        rtag = 2;
        found = 1;
      }
      // vert1(0) matches vert2(2), rot_tag=3
      else if ((abs(abs(xvert1(0, 0) - xvert2(2, 0)) - delta_cyclic(0)) < tol &&
               ...
               (abs(abs(xvert1(0, 0) - xvert2(2, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(2, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(2, 2)) - delta_cyclic(2)) <
                    tol)) {
        rtag = 3;
        found = 1;
      }
    } else if (num_v_per_f == 3)  // tri face
    {
      ...
      if ((abs(abs(xvert1(0, 0) - xvert2(0, 0)) - delta_cyclic(0)) < tol &&
           abs(abs(xvert1(0, 1) - xvert2(0, 1))) < tol &&
           abs(abs(xvert1(0, 2) - xvert2(0, 2))) < tol) ||
          (abs(abs(xvert1(0, 0) - xvert2(0, 0))) < tol &&
           abs(abs(xvert1(0, 1) - xvert2(0, 1)) - delta_cyclic(1)) < tol &&
      ...
      // vert1(0) matches vert2(1), rot_tag=2
      else if ((abs(abs(xvert1(0, 0) - xvert2(1, 0)) - delta_cyclic(0)) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(1, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(1, 2))) < tol) ||
               (abs(abs(xvert1(0, 0) - xvert2(1, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(1, 1)) - delta_cyclic(1)) < tol &&
               ...
               (abs(abs(xvert1(0, 0) - xvert2(1, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(1, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(1, 2)) - delta_cyclic(2)) <
                    tol)) {
        rtag = 2;
        found = 1;
      }
      ...
      FatalError(
          "ERROR: Haven't implemented this face type in compare_cyclic_face "
          "yet....");
    }
  }

  if (found == 0) FatalError("Could not match vertices in compare faces");
}

bool check_cyclic(array<double> &delta_cyclic,
                  array<double> &loc_center_inter_0,
                  array<double> &loc_center_inter_1, double tol,
                  struct solution *FlowSol) {
  bool output;
  if (FlowSol->n_dims == 3) {
    output = (abs(abs(loc_center_inter_0(0) - loc_center_inter_1(0)) -
                  delta_cyclic(0)) < tol &&
             ...
             (abs(loc_center_inter_0(0) - loc_center_inter_1(0)) < tol &&
              abs(abs(loc_center_inter_0(1) - loc_center_inter_1(1)) -
                  delta_cyclic(1)) < tol &&
              abs(loc_center_inter_0(2) - loc_center_inter_1(2)) < tol) ||

             (abs(loc_center_inter_0(0) - loc_center_inter_1(0)) < tol &&
              abs(loc_center_inter_0(1) - loc_center_inter_1(1)) < tol &&
              abs(abs(loc_center_inter_0(2) - loc_center_inter_1(2)) -
                  delta_cyclic(2)) < tol);
...
void match_mpifaces(array<int> &in_f2v, array<int> &in_f2nv,
                    array<double> &in_xv, array<int> &inout_f_mpi2f,
                    array<int> &out_mpifaces_part, array<double> &delta_cyclic,
                    int n_mpi_faces, double tol, struct solution *FlowSol) {
  // TODO: THIS IS NOT OPTIMAL: GOES AS N^2 operations, try sorting and
  // searching instead (cost will be 2*N*log(N)
  // TODO: TIDY UP
  ...
  for (i = 0; i < n_mpi_faces; i++) matched(i) = 0;

  // Initialize array mpifaces_part to 0
  for (i = 0; i < FlowSol->nproc; i++) out_mpifaces_part(i) = 0;

  // Exchange the number of mpi_faces to receive
  // Create array mpfaces_from
  array<int> mpifaces_from(FlowSol->nproc);
  MPI_Allgather(&n_mpi_faces, 1, MPI_INT, mpifaces_from.get_ptr_cpu(), 1,
  ...
  for (p = 0; p < FlowSol->nproc; p++) {
    if (p == FlowSol->rank) {  // Send data
      for (p2 = 0; p2 < FlowSol->nproc; p2++) {
        if (p2 != FlowSol->rank) {
          ...
          MPI_Send(loc_center_inter.get_ptr_cpu(),
                   FlowSol->n_dims * n_mpi_faces, MPI_DOUBLE, p2,
                   100000 * p + 1000 * p2, MPI_COMM_WORLD);
        }
      }
    ...
    } else                       // Receive data
    {
      ...
      MPI_Recv(in_loc_center_inter.get_ptr_cpu(),
               FlowSol->n_dims * mpifaces_from(p), MPI_DOUBLE, p,
               100000 * p + 1000 * FlowSol->rank, MPI_COMM_WORLD, &instatus);
        ...
        for (iloc = 0; iloc < n_mpi_faces; iloc++) {
          if (!matched(iloc))  // if local edge hasn't been matched yet
          {
            ...
            for (irem = 0; irem < mpifaces_from(p); irem++) {
              ...
              if (check_cyclic(delta_cyclic, loc_center_1, loc_center_2, tol,
                               FlowSol) ||
                  check_cyclic(delta_zero, loc_center_1, loc_center_2, tol,
                               FlowSol)) {
                matched(iloc) = 1;
                out_mpifaces_part(p)++;
                i = old_f_mpi2f(iloc);
  ...
  for (i = 0; i < n_mpi_faces; i++) {
    if (!matched(i)) {
      std::cout << "Some mpi_faces were not matched!!! could try changing tol, "
              "exiting!"
           << std::endl;
...
void find_rot_mpifaces(array<int> &in_f2v, array<int> &in_f2nv,
                       array<double> &in_xv, array<int> &in_f_mpi2f,
                       array<int> &out_rot_tag_mpi, array<int> &mpifaces_part,
                       array<double> delta_cyclic, int n_mpi_faces, double tol,
                       struct solution *FlowSol) {
  ...
  array<double> xvert1(
      MAX_V_PER_F,
      FlowSol->n_dims);  // 4 is maximum number of vertices per face
  ...
  MPI_Request *mpi_in_requests =
      (MPI_Request *)malloc(request_count * sizeof(MPI_Request));
  ...
  for (i_mpi = 0; i_mpi < n_mpi_faces; i_mpi++) {
    ...
    for (k = 0; k < in_f2nv(iglob); k++) n_vert_out++;
  }
  ...
  for (int p = 0; p < FlowSol->nproc; p++) {
    ...
    if (Nout) {
      MPI_Isend(xyz_vert_out.get_ptr_cpu(0, sk), Nout * FlowSol->n_dims,
                MPI_DOUBLE, p, 6666 + p, MPI_COMM_WORLD,
                &mpi_out_requests[request_count]);
      MPI_Irecv(xyz_vert_in.get_ptr_cpu(0, sk), Nout * FlowSol->n_dims,
                MPI_DOUBLE, p, 6666 + FlowSol->rank, MPI_COMM_WORLD,
                &mpi_in_requests[request_count]);
      sk += Nout;
      Nmess++;
      request_count++;
    }
  ...
  for (i_mpi = 0; i_mpi < n_mpi_faces; i_mpi++) {
    ...
    compare_mpi_faces(loc_vert_0, loc_vert_1, in_f2nv(iglob), rtag,
                      delta_cyclic, tol, FlowSol);
...
// method to compare two faces and check if they match
void compare_mpi_faces(array<double> &xvert1, array<double> &xvert2,
                       int &num_v_per_f, int &rtag, array<double> &delta_cyclic,
                       double tol, struct solution *FlowSol) {
  int found = 0;
  if (FlowSol->n_dims == 2) {
    found = 1;
    rtag = 0;
    ...
    if (num_v_per_f == 4)  // quad face
    {
      // Determine rot_tag based on xvert1(0)
      // vert1(0) matches vert2(1), rot_tag=0
      if ((abs(abs(xvert1(0, 0) - xvert2(1, 0))) < tol &&
           abs(abs(xvert1(0, 1) - xvert2(1, 1))) < tol &&
           abs(abs(xvert1(0, 2) - xvert2(1, 2))) < tol) ||
      ...
      // vert1(0) matches vert2(3), rot_tag=1
      else if ((abs(abs(xvert1(0, 0) - xvert2(3, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(3, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(3, 2))) < tol) ||
      ...
      // vert1(0) matches vert2(0), rot_tag=2
      else if ((abs(abs(xvert1(0, 0) - xvert2(0, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(0, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(0, 2))) < tol) ||
      ...
      // vert1(0) matches vert2(2), rot_tag=3
      else if ((abs(abs(xvert1(0, 0) - xvert2(2, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(2, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(2, 2))) < tol) ||
    ...
    {
      // vert1(0) matches vert2(0), rot_tag=0
      if ((abs(abs(xvert1(0, 0) - xvert2(0, 0))) < tol &&
           abs(abs(xvert1(0, 1) - xvert2(0, 1))) < tol &&
           abs(abs(xvert1(0, 2) - xvert2(0, 2))) < tol) ||
          (abs(abs(xvert1(0, 0) - xvert2(0, 0)) - delta_cyclic(0)) < tol &&
           abs(abs(xvert1(0, 1) - xvert2(0, 1))) < tol &&
      ...
      // vert1(0) matches vert2(1), rot_tag=2
      else if ((abs(abs(xvert1(0, 0) - xvert2(1, 0))) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(1, 1))) < tol &&
                abs(abs(xvert1(0, 2) - xvert2(1, 2))) < tol) ||
               (abs(abs(xvert1(0, 0) - xvert2(1, 0)) - delta_cyclic(0)) < tol &&
                abs(abs(xvert1(0, 1) - xvert2(1, 1))) < tol &&
