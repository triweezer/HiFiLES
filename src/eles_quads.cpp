...
void eles_quads::setup_ele_type_specific() {
  ...
  set_area_coord();  // Not sure if this is the right place to call it - check
                     // later (some differences in the master version)
}
...
void eles_quads::set_tloc_fpts(void) {
  ...
  for (i = 0; i < n_inters_per_ele; i++) {
    for (j = 0; j < (order + 1); j++) {
      ...
      // for tensor product elements flux point location depends on solution
      // point location
...
double eles_quads::compute_inter_detjac_inters_cubpts(int in_inter,
                                                      array<double> d_pos) {
  double output = 0.;
  double xr, xs;
  double yr, ys;
...
void eles_quads::compute_filter_upts(void) {
  ...
  {
    if (rank == 0)
      std::cout << "Building discrete Gaussian filter" << std::endl;
    int ctype, index;
...
void eles_quads::write_restart_info(std::ofstream &restart_file) {
  ...
  restart_file << "Number of solution points per quadrilateral element"
               << std::endl;
...
void eles_quads::set_concentration_array() {
  ...
  for (int i = 0; i < order + 1; i++)
    for (int j = 0; j < order + 1; j++)
      concentration_array(j + i * (order + 1)) =
          concentration_factor(j) * sqrt(1 - loc_1d_upts(i) * loc_1d_upts(i)) *
          grad_vandermonde(i, j);
}

// Set area co-ordinates/shape functions for bilinear interpolation used in AV
// routines
void eles_quads::set_area_coord(void) {
  ...
  if (n_dims == 2) {
    for (int i = 0; i < n_upts_per_ele; i++) {
      area_coord_upts(0, i) =
          0.25 * (1 - loc_upts(0, i)) * (1 - loc_upts(1, i));
      area_coord_upts(2, i) =
          0.25 * (1 - loc_upts(0, i)) * (1 + loc_upts(1, i));
      area_coord_upts(3, i) =
          0.25 * (1 + loc_upts(0, i)) * (1 + loc_upts(1, i));
    ...
    //     {
    //        area_coord_fpts(0,i) = 0.25*(1 - loc_fpts(0,i))*(1 -
    //        loc_fpts(1,i)); area_coord_fpts(2,i) = 0.25*(1 - loc_fpts(0,i))*(1
    //        + loc_fpts(1,i)); area_coord_fpts(3,i) = 0.25*(1 +
    //        loc_fpts(0,i))*(1 + loc_fpts(1,i)); area_coord_fpts(1,i) = 0.25*(1
    //        + loc_fpts(0,i))*(1 - loc_fpts(1,i));
    //     }
  }

  else
    std::cout
        << "Area coordinate calculation has not yet been implemented for this "
           "dimension"
        << std::endl;
}
...
double eles_quads::eval_nodal_basis(int in_index, array<double> in_loc) {
  ...
  nodal_basis = eval_lagrange(in_loc(0), j, loc_1d_upts) *
                eval_lagrange(in_loc(1), i, loc_1d_upts);
...
double eles_quads::eval_nodal_basis_restart(int in_index,
                                            array<double> in_loc) {
  int i, j;
  ...
  nodal_basis = eval_lagrange(in_loc(0), j, loc_1d_upts_rest) *
                eval_lagrange(in_loc(1), i, loc_1d_upts_rest);
...
double eles_quads::eval_d_nodal_basis(int in_index, int in_cpnt,
                                      array<double> in_loc) {
  int i, j;
  ...
  if (in_cpnt == 0) {
    d_nodal_basis = eval_d_lagrange(in_loc(0), j, loc_1d_upts) *
                    eval_lagrange(in_loc(1), i, loc_1d_upts);
  } else if (in_cpnt == 1) {
    d_nodal_basis = eval_lagrange(in_loc(0), j, loc_1d_upts) *
                    eval_d_lagrange(in_loc(1), i, loc_1d_upts);
  } else {
...
double eles_quads::eval_nodal_s_basis(int in_index, array<double> in_loc,
                                      int in_n_spts) {
  int i, j;
  ...
  if (is_perfect_square(in_n_spts)) {
    ...
    nodal_s_basis = eval_lagrange(in_loc(0), i, loc_1d_spts) *
                    eval_lagrange(in_loc(1), j, loc_1d_spts);
  } else if (in_n_spts == 8) {
    if (in_index == 0)
      nodal_s_basis = -0.25 * (1. - in_loc(0)) * (1. - in_loc(1)) *
                      (1. + in_loc(0) + in_loc(1));
    else if (in_index == 1)
      nodal_s_basis = -0.25 * (1. + in_loc(0)) * (1. - in_loc(1)) *
                      (1. - in_loc(0) + in_loc(1));
      ...
      nodal_s_basis =
          0.5 * (1. - in_loc(0)) * (1. + in_loc(0)) * (1. - in_loc(1));
    else if (in_index == 5)
      nodal_s_basis =
          0.5 * (1. + in_loc(0)) * (1. + in_loc(1)) * (1. - in_loc(1));
    else if (in_index == 6)
      nodal_s_basis =
          0.5 * (1. - in_loc(0)) * (1. + in_loc(0)) * (1. + in_loc(1));
    else if (in_index == 7)
...
void eles_quads::eval_d_nodal_s_basis(array<double> &d_nodal_s_basis,
  ...
  if (is_perfect_square(in_n_spts)) {
    ...
    for (int k = 0; k < in_n_spts; k++) {
      ...
      d_nodal_s_basis(k, 0) = eval_d_lagrange(in_loc(0), j, loc_1d_spts) *
                              eval_lagrange(in_loc(1), i, loc_1d_spts);
      d_nodal_s_basis(k, 1) = eval_lagrange(in_loc(0), j, loc_1d_spts) *
                              eval_d_lagrange(in_loc(1), i, loc_1d_spts);
    }
  } else if (in_n_spts == 8) {
    d_nodal_s_basis(0, 0) =
        -0.25 * (-1. + in_loc(1)) * (2. * in_loc(0) + in_loc(1));
    d_nodal_s_basis(1, 0) =
        0.25 * (-1. + in_loc(1)) * (in_loc(1) - 2. * in_loc(0));
    d_nodal_s_basis(2, 0) =
        0.25 * (1. + in_loc(1)) * (2. * in_loc(0) + in_loc(1));
    d_nodal_s_basis(3, 0) =
        -0.25 * (1. + in_loc(1)) * (in_loc(1) - 2. * in_loc(0));
    ...
    d_nodal_s_basis(0, 1) =
        -0.25 * (-1. + in_loc(0)) * (in_loc(0) + 2. * in_loc(1));
    d_nodal_s_basis(1, 1) =
        0.25 * (1. + in_loc(0)) * (2. * in_loc(1) - in_loc(0));
    d_nodal_s_basis(2, 1) =
        0.25 * (1. + in_loc(0)) * (in_loc(0) + 2. * in_loc(1));
    d_nodal_s_basis(3, 1) =
        -0.25 * (-1. + in_loc(0)) * (2. * in_loc(1) - in_loc(0));
...
double eles_quads::eval_legendre_basis_2D_hierarchical(int in_mode,
                                                       array<double> in_loc,
                                                       int in_basis_order) {
  double leg_basis;
  ...
  if (in_mode < n_dof) {
      ...
      for (j = 0; j < k + 1; j++) {
        ...
        if (i <= in_basis_order && j <= in_basis_order) {
          if (mode == in_mode)  // found the correct mode
            leg_basis =
                eval_legendre(in_loc(0), i) * eval_legendre(in_loc(1), j);
    ...
    std::cout << "ERROR: Invalid mode when evaluating Legendre basis ...."
              << std::endl;
  }
...
double eles_quads::exponential_filter(int in_mode, int in_basis_order) {
  ...
  if (in_mode < n_dof) {
    ...
    std::cout << "ERROR: Invalid mode when evaluating exponential filter ...."
              << std::endl;
  }
...
double eles_quads::eval_div_vcjh_basis(int in_index, array<double> &loc) {
  ...
  if (scheme < 5) {
    if (i == 0)
      div_vcjh_basis = -eval_lagrange(loc(0), j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(1), 0, order, eta);
    else if (i == 1)
      div_vcjh_basis = eval_lagrange(loc(1), j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(0), 1, order, eta);
    else if (i == 2)
      div_vcjh_basis = eval_lagrange(loc(0), order - j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(1), 1, order, eta);
    else if (i == 3)
      div_vcjh_basis = -eval_lagrange(loc(1), order - j, loc_1d_upts) *
...
double eles_quads::calc_h_ref_specific(int in_ele) {
  ...
  length(0) = sqrt(pow(shape(0, 0, in_ele) - shape(0, 1, in_ele), 2.0) +
                   pow(shape(1, 0, in_ele) - shape(1, 1, in_ele), 2.0));
  length(1) = sqrt(pow(shape(0, 1, in_ele) - shape(0, 3, in_ele), 2.0) +
                   pow(shape(1, 1, in_ele) - shape(1, 3, in_ele), 2.0));
  length(2) = sqrt(pow(shape(0, 3, in_ele) - shape(0, 2, in_ele), 2.0) +
                   pow(shape(1, 3, in_ele) - shape(1, 2, in_ele), 2.0));
  length(3) = sqrt(pow(shape(0, 2, in_ele) - shape(0, 0, in_ele), 2.0) +
                   pow(shape(1, 2, in_ele) - shape(1, 0, in_ele), 2.0));
