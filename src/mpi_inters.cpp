...
void mpi_inters::setup(int in_n_inters, int in_inters_type) {
  ...
  if (viscous) {
    out_buffer_grad_disu.setup(in_n_inters * n_fpts_per_inter * n_fields *
                               n_dims);
    in_buffer_grad_disu.setup(in_n_inters * n_fpts_per_inter * n_fields *
                              n_dims);
  }
...
void mpi_inters::set_mpi_requests(int in_number_of_requests) {
#ifdef _MPI
  mpi_in_requests =
      (MPI_Request *)malloc(in_number_of_requests * sizeof(MPI_Request));
  mpi_out_requests =
      (MPI_Request *)malloc(in_number_of_requests * sizeof(MPI_Request));
#endif
  if (viscous) {
#ifdef _MPI
    mpi_in_requests_grad =
        (MPI_Request *)malloc(in_number_of_requests * sizeof(MPI_Request));
    mpi_out_requests_grad =
        (MPI_Request *)malloc(in_number_of_requests * sizeof(MPI_Request));
#endif
  }
  ...
  if (LES) {
#ifdef _MPI
    mpi_in_requests_sgsf =
        (MPI_Request *)malloc(in_number_of_requests * sizeof(MPI_Request));
    mpi_out_requests_sgsf =
        (MPI_Request *)malloc(in_number_of_requests * sizeof(MPI_Request));
#endif
  }
}
...
void mpi_inters::set_mpi(int in_inter, int in_ele_type_l, int in_ele_l,
                         int in_local_inter_l, int rot_tag,
...
#ifdef _GPU
      disu_fpts_r(j, in_inter, i) =
          in_buffer_disu.get_ptr_gpu(in_inter * n_fpts_per_inter * n_fields +
                                     i * n_fpts_per_inter + j_rhs);
#else
      disu_fpts_r(j, in_inter, i) =
          in_buffer_disu.get_ptr_cpu(in_inter * n_fpts_per_inter * n_fields +
                                     i * n_fpts_per_inter + j_rhs);
#endif
      ...
      for (k = 0; k < n_dims; k++) {
...
#ifdef _GPU
          grad_disu_fpts_r(j, in_inter, i, k) = in_buffer_grad_disu.get_ptr_gpu(
              in_inter * n_fpts_per_inter * n_fields * n_dims +
              k * n_fpts_per_inter * n_fields + i * n_fpts_per_inter + j_rhs);
#else
          grad_disu_fpts_r(j, in_inter, i, k) = in_buffer_grad_disu.get_ptr_cpu(
              in_inter * n_fpts_per_inter * n_fields * n_dims +
              k * n_fpts_per_inter * n_fields + i * n_fpts_per_inter + j_rhs);
#endif
        }
        ...
        if (LES) {
          sgsf_fpts_l(j, in_inter, i, k) = get_sgsf_fpts_ptr(
              in_ele_type_l, in_ele_l, in_local_inter_l, i, k, j, FlowSol);
#ifdef _GPU
          sgsf_fpts_r(j, in_inter, i, k) = in_buffer_sgsf.get_ptr_gpu(
              in_inter * n_fpts_per_inter * n_fields * n_dims +
              k * n_fpts_per_inter * n_fields + i * n_fpts_per_inter + j_rhs);
#else
          sgsf_fpts_r(j, in_inter, i, k) = in_buffer_sgsf.get_ptr_cpu(
              in_inter * n_fpts_per_inter * n_fields * n_dims +
              k * n_fpts_per_inter * n_fields + i * n_fpts_per_inter + j_rhs);
#endif
        }
  ...
  if (motion) {
    for (j = 0; j < n_fpts_per_inter; j++) {
      ...
      // pretty sure these should be the same due to the continuous nature of
      // the dynamic->static mapping. But, leave it this way for now just in
      // case.
  ...
  for (i = 0; i < n_fpts_per_inter; i++) {
    ...
    tdA_fpts_l(i, in_inter) =
        get_tdA_fpts_ptr(in_ele_type_l, in_ele_l, in_local_inter_l, i, FlowSol);

    for (j = 0; j < n_dims; j++) {
      norm_fpts(i, in_inter, j) = get_norm_fpts_ptr(
          in_ele_type_l, in_ele_l, in_local_inter_l, i, j, FlowSol);
    }
  }
}
...
void mpi_inters::send_solution() {
  if (n_inters != 0) {
...
#endif
#ifdef _GPU
    pack_out_buffer_disu_gpu_kernel_wrapper(n_fpts_per_inter, n_inters,
                                            n_fields, disu_fpts_l.get_ptr_gpu(),
                                            out_buffer_disu.get_ptr_gpu());
    ...
    for (int p = 0; p < nproc; p++) {
      ...
      // std::cout << "rank=" << rank << "p=" << p << "inters_type=" << inters_type
      // << "Nout = " << Nout << std::endl;
      if (Nout) {
#ifdef _MPI
        MPI_Isend(out_buffer_disu.get_ptr_cpu(sk), Nout, MPI_DOUBLE, p,
                  inters_type * 10000 + p, MPI_COMM_WORLD,
                  &mpi_out_requests[request_count]);
        MPI_Irecv(in_buffer_disu.get_ptr_cpu(sk), Nout, MPI_DOUBLE, p,
                  inters_type * 10000 + rank, MPI_COMM_WORLD,
...
void mpi_inters::send_corrected_gradient() {
  if (n_inters != 0) {
...
#ifdef _GPU
    if (n_inters != 0)
      pack_out_buffer_grad_disu_gpu_kernel_wrapper(
          n_fpts_per_inter, n_inters, n_fields, n_dims,
          grad_disu_fpts_l.get_ptr_gpu(), out_buffer_grad_disu.get_ptr_gpu());
    ...
    for (int p = 0; p < nproc; p++) {
      ...
      if (Nout) {
#ifdef _MPI
        MPI_Isend(out_buffer_grad_disu.get_ptr_cpu(sk), Nout, MPI_DOUBLE, p,
                  inters_type * 10000 + p, MPI_COMM_WORLD,
                  &mpi_out_requests_grad[request_count]);
        MPI_Irecv(in_buffer_grad_disu.get_ptr_cpu(sk), Nout, MPI_DOUBLE, p,
                  inters_type * 10000 + rank, MPI_COMM_WORLD,
                  &mpi_in_requests_grad[request_count]);
#endif
        sk += Nout;
        Nmess++;
        request_count++;
...
void mpi_inters::send_sgsf_fpts() {
  if (n_inters != 0) {
...
#ifdef _GPU
    if (n_inters != 0)
      pack_out_buffer_sgsf_gpu_kernel_wrapper(
          n_fpts_per_inter, n_inters, n_fields, n_dims,
          sgsf_fpts_l.get_ptr_gpu(), out_buffer_sgsf.get_ptr_gpu());
    ...
    for (int p = 0; p < nproc; p++) {
      ...
      if (Nout) {
#ifdef _MPI
        MPI_Isend(out_buffer_sgsf.get_ptr_cpu(sk), Nout, MPI_DOUBLE, p,
                  inters_type * 10000 + p, MPI_COMM_WORLD,
                  &mpi_out_requests_sgsf[request_count]);
        MPI_Irecv(in_buffer_sgsf.get_ptr_cpu(sk), Nout, MPI_DOUBLE, p,
                  inters_type * 10000 + rank, MPI_COMM_WORLD,
                  &mpi_in_requests_sgsf[request_count]);
#endif
        sk += Nout;
        Nmess++;
        request_count++;
...
// calculate normal transformed continuous inviscid flux at the flux points at
// mpi faces
  ...
  for (int i = 0; i < n_inters; i++) {
    for (int j = 0; j < n_fpts_per_inter; j++) {
      ...
      if (run_input.riemann_solve_type == 0) {
        ...
        rusanov_flux(temp_u_l, temp_u_r, temp_v, temp_f_l, temp_f_r, norm, fn,
                     n_dims, n_fields, run_input.gamma);
...
#ifdef _GPU
  if (n_inters != 0) {
    calculate_common_invFlux_mpi_gpu_kernel_wrapper(
        n_fpts_per_inter, n_dims, n_fields, n_inters, disu_fpts_l.get_ptr_gpu(),
        disu_fpts_r.get_ptr_gpu(), norm_tconf_fpts_l.get_ptr_gpu(),
        ...
        run_input.riemann_solve_type, delta_disu_fpts_l.get_ptr_gpu(),
        run_input.gamma, run_input.pen_fact, run_input.viscous, motion,
        run_input.vis_riemann_solve_type, run_input.wave_speed(0),
        run_input.wave_speed(1), run_input.wave_speed(2), run_input.lambda,
...
void mpi_inters::calculate_common_viscFlux(void) {
  ...
  for (int i = 0; i < n_inters; i++) {
    for (int j = 0; j < n_fpts_per_inter; j++) {
      ...
      // Calling viscous riemann solver
      if (run_input.vis_riemann_solve_type == 0)
        ldg_flux(0, temp_u_l, temp_u_r, temp_f_l, temp_f_r, norm, fn, n_dims,
                 n_fields, run_input.tau, run_input.pen_fact);
      else
        FatalError("Viscous Riemann solver not implemented");
...
#ifdef _GPU
  if (n_inters != 0)
    calculate_common_viscFlux_mpi_gpu_kernel_wrapper(
        n_fpts_per_inter, n_dims, n_fields, n_inters, disu_fpts_l.get_ptr_gpu(),
        disu_fpts_r.get_ptr_gpu(), grad_disu_fpts_l.get_ptr_gpu(),
        grad_disu_fpts_r.get_ptr_gpu(), norm_tconf_fpts_l.get_ptr_gpu(),
        tdA_fpts_l.get_ptr_gpu(), ndA_dyn_fpts_l.get_ptr_gpu(),
        J_dyn_fpts_l.get_ptr_gpu(), norm_fpts.get_ptr_gpu(),
        norm_dyn_fpts.get_ptr_gpu(), sgsf_fpts_l.get_ptr_gpu(),
        sgsf_fpts_r.get_ptr_gpu(), run_input.riemann_solve_type,
        run_input.vis_riemann_solve_type, run_input.pen_fact, run_input.tau,
        run_input.gamma, run_input.prandtl, run_input.rt_inf, run_input.mu_inf,
        run_input.c_sth, run_input.fix_vis, run_input.diff_coeff, LES, motion,
