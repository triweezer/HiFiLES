...
void CSysSolve::applyGivens(const double &s, const double &c, double &h1,
                            double &h2) {
...
void CSysSolve::solveReduced(const int &n,
                             const std::vector<std::vector<double>> &Hsbg,
...
void CSysSolve::modGramSchmidt(int i, std::vector<std::vector<double>> &Hsbg,
  ...
  // test the resulting std::vector
  nrm = w[i + 1].norm();
  Hsbg[i + 1][i] = nrm;
  if (nrm <= 0.0) {
    // w[i+1] is a linear combination of the w[0:i]
    std::cerr << "CSysSolve::modGramSchmidt: w[i+1] linearly dependent on w[0:i]"
         << std::endl;
    throw(-1);
  }

  // scale the resulting std::vector
  w[i + 1] /= nrm;
}

void CSysSolve::writeHeader(const std::string &solver, const double &restol,
                            const double &resinit) {
  std::cout << "# " << solver << " residual history" << std::endl;
  std::cout << "# Residual tolerance target = " << restol << std::endl;
  std::cout << "# Initial residual norm     = " << resinit << std::endl;
}

void CSysSolve::writeHistory(const int &iter, const double &res,
                             const double &resinit) {
  std::cout << "     " << iter << "     " << res / resinit << std::endl;
}

unsigned long CSysSolve::ConjugateGradient(const CSysVector &b, CSysVector &x,
  ...
  if (m < 1) {
    if (rank == 0)
      std::cerr << "CSysSolve::ConjugateGradient: illegal value for subspace size, "
              "m = "
           << m << std::endl;
#ifndef _MPI
    exit(1);
  ...
  /*--- Calculate the initial residual, compute norm, and check if system is
   * already solved ---*/
  mat_vec(x, A_p);
  ...
  if ((norm_r < tol * norm0) || (norm_r < eps)) {
    if (rank == 0)
      std::cout << "CSysSolve::ConjugateGradient(): system solved by initial guess."
           << std::endl;
    return 0;
  }
  ...
  for (i = 0; i < m; i++) {
    ...
    /*--- Check if solution has converged, else output the relative residual if
     * necessary ---*/
    norm_r = r.norm();
    if (norm_r < tol * norm0) break;
    if (((monitoring) && (rank == 0)) && ((i + 1) % 5 == 0))
  ...
  //    if (rank == 0) {
  //      std::cout << "# WARNING in CSysSolve::ConjugateGradient(): " << std::endl;
  //      std::cout << "# true residual norm and calculated residual norm do not
  //      agree." << std::endl; std::cout << "# true_res - calc_res = " << true_res -
  //      norm_r << std::endl;
  //    }
  //  }
...
unsigned long CSysSolve::FGMRES(const CSysVector &b, CSysVector &x,
                                CMatrixVectorProduct &mat_vec,
                                CPreconditioner &precond, double tol,
                                unsigned long m, bool monitoring,
                                solution *FlowSol) {
  ...
  if (m < 1) {
    if (rank == 0)
      std::cerr << "CSysSolve::FGMRES: illegal value for subspace size, m = " << m
           << std::endl;
#ifndef _MPI
    exit(1);
#else
  ...
  if (m > 1000) {
    if (rank == 0)
      std::cerr << "CSysSolve::FGMRES: illegal value for subspace size (too high), "
              "m = "
           << m << std::endl;
#ifndef _MPI
    exit(1);
  ...
  std::vector<CSysVector> w(m + 1, x);
  std::vector<CSysVector> z(m + 1, x);
  ...
  /*---  Calculate the norm of the rhs std::vector ---*/
  double norm0 = b.norm();
  ...
  if ((beta < tol * norm0) || (beta < eps)) {
    ...
    if (rank == 0)
      std::cout << "CSysSolve::FGMRES(): system solved by initial guess." << std::endl;
    return 0;
  }
  ...
  for (i = 0; i < m; i++) {
    ...
    precond(w[i], z[i]);
    // std::cout << "Adding to Krylov subspace" << std::endl;
    ...
    if ((((monitoring) && (rank == 0)) && ((i + 1) % 1 == 0)) && (rank == 0))
...
unsigned long CSysSolve::BCGSTAB(const CSysVector &b, CSysVector &x,
                                 CMatrixVectorProduct &mat_vec,
                                 CPreconditioner &precond, double tol,
                                 unsigned long m, bool monitoring) {
  int rank = 0;
  ...
  if (m < 1) {
    if (rank == 0)
      std::cerr << "CSysSolve::BCGSTAB: illegal value for subspace size, m = " << m
           << std::endl;
#ifndef _MPI
    exit(1);
#else
  ...
  mat_vec(x, A_x);
  r -= A_x;
  r_0 = r;  // recall, r holds b initially
  double norm_r = r.norm();
  double norm0 = b.norm();
  if ((norm_r < tol * norm0) || (norm_r < eps)) {
    if (rank == 0)
      std::cout << "CSysSolve::BCGSTAB(): system solved by initial guess." << std::endl;
    return 0;
  }
  ...
  for (i = 0; i < m; i++) {
    ...
    /*--- Update solution and residual: ---*/
    x.Plus_AX(alpha, phat);
    x.Plus_AX(omega, shat);
    ...
    norm_r = r.norm();
    if (norm_r < tol * norm0) break;
    if (((monitoring) && (rank == 0)) && ((i + 1) % 5 == 0) && (rank == 0))
      writeHistory(i + 1, norm_r, norm0);
  }
  ...
  //    std::cout << "# WARNING in CSysSolve::BCGSTAB(): " << std::endl;
  //    std::cout << "# true residual norm and calculated residual norm do not
  //    agree." << std::endl; std::cout << "# true_res - calc_res = " << true_res <<" "<<
  //    norm_r << std::endl;
  //  }
