...
void input::set_order(int in_order) { order = in_order; }

void input::set_dt(double in_dt) { dt = in_dt; }

void input::set_c(double in_c_tri, double in_c_quad) {
  c_tri = in_c_tri;
  c_quad = in_c_quad;

  double a_k = eval_gamma(2 * order + 1) /
               (pow(2., order) * pow(eval_gamma(order + 1), 2));
  eta_quad = in_c_quad * 0.5 * (2. * order + 1.) * a_k * eval_gamma(order + 1) *
             a_k * eval_gamma(order + 1);
}
...
void input::read_input_file(std::string fileName, int rank) {
  ...
  opts.getScalarValue("data_file_name", data_file_name, std::string("Mesh"));
  ...
  if (dt_type == 2 && rank == 0) {
    std::cout << "!!!!!!" << std::endl;
    std::cout
        << "  Note: Local timestepping is still in an experimental phase,";
...
void input::setup_params(int rank) {
  ...
  if (equation == 0) {
    ...
    if (riemann_solve_type == 0)
      FatalError(
          "Rusanov flux not supported with Advection-Diffusion equation");
    if (ic_form == 0 || ic_form == 1)
      FatalError(
          "Initial condition not supported with Advection-Diffusion equation");
  }
  ...
  if (viscous) {
    ...
    if (ic_form == 0 || artif_only || ic_form == 8) {
      ...
      // Set either a fixed value for the viscosity or a value from Sutherland's
      // law

      if (fix_vis) {
        mu_free_stream = mu_gas;
      } else {
        mu_free_stream = mu_gas * pow(T_free_stream / T_gas, 1.5) *
                         ((T_gas + S_gas) / (T_free_stream + S_gas));
      }

      // Compute the corresponding density from the definition of the Reynolds
      // number Re and the Re length are specified in the input file.

      rho_free_stream =
          (mu_free_stream * Re_free_stream) / (uvw_ref * L_free_stream);

      // Compute the freestream pressure from the gas law

      p_free_stream = rho_free_stream * R_gas * T_free_stream;

      // Choose the following consistent reference quantities for other
      // variables
      ...
      rho_bound = 1.;  // Note that we have chosen our non-dim. such that
                       // rho_ref = rho_free_stream
      ...
      T_total_bound =
          (T_free_stream / T_ref) *
          (1.0 + 0.5 * (gamma - 1.0) * Mach_free_stream * Mach_free_stream);
      p_total_bound =
          p_bound *
          pow(1.0 + 0.5 * (gamma - 1.0) * Mach_free_stream * Mach_free_stream,
              gamma / (gamma - 1.0));
      ...
      // Set up the dimensionless initial conditions (repeat process above for
      // freestream)
      ...
      if (fix_vis) {
        ...
        mu_c_ic = mu_gas * pow(T_c_ic / T_gas, 1.5) *
                  ((T_gas + S_gas) / (T_c_ic + S_gas));
      }
...
void fileReader::setFile(std::string fileName) { this->fileName = fileName; }
...
template <typename T>
void fileReader::getScalarValue(std::string optName, T &opt, T defaultVal) {
  ...
  if (!optFile.is_open() || !getline(optFile, str)) {
    ...
    if (!optFile.is_open()) FatalError("Cannont open input file for reading.");
  }
  ...
  while (getline(optFile, str)) {
    ...
    if (optKey.compare(optName) == 0) {
      if (!(ss >> opt)) {
        ...
        // double
        std::cout << "WARNING: Unable to assign value to option " << optName
                  << std::endl;
        std::cout << "Using default value of " << defaultVal << " instead."
                  << std::endl;
        opt = defaultVal;
      }
...
template <typename T>
void fileReader::getScalarValue(std::string optName, T &opt) {
  std::string str, optKey;
  ...
  while (getline(optFile, str)) {
    ...
    if (optKey.compare(optName) == 0) {
      if (!(ss >> opt)) {
        // This could happen if, for example, trying to assign a string to a
        // double
        std::cerr << "WARNING: Unable to assign value to option " << optName
                  << std::endl;
...
void fileReader::getMap(std::string optName, std::map<T, U> &opt) {
  std::string str, optKey;
  T tmpT;
  U tmpU;
  bool found;
...
template <typename T>
void fileReader::getVectorValue(std::string optName, std::vector<T> &opt) {
  std::string str, optKey;
  ...
  while (getline(optFile, str)) {
    ...
    if (optKey.compare(optName) == 0) {
      ...
      if (!(ss >> nVals)) {
        ...
        // double
        std::cerr
            << "WARNING: Unable to read number of entries for vector option "
            << optName << std::endl;
        std::string errMsg = "Required option not set: " + optName;
        FatalError(errMsg.c_str());
      }
      ...
      for (int i = 0; i < nVals; i++) {
        if (!ss >> opt[i]) {
          std::cerr << "WARNING: Unable to assign all values to vector option "
                    << optName << std::endl;
          std::string errMsg = "Required option not set: " + optName;
          FatalError(errMsg.c_str())
        }
...
template <typename T>
void fileReader::getVectorValue(std::string optName, array<T> &opt) {
  std::string str, optKey;
...
template <typename T>
void fileReader::getVectorValueOptional(std::string optName, array<T> &opt) {
  std::string str, optKey;
