...
void eles_tets::setup_ele_type_specific() {
  ...
  n_peles_per_ele = (p_res - 1) * (p_res) * (p_res + 1) / 6 +
                    4 * (p_res - 2) * (p_res - 1) * (p_res) / 6 +
                    (p_res - 3) * (p_res - 2) * (p_res - 1) / 6;
  n_verts_per_ele = 4;
  ...
  for (int k = 0; k < p_res - 1; ++k) {
    for (int j = 0; j < p_res - 1 - k; ++j) {
      for (int i = 0; i < p_res - 1 - k - j; ++i) {
        vertex_0 = temp - (p_res - k) * (p_res + 1 - k) * (p_res + 2 - k) / 6 +
                   j * (p_res - k) - (j - 1) * j / 2 + i;
        vertex_1 = temp - (p_res - k) * (p_res + 1 - k) * (p_res + 2 - k) / 6 +
                   j * (p_res - k) - (j - 1) * j / 2 + i + 1;
        vertex_2 = temp - (p_res - k) * (p_res + 1 - k) * (p_res + 2 - k) / 6 +
    ...
    for (int j = 0; j < p_res - 2 - k; ++j) {
      for (int i = 0; i < p_res - 2 - k - j; ++i) {
        ...
        vertex_3 = temp -
                   (p_res - (k + 1)) * (p_res + 1 - (k + 1)) *
                       (p_res + 2 - (k + 1)) / 6 +
                   (j + 1) * (p_res - (k + 1)) - (j) * (j + 1) / 2 + (i - 1) +
                   1;
        vertex_4 = temp -
                   (p_res - (k + 1)) * (p_res + 1 - (k + 1)) *
                       (p_res + 2 - (k + 1)) / 6 +
                   (j) * (p_res - (k + 1)) - (j - 1) * (j) / 2 + (i - 1) + 1;
        vertex_5 = temp -
...
#include "../data/loc_tet_alpha_pts.dat"
    ...
    std::cout << "Error: Unknown solution points location type...."
              << std::endl;
    exit(1);
  }
}
...
#include "../data/loc_tri_alpha_pts.dat"
  ...
  for (j = 0; j < order + 1; j++) {
    for (i = 0; i < order + 1 - j; i++) {
      ...
      tloc_fpts(0, 2 * n_fpts_per_inter(0) + i_tri_fpts) =
          loc_tri_fpts(i_alpha, 0);
      tloc_fpts(1, 2 * n_fpts_per_inter(0) + i_tri_fpts) = -1;
      tloc_fpts(2, 2 * n_fpts_per_inter(0) + i_tri_fpts) =
          loc_tri_fpts(i_alpha, 1);

      tloc_fpts(0, 3 * n_fpts_per_inter(0) + i_tri_fpts) =
          loc_tri_fpts(i_alpha, 1);
      tloc_fpts(1, 3 * n_fpts_per_inter(0) + i_tri_fpts) =
          loc_tri_fpts(i_alpha, 0);
...
double eles_tets::compute_inter_detjac_inters_cubpts(int in_inter,
                                                     array<double> d_pos) {
  double output = 0.;
  double xr, xs, xt;
  double yr, ys, yt;
  ...
  // std::cout << "temp0=" << temp0 << "temp1" << temp1 << "temp2" << temp2 <<
  // std::endl; std::cout << "in_inter=" << in_inter << "output=" << output <<
  // std::endl;
...
void eles_tets::compute_filter_upts(void) {
  ...
  for (i = 0; i < N; i++)
    for (j = i; j < N; j++)
      beta(i, j) = sqrt(pow(X(0, i) - X(0, j), 2) + pow(X(1, i) - X(1, j), 2) +
                        pow(X(2, i) - X(2, j), 2)) /
                   dlt;
  for (i = 0; i < N; i++)
    for (j = 0; j < i; j++) beta(i, j) = beta(j, i);
  ...
  } else if (run_input.filter_type == 1)  // Discrete Gaussian filter
  {
    ...
    if (N != n_cubpts_per_ele) {
      FatalError(
          "WARNING: Gaussian filter cannot be built for tets since "
          "n_upts_per_ele != n_cubpts_per_ele for any order. Exiting");
    }
  } else if (run_input.filter_type == 2)  // Modal coefficient filter
  {
    ...
    compute_modal_filter_tet(filter_upts, vandermonde, inv_vandermonde, N,
                             order);
  ...
  for (i = 0; i < N2; i++) {
    for (j = 0; j < N; j++) {
      filter_upts(i, j) =
          0.5 * filter_upts(i, j) + filter_upts(N - i - 1, N - j - 1);
  ...
  sum = 0;
  for (i = 0; i < N; i++)
    for (j = 0; j < N; j++) sum += filter_upts(i, j);
}
...
void eles_tets::set_vandermonde(void) {
  ...
  for (int i = 0; i < n_upts_per_ele; i++)
    for (int j = 0; j < n_upts_per_ele; j++)
      vandermonde(i, j) = eval_dubiner_basis_3d(loc_upts(0, i), loc_upts(1, i),
                                                loc_upts(2, i), j, order);
...
void eles_tets::set_vandermonde_restart() {
  ...
  for (int i = 0; i < n_upts_per_ele_rest; i++)
    for (int j = 0; j < n_upts_per_ele_rest; j++)
      vandermonde(i, j) =
          eval_dubiner_basis_3d(loc_upts_rest(0, i), loc_upts_rest(1, i),
                                loc_upts_rest(2, i), j, order_rest);
...
void eles_tets::write_restart_info(std::ofstream &restart_file) {
  ...
  restart_file << "Location of solution points in tetrahedral elements"
               << std::endl;
  for (int i = 0; i < n_upts_per_ele; i++) {
    for (int j = 0; j < n_dims; j++) {
...
double eles_tets::eval_nodal_basis(int in_index, array<double> in_loc) {
  ...
  for (int i = 0; i < n_upts_per_ele; i++)
    dubiner_basis_at_loc(i) =
        eval_dubiner_basis_3d(in_loc(0), in_loc(1), in_loc(2), i, order);

  // From Hesthaven, equation 3.3, V^T * l = P, or l = (V^-1)^T P
  out_nodal_basis_at_loc = 0.;
  for (int i = 0; i < n_upts_per_ele; i++)
    out_nodal_basis_at_loc +=
        inv_vandermonde(i, in_index) * dubiner_basis_at_loc(i);
...
double eles_tets::eval_nodal_basis_restart(int in_index, array<double> in_loc) {
  ...
  for (int i = 0; i < n_upts_per_ele_rest; i++)
    dubiner_basis_at_loc(i) =
        eval_dubiner_basis_3d(in_loc(0), in_loc(1), in_loc(2), i, order_rest);

  // From Hesthaven, equation 3.3, V^T * l = P, or l = (V^-1)^T P
  out_nodal_basis_at_loc = 0.;
  for (int i = 0; i < n_upts_per_ele_rest; i++)
    out_nodal_basis_at_loc +=
        inv_vandermonde_rest(i, in_index) * dubiner_basis_at_loc(i);
...
double eles_tets::eval_d_nodal_basis(int in_index, int in_cpnt,
                                     array<double> in_loc) {
  ...
  for (int i = 0; i < n_upts_per_ele; i++)
    d_dubiner_basis_at_loc(i) = eval_grad_dubiner_basis_3d(
        in_loc(0), in_loc(1), in_loc(2), i, order, in_cpnt);

  // From Hesthaven, equation 3.3, V^T * l = P, or l = (V^-1)^T P
  out_d_nodal_basis_at_loc = 0.;
  for (int i = 0; i < n_upts_per_ele; i++)
    out_d_nodal_basis_at_loc +=
        inv_vandermonde(i, in_index) * d_dubiner_basis_at_loc(i);
...
double eles_tets::eval_nodal_s_basis(int in_index, array<double> in_loc,
                                     int in_n_spts) {
  ...
  if (in_n_spts == 4) {
    ...
    if (in_index == 0)
      nodal_s_basis = (1. / 2. * (2. + in_loc(0) + in_loc(1) + in_loc(2))) *
                      (in_loc(0) + 1. + in_loc(1) + in_loc(2));
    else if (in_index == 1)
      ...
      nodal_s_basis =
          -(in_loc(0) + 1. + in_loc(1) + in_loc(2)) * (in_loc(0) + 1.);
    else if (in_index == 5)
      nodal_s_basis =
          -(in_loc(0) + 1. + in_loc(1) + in_loc(2)) * (in_loc(1) + 1.);
    else if (in_index == 6)
      nodal_s_basis =
          -(in_loc(0) + 1. + in_loc(1) + in_loc(2)) * (in_loc(2) + 1.);
    else if (in_index == 7)
...
void eles_tets::eval_d_nodal_s_basis(array<double> &d_nodal_s_basis,
...
double eles_tets::eval_div_dg_tet(int in_index, array<double> &loc) {
  ...
  if (isnan(coeff_gdotn(0, 0))) exit(1);
  ...
  for (int i = 0; i < n_upts_per_ele; i++) {
    ...
    for (int j = 0; j < cub2d.get_n_pts(); j++) {
      ...
      for (int k = 0; k < n_fpts_per_inter(0); k++)
        gdotn_at_cubpt +=
            coeff_gdotn(k, 0) * eval_dubiner_basis_2d(r_face, s_face, k, order);

      integral += cub2d.get_weight(j) *
                  eval_dubiner_basis_3d(r, s, t, i, order) * gdotn_at_cubpt;
    }
  ...
  for (int i = 0; i < n_upts_per_ele; i++)
    div_vcjh_basis += coeff_divg(i, 0) *
                      eval_dubiner_basis_3d(loc(0), loc(1), loc(2), i, order);

  return div_vcjh_basis;
}

void eles_tets::compute_filt_matrix_tet(array<double> &Filt,
                                        int vcjh_scheme_tet, double c_tet) {
  // -----------------
  // VCJH Filter
  // -----------------
  int Ncoeff, indx;
  ...
  for (int i = 0; i < n_upts_per_ele; i++) {
    for (int j = 0; j < n_upts_per_ele; j++) {
      tempr(i, j) = eval_grad_dubiner_basis_3d(loc_upts(0, i), loc_upts(1, i),
                                               loc_upts(2, i), j, order, 0);
      temps(i, j) = eval_grad_dubiner_basis_3d(loc_upts(0, i), loc_upts(1, i),
                                               loc_upts(2, i), j, order, 1);
      tempt(i, j) = eval_grad_dubiner_basis_3d(loc_upts(0, i), loc_upts(1, i),
                                               loc_upts(2, i), j, order, 2);
    }
  }
  ...
  for (int i = 0; i < n_upts_per_ele; i++) Identity(i, i) = 1.;
  ...
  for (int v = 1; v <= (order + 1); v++) {
    for (int w = 1; w <= v; w++) {
      c_coeff(indx) =
          (1. / n_upts_per_ele) *
          (factorial(order) / (factorial(v - 1) * factorial(order - (v - 1)))) *
          (factorial(v - 1) /
           (factorial(w - 1) * factorial((v - 1) - (w - 1))));
      // std::cout << "v=" << v << " w=" << w << " indx=" << indx << " coeff= "
      // << c_coeff(indx) << std::endl;
      indx++;
    }
  }
  ...
  for (int i = 0; i < n_upts_per_ele; i++)
    for (int j = 0; j < n_upts_per_ele; j++) mtemp_1(i, j) += Identity(i, j);
