...
void calc_invf_3d(array<double> &in_u, array<double> &out_f) {
  if (run_input.equation == 0)  // Euler and NS Equation
  {
    ...
    vz = in_u(3) / in_u(0);
    p = (run_input.gamma - 1.0) *
        (in_u(4) - (0.5 * in_u(0) * ((vx * vx) + (vy * vy) + (vz * vz))));
...
void calc_visf_2d(array<double> &in_u, array<double> &in_grad_u,
                  array<double> &out_f) {
  if (run_input.equation == 0)  // Navier-Stokes equations
  {
    ...
    if (run_input.turb_model == 1) {
      ...
      if (nu_tilde >= 0.0) {
        double f_v1 = pow(in_u(4) / mu, 3.0) /
                      (pow(in_u(4) / mu, 3.0) + pow(run_input.c_v1, 3.0));
        mu_t = in_u(4) * f_v1;
      } else {
        mu_t = 0.0;
      }
    ...
    out_f(1, 0) = -tauxx;
    out_f(2, 0) = -tauxy;
    out_f(3, 0) = -(u * tauxx + v * tauxy +
                    (mu / run_input.prandtl + mu_t / run_input.prandtl_t) *
                        (run_input.gamma) * de_dx);

    out_f(0, 1) = 0.0;
    out_f(1, 1) = -tauxy;
    out_f(2, 1) = -tauyy;
    out_f(3, 1) = -(u * tauxy + v * tauyy +
                    (mu / run_input.prandtl + mu_t / run_input.prandtl_t) *
                        (run_input.gamma) * de_dy);
...
void calc_visf_3d(array<double> &in_u, array<double> &in_grad_u,
                  array<double> &out_f) {
  if (run_input.equation == 0)  // Navier-Stokes equations
  {
    ...
    if (run_input.turb_model == 1) {
      ...
      if (nu_tilde >= 0.0) {
        double f_v1 = pow(in_u(5) / mu, 3.0) /
                      (pow(in_u(5) / mu, 3.0) + pow(run_input.c_v1, 3.0));
        mu_t = in_u(5) * f_v1;
      } else {
        mu_t = 0.0;
      }
    ...
    dke_dx = 0.5 * (u * u + v * v + w * w) * rho_dx +
             rho * (u * du_dx + v * dv_dx + w * dw_dx);
    dke_dy = 0.5 * (u * u + v * v + w * w) * rho_dy +
             rho * (u * du_dy + v * dv_dy + w * dw_dy);
    dke_dz = 0.5 * (u * u + v * v + w * w) * rho_dz +
    ...
    out_f(2, 0) = -tauxy;
    out_f(3, 0) = -tauxz;
    out_f(4, 0) = -(u * tauxx + v * tauxy + w * tauxz +
                    (mu / run_input.prandtl + mu_t / run_input.prandtl_t) *
                        (run_input.gamma) * de_dx);
    ...
    out_f(2, 1) = -tauyy;
    out_f(3, 1) = -tauyz;
    out_f(4, 1) = -(u * tauxy + v * tauyy + w * tauyz +
                    (mu / run_input.prandtl + mu_t / run_input.prandtl_t) *
                        (run_input.gamma) * de_dy);
    ...
    out_f(2, 2) = -tauyz;
    out_f(3, 2) = -tauzz;
    out_f(4, 2) = -(u * tauxz + v * tauyz + w * tauzz +
                    (mu / run_input.prandtl + mu_t / run_input.prandtl_t) *
                        (run_input.gamma) * de_dz);
...
void calc_alef_2d(array<double> &in_u, array<double> &in_v,
                  array<double> &out_f) {
...
void calc_alef_3d(array<double> &in_u, array<double> &in_v,
                  array<double> &out_f) {
  if (run_input.equation == 0)  // Euler / N-S
  {
    for (int i = 0; i < 5; i++) {
