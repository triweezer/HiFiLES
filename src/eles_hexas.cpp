...
void eles_hexas::setup_ele_type_specific() {
  ...
  if (run_input.turb_model == 1) n_fields++;
...
void eles_hexas::set_connectivity_plot() {
  int vertex_0, vertex_1, vertex_2, vertex_3, vertex_4, vertex_5, vertex_6,
      vertex_7;
  int count = 0;
  int k, l, m;
...
// set location of 1d solution points in standard interval (required for tensor
// product elements)
...
// set location of 1d shape points in standard interval (required for tensor
// product element)
  ...
  for (i = 0; i < n_inters_per_ele; ++i) {
    for (j = 0; j < (order + 1); ++j) {
      for (k = 0; k < (order + 1); ++k) {
        ...
        // for tensor prodiuct elements flux point location depends on solution
        // point location
...
double eles_hexas::compute_inter_detjac_inters_cubpts(int in_inter,
                                                      array<double> d_pos) {
  double output = 0.;
  double xr, xs, xt;
  double yr, ys, yt;
...
void eles_hexas::compute_filter_upts(void) {
  ...
  for (i = 0; i < N; ++i)
    for (j = 0; j < N; ++j) beta(j, i) = (X(j) - X(i)) / dlt;
  ...
  if (run_input.filter_type == 0 and N >= 3) {
    if (rank == 0)
      std::cout << "Building high-order-commuting Vasilyev filter" << std::endl;
    array<double> C(N);
    array<double> A(N, N);
    ...
    for (i = 0; i < N; ++i) {
      ...
      for (k = 3; k < N; ++k) {
        for (j = 0; j < N; ++j) A(j, k) = pow(beta(j, i), k + 1);
      ...
      // Gauss-Jordan method
      gaussj(N, A, B);
      for (j = 0; j < N; ++j) filter_upts_1D(j, i) = B(j);
    }
  } else if (run_input.filter_type == 1)  // Discrete Gaussian filter
  {
    ...
    if (N != n_cubpts_1d) {
      FatalError(
          "WARNING: To build Gaussian filter, the interface cubature order "
          "must equal solution order, e.g. inters_cub_order=9 if order=4, "
          "inters_cub_order=7 if order=3, inters_cub_order=5 if order=2. "
          "Exiting");
    }
    for (j = 0; j < n_cubpts_1d; j++) wf(j) = cub_1d.get_weight(j);

    // Determine corrected filter width for skewed quadrature points
    // using iterative constraining procedure
    // ctype options: (-1) no constraining, (0) constrain moment, (1) constrain
    // cutoff frequency
    ctype = -1;
    if (ctype >= 0) {
      for (i = 0; i < N2; ++i) {
        for (j = 0; j < N; ++j) B(j) = beta(j, i);
    ...
    for (i = 0; i < N; ++i) {
      ...
      for (j = 0; j < N; ++j) {
        filter_upts_1D(i, j) =
            wf(j) * exp(-6.0 * pow(alpha(i) * beta(i, j), 2));
  ...
  } else if (run_input.filter_type == 2)  // Modal coefficient filter
  {
    ...
    compute_modal_filter_1d(filter_upts_1D, vandermonde, inv_vandermonde, N,
                            order);

    sum = 0;
    for (i = 0; i < N; i++)
      for (j = 0; j < N; j++) sum += filter_upts_1D(i, j);
    ...
    for (j = 0; j < N; ++j) {
      for (k = 0; k < N; ++k) {
        ...
        for (l = 0; l < N; ++l) {
          for (m = 0; m < N; ++m) {
            for (n = 0; n < N; ++n) {
              filter_upts(ii, jj) = filter_upts_1D(k, n) *
                                    filter_upts_1D(j, m) * filter_upts_1D(i, l);
...
int eles_hexas::read_restart_info(std::ifstream &restart_file) {
  ...
  for (int i = 0; i < order_rest + 1; ++i) restart_file >> loc_1d_upts_rest(i);
...
double eles_hexas::eval_nodal_basis(int in_index, array<double> in_loc) {
  ...
  nodal_basis = eval_lagrange(in_loc(0), k, loc_1d_upts) *
                eval_lagrange(in_loc(1), j, loc_1d_upts) *
                eval_lagrange(in_loc(2), i, loc_1d_upts);
...
//
double eles_hexas::eval_nodal_basis_restart(int in_index,
                                            array<double> in_loc) {
  int i, j, k;
  ...
  k = in_index - ((order_rest + 1) * j) -
      ((order_rest + 1) * (order_rest + 1) * i);

  nodal_basis = eval_lagrange(in_loc(0), k, loc_1d_upts_rest) *
                eval_lagrange(in_loc(1), j, loc_1d_upts_rest) *
                eval_lagrange(in_loc(2), i, loc_1d_upts_rest);
...
double eles_hexas::eval_d_nodal_basis(int in_index, int in_cpnt,
                                      array<double> in_loc) {
  int i, j, k;
  ...
  if (in_cpnt == 0) {
    d_nodal_basis = eval_d_lagrange(in_loc(0), k, loc_1d_upts) *
                    eval_lagrange(in_loc(1), j, loc_1d_upts) *
                    eval_lagrange(in_loc(2), i, loc_1d_upts);
  } else if (in_cpnt == 1) {
    d_nodal_basis = eval_lagrange(in_loc(0), k, loc_1d_upts) *
                    eval_d_lagrange(in_loc(1), j, loc_1d_upts) *
                    eval_lagrange(in_loc(2), i, loc_1d_upts);
  } else if (in_cpnt == 2) {
    d_nodal_basis = eval_lagrange(in_loc(0), k, loc_1d_upts) *
                    eval_lagrange(in_loc(1), j, loc_1d_upts) *
                    eval_d_lagrange(in_loc(2), i, loc_1d_upts);
  } else {
...
double eles_hexas::eval_nodal_s_basis(int in_index, array<double> in_loc,
                                      int in_n_spts) {
  int i, j, k;
  ...
  if (is_perfect_cube(in_n_spts)) {
    ...
    nodal_s_basis = eval_lagrange(in_loc(0), k, loc_1d_spts) *
                    eval_lagrange(in_loc(1), j, loc_1d_spts) *
                    eval_lagrange(in_loc(2), i, loc_1d_spts);
  } else if (in_n_spts == 20)  // Quadratic hex with 20 nodes
  {
    if (in_index == 0)
      nodal_s_basis = (1. / 8. * (-1 + in_loc(0))) * (-1 + in_loc(1)) *
                      (-1 + in_loc(2)) *
                      (in_loc(0) + 2 + in_loc(1) + in_loc(2));
    else if (in_index == 1)
      nodal_s_basis = -(1. / 8. * (in_loc(0) + 1)) * (-1 + in_loc(1)) *
                      (-1 + in_loc(2)) *
                      (-in_loc(0) + 2 + in_loc(1) + in_loc(2));
    else if (in_index == 2)
      nodal_s_basis = -(1. / 8. * (in_loc(0) + 1)) * (in_loc(1) + 1) *
                      (-1 + in_loc(2)) *
                      (in_loc(0) - 2 + in_loc(1) - in_loc(2));
    else if (in_index == 3)
      nodal_s_basis = (1. / 8. * (-1 + in_loc(0))) * (in_loc(1) + 1) *
                      (-1 + in_loc(2)) *
                      (-in_loc(0) - 2 + in_loc(1) - in_loc(2));
    else if (in_index == 4)
      nodal_s_basis = -(1. / 8. * (-1 + in_loc(0))) * (-1 + in_loc(1)) *
                      (in_loc(2) + 1) * (in_loc(0) + 2 + in_loc(1) - in_loc(2));
    else if (in_index == 5)
      nodal_s_basis = (1. / 8. * (in_loc(0) + 1)) * (-1 + in_loc(1)) *
                      (in_loc(2) + 1) *
                      (-in_loc(0) + 2 + in_loc(1) - in_loc(2));
      ...
      nodal_s_basis = -(1. / 4. * (-1 + in_loc(1))) * (-1 + in_loc(2)) *
                      (in_loc(0) * in_loc(0) - 1);
    else if (in_index == 9)
      nodal_s_basis = (1. / 4. * (in_loc(0) + 1)) * (-1 + in_loc(2)) *
                      (in_loc(1) * in_loc(1) - 1);
    else if (in_index == 10)
      nodal_s_basis = (1. / 4. * (in_loc(1) + 1)) * (-1 + in_loc(2)) *
                      (in_loc(0) * in_loc(0) - 1);
    else if (in_index == 11)
      nodal_s_basis = -(1. / 4. * (-1 + in_loc(0))) * (-1 + in_loc(2)) *
                      (in_loc(1) * in_loc(1) - 1);
    else if (in_index == 12)
      nodal_s_basis = -(1. / 4. * (-1 + in_loc(0))) * (-1 + in_loc(1)) *
                      (in_loc(2) * in_loc(2) - 1);
    else if (in_index == 13)
      nodal_s_basis = (1. / 4. * (in_loc(0) + 1)) * (-1 + in_loc(1)) *
                      (in_loc(2) * in_loc(2) - 1);
    else if (in_index == 14)
      nodal_s_basis = -(1. / 4. * (in_loc(0) + 1)) * (in_loc(1) + 1) *
                      (in_loc(2) * in_loc(2) - 1);
    else if (in_index == 15)
      nodal_s_basis = (1. / 4. * (-1 + in_loc(0))) * (in_loc(1) + 1) *
                      (in_loc(2) * in_loc(2) - 1);
    else if (in_index == 16)
      nodal_s_basis = (1. / 4. * (-1 + in_loc(1))) * (in_loc(2) + 1) *
                      (in_loc(0) * in_loc(0) - 1);
    else if (in_index == 17)
      ...
      nodal_s_basis = (1. / 4. * (-1 + in_loc(0))) * (in_loc(2) + 1) *
                      (in_loc(1) * in_loc(1) - 1);
  } else {
...
void eles_hexas::eval_d_nodal_s_basis(array<double> &d_nodal_s_basis,
  ...
  if (is_perfect_cube(in_n_spts)) {
    ...
    for (int m = 0; m < in_n_spts; ++m) {
      ...
      d_nodal_s_basis(m, 0) = eval_d_lagrange(in_loc(0), k, loc_1d_spts) *
                              eval_lagrange(in_loc(1), j, loc_1d_spts) *
                              eval_lagrange(in_loc(2), i, loc_1d_spts);
      d_nodal_s_basis(m, 1) = eval_lagrange(in_loc(0), k, loc_1d_spts) *
                              eval_d_lagrange(in_loc(1), j, loc_1d_spts) *
                              eval_lagrange(in_loc(2), i, loc_1d_spts);
      d_nodal_s_basis(m, 2) = eval_lagrange(in_loc(0), k, loc_1d_spts) *
                              eval_lagrange(in_loc(1), j, loc_1d_spts) *
                              eval_d_lagrange(in_loc(2), i, loc_1d_spts);
    }

  } else if (in_n_spts == 20) {
    d_nodal_s_basis(0, 0) = (1. / 8. * (in_loc(2) - 1)) * (-1 + in_loc(1)) *
                            (in_loc(1) + in_loc(2) + 2 * in_loc(0) + 1);
    d_nodal_s_basis(1, 0) = -(1. / 8. * (in_loc(2) - 1)) * (-1 + in_loc(1)) *
                            (in_loc(1) + in_loc(2) - 2 * in_loc(0) + 1);
    d_nodal_s_basis(2, 0) = -(1. / 8. * (in_loc(2) - 1)) * (in_loc(1) + 1) *
                            (in_loc(1) - in_loc(2) + 2 * in_loc(0) - 1);
    d_nodal_s_basis(3, 0) = (1. / 8. * (in_loc(2) - 1)) * (in_loc(1) + 1) *
                            (in_loc(1) - in_loc(2) - 2 * in_loc(0) - 1);
    d_nodal_s_basis(4, 0) = -(1. / 8. * (in_loc(2) + 1)) * (-1 + in_loc(1)) *
                            (in_loc(1) - in_loc(2) + 2 * in_loc(0) + 1);
    d_nodal_s_basis(5, 0) = (1. / 8. * (in_loc(2) + 1)) * (-1 + in_loc(1)) *
                            (in_loc(1) - in_loc(2) - 2 * in_loc(0) + 1);
    d_nodal_s_basis(6, 0) = (1. / 8. * (in_loc(2) + 1)) * (in_loc(1) + 1) *
                            (in_loc(1) + in_loc(2) + 2 * in_loc(0) - 1);
    d_nodal_s_basis(7, 0) = -(1. / 8. * (in_loc(2) + 1)) * (in_loc(1) + 1) *
                            (in_loc(1) + in_loc(2) - 1 - 2 * in_loc(0));
    d_nodal_s_basis(8, 0) =
        -(1. / 2.) * in_loc(0) * (in_loc(2) - 1) * (-1 + in_loc(1));
    d_nodal_s_basis(9, 0) =
        (1. / 4. * (in_loc(2) - 1)) * (in_loc(1) * in_loc(1) - 1);
    d_nodal_s_basis(10, 0) =
        (1. / 2.) * in_loc(0) * (in_loc(2) - 1) * (in_loc(1) + 1);
    d_nodal_s_basis(11, 0) =
        -(1. / 4. * (in_loc(2) - 1)) * (in_loc(1) * in_loc(1) - 1);
    d_nodal_s_basis(12, 0) =
        -(1. / 4. * (-1 + in_loc(1))) * (in_loc(2) * in_loc(2) - 1);
    d_nodal_s_basis(13, 0) =
        (1. / 4. * (-1 + in_loc(1))) * (in_loc(2) * in_loc(2) - 1);
    d_nodal_s_basis(14, 0) =
        -(1. / 4. * (in_loc(1) + 1)) * (in_loc(2) * in_loc(2) - 1);
    d_nodal_s_basis(15, 0) =
        (1. / 4. * (in_loc(1) + 1)) * (in_loc(2) * in_loc(2) - 1);
    d_nodal_s_basis(16, 0) =
        (1. / 2.) * in_loc(0) * (in_loc(2) + 1) * (-1 + in_loc(1));
    d_nodal_s_basis(17, 0) =
        -(1. / 4. * (in_loc(2) + 1)) * (in_loc(1) * in_loc(1) - 1);
    d_nodal_s_basis(18, 0) =
        -(1. / 2.) * in_loc(0) * (in_loc(2) + 1) * (in_loc(1) + 1);
    d_nodal_s_basis(19, 0) =
        (1. / 4. * (in_loc(2) + 1)) * (in_loc(1) * in_loc(1) - 1);

    d_nodal_s_basis(0, 1) = (1. / 8. * (in_loc(2) - 1)) * (-1 + in_loc(0)) *
                            (in_loc(0) + in_loc(2) + 2 * in_loc(1) + 1);
    d_nodal_s_basis(1, 1) = -(1. / 8. * (in_loc(2) - 1)) * (in_loc(0) + 1) *
                            (-in_loc(0) + in_loc(2) + 2 * in_loc(1) + 1);
    d_nodal_s_basis(2, 1) = -(1. / 8. * (in_loc(2) - 1)) * (in_loc(0) + 1) *
                            (in_loc(0) - in_loc(2) + 2 * in_loc(1) - 1);
    d_nodal_s_basis(3, 1) = (1. / 8. * (in_loc(2) - 1)) * (-1 + in_loc(0)) *
                            (-in_loc(0) - in_loc(2) + 2 * in_loc(1) - 1);
    d_nodal_s_basis(4, 1) = -(1. / 8. * (in_loc(2) + 1)) * (-1 + in_loc(0)) *
                            (in_loc(0) - in_loc(2) + 2 * in_loc(1) + 1);
    d_nodal_s_basis(5, 1) = (1. / 8. * (in_loc(2) + 1)) * (in_loc(0) + 1) *
                            (-in_loc(0) - in_loc(2) + 2 * in_loc(1) + 1);
    d_nodal_s_basis(6, 1) = (1. / 8. * (in_loc(2) + 1)) * (in_loc(0) + 1) *
                            (in_loc(0) + in_loc(2) - 1 + 2 * in_loc(1));
    d_nodal_s_basis(7, 1) = -(1. / 8. * (in_loc(2) + 1)) * (-1 + in_loc(0)) *
                            (-in_loc(0) + in_loc(2) - 1 + 2 * in_loc(1));
    d_nodal_s_basis(8, 1) =
        -(1. / 4. * (in_loc(2) - 1)) * (in_loc(0) * in_loc(0) - 1);
    d_nodal_s_basis(9, 1) =
        (1. / 2.) * in_loc(1) * (in_loc(2) - 1) * (in_loc(0) + 1);
    d_nodal_s_basis(10, 1) =
        (1. / 4. * (in_loc(2) - 1)) * (in_loc(0) * in_loc(0) - 1);
    d_nodal_s_basis(11, 1) =
        -(1. / 2.) * in_loc(1) * (in_loc(2) - 1) * (-1 + in_loc(0));
    d_nodal_s_basis(12, 1) =
        -(1. / 4. * (-1 + in_loc(0))) * (in_loc(2) * in_loc(2) - 1);
    d_nodal_s_basis(13, 1) =
        (1. / 4. * (in_loc(0) + 1)) * (in_loc(2) * in_loc(2) - 1);
    d_nodal_s_basis(14, 1) =
        -(1. / 4. * (in_loc(0) + 1)) * (in_loc(2) * in_loc(2) - 1);
    d_nodal_s_basis(15, 1) =
        (1. / 4. * (-1 + in_loc(0))) * (in_loc(2) * in_loc(2) - 1);
    d_nodal_s_basis(16, 1) =
        (1. / 4. * (in_loc(2) + 1)) * (in_loc(0) * in_loc(0) - 1);
    d_nodal_s_basis(17, 1) =
        -(1. / 2.) * in_loc(1) * (in_loc(2) + 1) * (in_loc(0) + 1);
    d_nodal_s_basis(18, 1) =
        -(1. / 4. * (in_loc(2) + 1)) * (in_loc(0) * in_loc(0) - 1);
    d_nodal_s_basis(19, 1) =
        (1. / 2.) * in_loc(1) * (in_loc(2) + 1) * (-1 + in_loc(0));

    d_nodal_s_basis(0, 2) = (1. / 8. * (-1 + in_loc(0))) * (-1 + in_loc(1)) *
                            (in_loc(1) + in_loc(0) + 2 * in_loc(2) + 1);
    d_nodal_s_basis(1, 2) = -(1. / 8. * (in_loc(1) - 1)) * (in_loc(0) + 1) *
                            (-in_loc(0) + in_loc(1) + 2 * in_loc(2) + 1);
    d_nodal_s_basis(2, 2) = -(1. / 8. * (in_loc(0) + 1)) * (in_loc(1) + 1) *
                            (in_loc(1) + in_loc(0) - 2 * in_loc(2) - 1);
    d_nodal_s_basis(3, 2) = (1. / 8. * (-1 + in_loc(0))) * (in_loc(1) + 1) *
                            (in_loc(1) - in_loc(0) - 2 * in_loc(2) - 1);
    d_nodal_s_basis(4, 2) = -(1. / 8. * (-1 + in_loc(0))) * (-1 + in_loc(1)) *
                            (in_loc(1) + in_loc(0) - 2 * in_loc(2) + 1);
    d_nodal_s_basis(5, 2) = (1. / 8. * (in_loc(0) + 1)) * (-1 + in_loc(1)) *
                            (in_loc(1) - in_loc(0) - 2 * in_loc(2) + 1);
    d_nodal_s_basis(6, 2) = (1. / 8. * (in_loc(0) + 1)) * (in_loc(1) + 1) *
                            (in_loc(1) + in_loc(0) + 2 * in_loc(2) - 1);
    d_nodal_s_basis(7, 2) = -(1. / 8. * (-1 + in_loc(0))) * (in_loc(1) + 1) *
                            (in_loc(1) - in_loc(0) + 2 * in_loc(2) - 1);
    d_nodal_s_basis(8, 2) =
        -(1. / 4. * (-1 + in_loc(1))) * (in_loc(0) * in_loc(0) - 1);
    d_nodal_s_basis(9, 2) =
        (1. / 4. * (in_loc(0) + 1)) * (in_loc(1) * in_loc(1) - 1);
    d_nodal_s_basis(10, 2) =
        (1. / 4. * (in_loc(1) + 1)) * (in_loc(0) * in_loc(0) - 1);
    d_nodal_s_basis(11, 2) =
        -(1. / 4. * (-1 + in_loc(0))) * (in_loc(1) * in_loc(1) - 1);
    d_nodal_s_basis(12, 2) =
        -(1. / 2.) * in_loc(2) * (-1 + in_loc(0)) * (-1 + in_loc(1));
    d_nodal_s_basis(13, 2) =
        (1. / 2.) * in_loc(2) * (in_loc(0) + 1) * (-1 + in_loc(1));
    d_nodal_s_basis(14, 2) =
        -(1. / 2.) * in_loc(2) * (in_loc(0) + 1) * (in_loc(1) + 1);
    d_nodal_s_basis(15, 2) =
        (1. / 2.) * in_loc(2) * (-1 + in_loc(0)) * (in_loc(1) + 1);
    d_nodal_s_basis(16, 2) =
        (1. / 4. * (-1 + in_loc(1))) * (in_loc(0) * in_loc(0) - 1);
    d_nodal_s_basis(17, 2) =
        -(1. / 4. * (in_loc(0) + 1)) * (in_loc(1) * in_loc(1) - 1);
    d_nodal_s_basis(18, 2) =
        -(1. / 4. * (in_loc(1) + 1)) * (in_loc(0) * in_loc(0) - 1);
    d_nodal_s_basis(19, 2) =
        (1. / 4. * (-1 + in_loc(0))) * (in_loc(1) * in_loc(1) - 1);
  } else {
    FatalError("Shape basis not implemented yet, exiting");
  }
...
double eles_hexas::eval_div_vcjh_basis(int in_index, array<double> &loc) {
  ...
  if (scheme < 5) {
    if (i == 0)
      div_vcjh_basis = -eval_lagrange(loc(0), order - k, loc_1d_upts) *
                       eval_lagrange(loc(1), j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(2), 0, order, eta);
    else if (i == 1)
      div_vcjh_basis = -eval_lagrange(loc(0), k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(1), 0, order, eta);
    else if (i == 2)
      div_vcjh_basis = eval_lagrange(loc(1), k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(0), 1, order, eta);
    else if (i == 3)
      div_vcjh_basis = eval_lagrange(loc(0), order - k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(1), 1, order, eta);
    else if (i == 4)
      div_vcjh_basis = -eval_lagrange(loc(1), order - k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(0), 0, order, eta);
    else if (i == 5)
      div_vcjh_basis = eval_lagrange(loc(0), k, loc_1d_upts) *
                       eval_lagrange(loc(1), j, loc_1d_upts) *
                       eval_d_vcjh_1d(loc(2), 1, order, eta);
  ...
  // OFR scheme
    ...
    if (i == 0)
      div_vcjh_basis = -eval_lagrange(loc(0), order - k, loc_1d_upts) *
                       eval_lagrange(loc(1), j, loc_1d_upts) *
                       eval_d_ofr_1d(loc(2), 0, order);
    else if (i == 1)
      div_vcjh_basis = -eval_lagrange(loc(0), k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_ofr_1d(loc(1), 0, order);
    else if (i == 2)
      div_vcjh_basis = eval_lagrange(loc(1), k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_ofr_1d(loc(0), 1, order);
    else if (i == 3)
      div_vcjh_basis = eval_lagrange(loc(0), order - k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_ofr_1d(loc(1), 1, order);
    else if (i == 4)
      div_vcjh_basis = -eval_lagrange(loc(1), order - k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_ofr_1d(loc(0), 0, order);
    else if (i == 5)
      div_vcjh_basis = eval_lagrange(loc(0), k, loc_1d_upts) *
                       eval_lagrange(loc(1), j, loc_1d_upts) *
                       eval_d_ofr_1d(loc(2), 1, order);
  ...
  // OESFR scheme
    ...
    if (i == 0)
      div_vcjh_basis = -eval_lagrange(loc(0), order - k, loc_1d_upts) *
                       eval_lagrange(loc(1), j, loc_1d_upts) *
                       eval_d_oesfr_1d(loc(2), 0, order);
    else if (i == 1)
      div_vcjh_basis = -eval_lagrange(loc(0), k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_oesfr_1d(loc(1), 0, order);
    else if (i == 2)
      div_vcjh_basis = eval_lagrange(loc(1), k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_oesfr_1d(loc(0), 1, order);
    else if (i == 3)
      div_vcjh_basis = eval_lagrange(loc(0), order - k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_oesfr_1d(loc(1), 1, order);
    else if (i == 4)
      div_vcjh_basis = -eval_lagrange(loc(1), order - k, loc_1d_upts) *
                       eval_lagrange(loc(2), j, loc_1d_upts) *
                       eval_d_oesfr_1d(loc(0), 0, order);
    else if (i == 5)
      div_vcjh_basis = eval_lagrange(loc(0), k, loc_1d_upts) *
                       eval_lagrange(loc(1), j, loc_1d_upts) *
                       eval_d_oesfr_1d(loc(2), 1, order);
  }
