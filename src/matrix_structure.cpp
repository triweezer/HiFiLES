...
void CSysMatrix::Initialize(int n_verts, int n_verts_global, int n_var,
                            int n_eqns, array<array<int>> &v2e,
                            array<int> &v2n_e, array<int> &e2v) {
  ...
  /*--- Don't delete *row_ptr, *col_ind because they are asigned to the Jacobian
   * structure. ---*/
  ...
  for (iPoint = 0; iPoint < nPoint; iPoint++)
    row_ptr[iPoint + 1] =
        row_ptr[iPoint] +
        (v2n_e(iPoint) + 1);  // +1 -> to include diagonal element
  nnz = row_ptr[nPoint];
  ...
  for (iPoint = 0; iPoint < nPoint; iPoint++) {
    ...
    std::sort(vneighs, vneighs + nNeigh + 1);
...
void CSysMatrix::SetIndexes(int n_verts, int n_verts_global, int n_var,
                            int n_eqns, unsigned long *val_row_ptr,
                            unsigned long *val_col_ind, unsigned long val_nnz) {
  nPoint = n_verts;  // Assign number of points in the mesh (on processor)
  nPointDomain =
      n_verts_global;  // Assign number of points in the mesh (across all procs)
  ...
  matrix = new double[nnz * nVar *
                      nEqn];  // Reserve memory for the values of the matrix
  block = new double[nVar * nEqn];
  prod_block_vector = new double[nEqn];
  prod_row_vector = new double[nVar];
  aux_vector = new double[nVar];
  invM = new double[nPoint * nVar * nEqn];  // Reserve memory for the values of
...
void CSysMatrix::SetBlock(unsigned long block_i, unsigned long block_j,
                          double **val_block) {
  ...
  for (index = row_ptr[block_i]; index < row_ptr[block_i + 1]; index++) {
    ...
    if (col_ind[index] == block_j) {
      for (iVar = 0; iVar < nVar; iVar++)
        for (jVar = 0; jVar < nEqn; jVar++)
          matrix[(row_ptr[block_i] + step - 1) * nVar * nEqn + iVar * nEqn +
                 jVar] = val_block[iVar][jVar];
      break;
    }
  }
}

void CSysMatrix::AddBlock(unsigned long block_i, unsigned long block_j,
                          double **val_block) {
  unsigned long iVar, jVar, index, step = 0;
...
void CSysMatrix::SubtractBlock(unsigned long block_i, unsigned long block_j,
                               double **val_block) {
  unsigned long iVar, jVar, index, step = 0;
...
void CSysMatrix::AddVal2Diag(unsigned long block_i, double val_matrix) {
  ...
  for (index = row_ptr[block_i]; index < row_ptr[block_i + 1]; index++) {
    ...
    if (col_ind[index] == block_i) {  // Only elements on the diagonal
      for (iVar = 0; iVar < nVar; iVar++)
        matrix[(row_ptr[block_i] + step - 1) * nVar * nVar + iVar * nVar +
               iVar] += val_matrix;
      break;
    }
  }
...
void CSysMatrix::AddVal2Diag(unsigned long block_i, double *val_matrix,
  ...
  for (unsigned long index = row_ptr[block_i]; index < row_ptr[block_i + 1];
       index++) {
    step++;
    if (col_ind[index] == block_i) {  // Only elements on the diagonal
      for (iVar = 0; iVar < nVar; iVar++) {
...
void CSysMatrix::AddVal2Diag(unsigned long block_i, double *val_matrix,
  ...
  for (unsigned long index = row_ptr[block_i]; index < row_ptr[block_i + 1];
    ...
    if (col_ind[index] == block_i) {  // Only elements on the diagonal
      for (iVar = 0; iVar < nVar; iVar++) {
        if (iVar < (val_nDim + 3) * val_nDiatomics)
          iSpecies = iVar / (val_nDim + 3);
        else
          iSpecies = (iVar - (val_nDim + 3) * val_nDiatomics) / (val_nDim + 2) +
                     val_nDiatomics;
        matrix[(row_ptr[block_i] + step - 1) * nVar * nVar + iVar * nVar +
               iVar] += val_matrix[iSpecies];
      }
...
void CSysMatrix::DeleteValsRowi(unsigned long i) {
  ...
  for (index = row_ptr[block_i]; index < row_ptr[block_i + 1]; index++) {
    for (iVar = 0; iVar < nVar; iVar++)
      matrix[index * nVar * nVar + row * nVar + iVar] =
          0.0;  // Delete row values in the block
    if (col_ind[index] == block_i)
      matrix[index * nVar * nVar + row * nVar + row] =
          1.0;  // Set 1 to the diagonal element
  }
}
...
void CSysMatrix::Gauss_Elimination(unsigned long block_i, double *rhs) {
  ...
  if (nVar == 1) {
    if (fabs(block[0]) < eps)
      std::cout << "Gauss' elimination error, value:" << abs(block[0]) << "."
           << std::endl;
    rhs[0] /= block[0];
  } else {
    // std::cout << "Performing Gauss Elimination to get UT matrix" << std::endl;
    /*std::cout << "Block (" << block_i << "," << block_i << "):" << std::endl;
    DisplayBlock();*/
    /*--- Transform system in Upper Matrix ---*/
    for (iVar = 1; iVar < (short)nVar; iVar++) {
      for (jVar = 0; jVar < iVar; jVar++) {
        if (fabs(block[jVar * nVar + jVar]) < eps)
          std::cout << "Gauss' elimination error, value:"
               << fabs(block[jVar * nVar + jVar]) << "." << std::endl;
    ...
    if (fabs(block[nVar * nVar - 1]) < eps)
      std::cout << "Gauss' elimination error, value:" << fabs(block[nVar * nVar - 1])
           << "." << std::endl;
    ...
    for (iVar = nVar - 2; iVar >= 0; iVar--) {
      ...
      if (fabs(block[iVar * nVar + iVar]) < eps)
        std::cout << "Gauss' elimination error, value:"
             << fabs(block[iVar * nVar + iVar]) << "." << std::endl;
...
void CSysMatrix::Gauss_Elimination(double *Block, double *rhs) {
  ...
  if (nVar == 1) {
    if (fabs(block[0]) < eps) std::cout << "Gauss' elimination error." << std::endl;
    rhs[0] /= block[0];
  } else {
    /*--- Transform system in Upper Matrix ---*/
    for (iVar = 1; iVar < (short)nVar; iVar++) {
      for (jVar = 0; jVar < iVar; jVar++) {
        if (fabs(block[jVar * nVar + jVar]) < eps)
          std::cout << "Gauss' elimination error." << std::endl;
        weight = block[iVar * nVar + jVar] / block[jVar * nVar + jVar];
        for (kVar = jVar; kVar < nVar; kVar++)
          block[iVar * nVar + kVar] -= weight * block[jVar * nVar + kVar];
        rhs[iVar] -= weight * rhs[jVar];
...
void CSysMatrix::ProdBlockVector(unsigned long block_i, unsigned long block_j,
...
void CSysMatrix::UpperProduct(CSysVector &vec, unsigned long row_i) {
  ...
  for (iVar = 0; iVar < nVar; iVar++) prod_row_vector[iVar] = 0;
...
#  endif
  ...
  for (iMarker = 0; iMarker < config->GetnMarker_All(); iMarker++) {
    ...
    if ((config->GetMarker_All_Boundary(iMarker) == SEND_RECEIVE) &&
      ...
      nVertexS = FlowSol->nVertex[MarkerS];
      nVertexR = FlowSol->nVertex[MarkerR];
...
#  ifdef _MPI
      ...
      MPI_Sendrecv(Buffer_Send, nBufferS_Vector, MPI_DOUBLE, send_to, 0,
                   Buffer_Receive, nBufferR_Vector, MPI_DOUBLE, receive_from,
                   0);
#  else
      ...
      for (iVertex = 0; iVertex < nVertexR; iVertex++) {
        ...
        for (iVar = 0; iVar < nVar; iVar++)
          Buffer_Receive[iVar * nVertexR + iVertex] =
              Buffer_Send[iVar * nVertexR + iVertex];
      }
...
void CSysMatrix::MatrixVectorProduct(const CSysVector &vec, CSysVector &prod) {
  ...
  if ((nVar != vec.GetNVar()) || (nVar != prod.GetNVar())) {
    std::cerr << "CSysMatrix::MatrixVectorProduct(const CSysVector&, CSysVector): "
         << "nVar values incompatible." << std::endl;
    throw(-1);
  }
  if ((nPoint != vec.GetNBlk()) || (nPoint != prod.GetNBlk())) {
    std::cerr << "CSysMatrix::MatrixVectorProduct(const CSysVector&, CSysVector): "
         << "nPoint and nBlk values incompatible." << std::endl;
    throw(-1);
  }
  ...
  for (row_i = 0; row_i < nPointDomain; row_i++) {
    ...
    for (index = row_ptr[row_i]; index < row_ptr[row_i + 1]; index++) {
      vec_begin =
          col_ind[index] * nVar;  // offset to beginning of block col_ind[index]
      mat_begin =
          (index * nVar *
           nVar);  // offset to beginning of matrix block[row_i][col_ind[indx]]
      for (iVar = 0; iVar < nVar; iVar++) {
        for (jVar = 0; jVar < nVar; jVar++) {
          prod[(const unsigned int)(prod_begin + iVar)] +=
              matrix[(const unsigned int)(mat_begin + iVar * nVar + jVar)] *
              vec[(const unsigned int)(vec_begin + jVar)];
        }
      }
    }
  }
...
void CSysMatrix::GetSubsVector(double *c, double *a, double *b) {
  ...
  for (iVar = 0; iVar < nVar; iVar++) c[iVar] = a[iVar] - b[iVar];
}
...
void CSysMatrix::InverseBlock(double *Block, double *invBlock) {
  ...
  for (iVar = 0; iVar < nVar; iVar++) {
    for (jVar = 0; jVar < nVar; jVar++) aux_vector[jVar] = 0.0;
    aux_vector[iVar] = 1.0;
...
void CSysMatrix::InverseDiagonalBlock(unsigned long block_i,
  ...
  for (iVar = 0; iVar < nVar; iVar++) {
    ...
    for (jVar = 0; jVar < nVar; jVar++) invBlock[jVar][iVar] = aux_vector[jVar];
  }
}
...
void CSysMatrix::ComputeLU_SGSPreconditioner(const CSysVector &vec,
  ...
  /*--- There are two approaches to the parallelization (AIAA-2000-0927):
   1. Use a special scheduling algorithm which enables data parallelism by
   regrouping edges. This method has the advantage of producing exactly the same
   result as the single processor case, but it suffers from severe overhead
      penalties for parallel loop initiation, heavy interprocessor
   communications and poor load balance.
   2. Split the computational domain into several nonoverlapping regions
   according to the number of processors, and apply the SGS method inside of
   each region with (or without) some special interprocessor boundary treatment.
   This approach
      may suffer from convergence degradation but takes advantage of minimal
   parallelization overhead and good load balance. ---*/

  /*--- First part of the symmetric iteration: (D+L).x* = b ---*/
  /*std::cout <<"--ComputLU_SGSPreconditioner--" << std::endl;
  std::cout << "n_dims: " << nVar << ", nPointDomain: " << nPointDomain << std::endl;
  std::cout << "--paused--" << std::endl; cin.get();*/
  for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
    LowerProduct(prod, iPoint);  // Compute L.x*
    for (iVar = 0; iVar < nVar; iVar++)
      aux_vector[iVar] =
          vec[iPoint * nVar + iVar] -
          prod_row_vector[iVar];            // Compute aux_vector = b - L.x*
  ...
  for (iPoint = nPointDomain - 1; (int)iPoint >= 0; iPoint--) {
    ...
    for (iVar = 0; iVar < nVar; iVar++)
      aux_vector[iVar] -=
          prod_row_vector[iVar];  // Compute aux_vector = D.x*-U.x_(n+1)
    Gauss_Elimination(iPoint, aux_vector);  // Solve D.x* = aux_vector
    for (iVar = 0; iVar < nVar; iVar++)
      prod[iPoint * nVar + iVar] =
          aux_vector[iVar];  // Assesing x_(1) = solution
  }

  /*--- Final send-receive operation the solution vector (redundant in CFD
   * simulations) ---*/
  // SendReceive_Solution(prod, geometry, config);
}
